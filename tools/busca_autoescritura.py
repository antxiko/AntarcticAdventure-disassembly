...
"""Busca escrituras a la ROM: el efecto de una proteccion, no una forma concreta.
    ...
    python3 tools/busca_autoescritura.py <version> [...]

POR QUE ESTA HERRAMIENTA. La primera vez que se busco la proteccion anticopia
de este cartucho se busco UN PATRON: `ld hl,nn / ld de,nn / ld bc,0003h / ldir`.
Y de no encontrarlo en una version se concluyo que esa version no tenia
proteccion. Eso no se sigue: basta con otro orden de registros, otra longitud o
un mecanismo distinto para que la busqueda no vea nada. Buscar una forma solo
sirve para confirmar que ESA forma no esta.
...
se marca todo lo que escriba por debajo de 0x8000:

  - `ld (nn),a` y `ld (nn),hl`, y los `ld (nn),rr` con prefijo ED, cuando nn
    cae en la BIOS (0x0000-0x3FFF) o en el propio cartucho (0x4000-0x7FFF);
  - LDIR y LDDR, mirando hacia atras que se cargo en DE: si el destino es un
    inmediato por debajo de 0x8000, esa copia va a la ROM;
...
def analiza(v):
    ...
    blocks = json.load(open("work/%s/antarctic.trace.json" % v))["blocks"]
        ...
        if op == 0xED and by(a + 1) in (0xB0, 0xB8):
            ...
            for j in range(i - 1, max(-1, i - 8), -1):
                ...
                if by(aj) == 0x11:                      # ld de,nn
                    destino = wd(aj + 1)
                    break
            if destino is not None and destino < 0x8000:
...
def main():
    versiones = sys.argv[1:] or ["jap1", "jap2", "eu"]
    for v in versiones:
        h = analiza(v)
        print("=" * 70)
        print(" %s: %d escrituras a memoria por debajo de 0x8000" % (v, len(h)))
        print("=" * 70)
        for a, texto, destino, clase in h:
            zona = ("BIOS" if destino < 0x4000 else "EL PROPIO CARTUCHO")
