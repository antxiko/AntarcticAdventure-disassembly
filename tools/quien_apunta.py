...
def main():
    ...
    traza = json.load(open(os.path.join(work, "antarctic.trace.json")))
    blocks = traza["blocks"]
    notas = rangos_d(os.path.join(src, "antarctic.notes"))

    # La ROM: la deducimos del propio nombre de la carpeta de trabajo.
    v = os.path.basename(work.rstrip("/"))
    rom = "antarctic-%s.rom" % v
