...
H_TIMI:		; Rutina de interrupcion; INIT la engancha en 0xFD9A
	push af			;405d   ; el gancho H_TIMI guarda los cuatro pares que va a tocar y los devuelve al salir
	push bc			;405e
	push de			;405f
	push hl			;4060
	di			;4061
...
HT_YA_DENTRO:		; Salida cuando la vuelta anterior sigue trabajando: este fotograma se pierde
	pop hl			;40ac   ; Recupera los registros y se va: el cerrojo sigue echado y lo quitara la vuelta que trabaja
	pop de			;40ad
	pop bc			;40ae
	pop af			;40af
	ei			;40b0
...
DESPACHA:		; Salta al destino A de la tabla que va detras del CALL
	add a,a			;40b2   ; el indice por dos, que la tabla es de palabras
	pop hl			;40b3   ; La direccion de retorno es la tabla
	call SUMA_A_HL		;40b4
	ld e,(hl)			;40b7   ; la palabra apuntada es el destino, y el `jp (hl)` salta sin volver aqui
	inc hl			;40b8
	ld d,(hl)			;40b9
	ex de,hl			;40ba
	jp (hl)			;40bb
...
LEE_MANDOS:		; Deja en 0xE009 lo pulsado ahora y en 0xE008 lo de antes
	ld a,(0e000h)		;40bc   ; antes del estado 7 no hay nada que leer: no se juega
	cp 007h		;40bf   ; Antes del estado 7 (la demo) no se lee nada
	ret c			;40c1
	ld a,(0e002h)		;40c2
	bit 6,a		;40c5   ; Bit 6 de 0xE002: sin partida, los mandos salen de la grabacion de la demo
...
GUARDA_MANDOS:		; 0xE009 = ahora, 0xE008 = el fotograma anterior
	ld hl,0e009h		;40d5
	ld c,(hl)			;40d8   ; C hace de tercera mano: lo que estaba en 0xE009 baja a 0xE008 sin tocar A
	ld (hl),a			;40d9
	dec hl			;40da
	ld (hl),c			;40db
	ret			;40dc
LEE_TECLADO:		; Monta con las filas 7 y 8 de la matriz el mismo mapa de bits que el joystick
	...
	cpl			;40e2   ; El `cpl` endereza la logica negativa de SNSMAT y la rotacion baja la tecla al bit 5: el segundo gatillo
	rrca			;40e3
	and 020h		;40e4
	ld e,a			;40e6
	ld a,008h		;40e7
	...
	rrca			;40ee
	ld b,a			;40ef
	and 004h		;40f0   ; Tres tandas de rotaciones recolocan la fila 8 en el mapa del joystick: izquierda al bit 2, derecha y espacio al 3 y al 4, arriba y abajo al 0 y al 1
	or e			;40f2
	ld c,a			;40f3
	ld a,b			;40f4
	rrca			;40f5
...
; ----------------------------------------------------------------------
; DATOS tres_bytes_jp_cero: Los tres bytes C3 00 00 que INIT copia encima del
;   despachador. Ver la nota de 0x404C
;   0x411f..0x4122  (3 bytes)
DATA_tres_bytes_jp_cero:
...
; ----------------------------------------------------------------------
; DATOS tabla_de_estados: Los DIECISEIS destinos de la maquina de estados,
;   indexados por 0xE000. Va incrustada detras del CALL de 0x412C y acaba
;   justo donde empieza su primer destino, que es lo que fija su tamano
;   0x412f..0x414f  (32 bytes)
DATA_tabla_de_estados:
	defw 0414fh	; 412f  -> ESTADO_00_PARADO
	defw 04150h	; 4131  -> ESTADO_01_ARRANCA
	defw 04166h	; 4133  -> ESTADO_02_LOGO
	defw 04178h	; 4135  -> ESTADO_03_ESPERA
	defw 04183h	; 4137  -> ESTADO_04_ROTULO
	defw 04191h	; 4139  -> ESTADO_05_ESPERA
	defw 04199h	; 413b  -> ESTADO_06_CORTINILLA
	defw 041a0h	; 413d  -> ESTADO_07_DEMO
	defw 041efh	; 413f  -> ESTADO_08_MENU
	defw 0425bh	; 4141  -> ESTADO_09_PREPARA
	defw 0429dh	; 4143  -> ESTADO_10_ENTRA
	defw 042b6h	; 4145  -> ESTADO_11_PARTIDA
	defw 042dch	; 4147  -> ESTADO_12_TIME_OUT
	defw 04301h	; 4149  -> ESTADO_13_FIN
	defw 04312h	; 414b  -> ESTADO_14_META
	defw 04908h	; 414d  -> ESTADO_15_MAPA
...
ESTADO_01_ARRANCA:		; Estado 1: prepara la pantalla de presentacion
	call MONTA_LA_FUENTE		;4150   ; Estado 1: el titulo, con la cortinilla ya montada
...
ESTADO_02_LOGO:		; Estado 2: sube el logotipo una fila cada dos fotogramas, y al llegar descomprime la pantalla de titulo
	ld a,(0e003h)		;4166   ; el logo sube un fotograma si y otro no: la mitad de velocidad
	rra			;4169   ; Un fotograma si y otro no
	ret nc			;416a
	call SUBE_LOGO		;416b
	ret nz			;416e
	ld hl,05839h		;416f   ; Los datos comprimidos de la pantalla de titulo
	call DESCOMPRIME		;4172   ; la pantalla de titulo va comprimida y se escribe con la misma rutina que los rotulos
	jp ESPERA_80_Y_ESTADO		;4175
ESTADO_03_ESPERA:		; Estado 3: espera a que 0xE004 llegue a cero y prepara el rotulo
	ld hl,0e004h		;4178
	dec (hl)			;417b
	ret nz			;417c
	call PREPARA_ROTULO		;417d   ; el rotulo se prepara antes de dibujarlo, que va letra a letra
	jp ESPERA_A_Y_ESTADO		;4180
ESTADO_04_ROTULO:		; Estado 4: dibuja el rotulo columna a columna; mientras dibuja, vuelve con acarreo
	call DIBUJA_ROTULO		;4183   ; mientras DIBUJA_ROTULO devuelva acarreo, sigue escribiendo
	ret c			;4186
...
ESTADO_06_CORTINILLA:		; Estado 6: borra la pantalla por columnas; sale cuando la cortinilla vuelve negativa
	call CORTINILLA		;4199   ; la cortinilla devuelve el signo: mientras sea negativo, sigue
	ret p			;419c
...
ESTADO_07_DEMO:		; Estado 7: la partida de demostracion, en tres pasos
	ld a,(0e001h)		;41a0   ; 0xE001 es el paso dentro del estado, y tiene su propia tabla
	call DESPACHA		;41a3
...
DATA_tabla_demo:
	defw 041ach	; 41a6  -> DEMO_0_ARRANCA
	defw 041c3h	; 41a8  -> DEMO_1_CORRE
	defw 041e4h	; 41aa  -> DEMO_2_SALE
...
DEMO_0_ARRANCA:		; Paso 0: pone a cero la partida y arranca la grabacion
	call REINICIA_PARTIDA		;41ac   ; la demo empieza como una partida: se reinicia todo igual
	ld hl,0e002h		;41af
...
DEMO_1_CORRE:		; Paso 1: mientras corre la demo, escribe el aviso y descuenta
	...
	ld hl,(0e0eeh)		;41d4
	dec hl			;41d7   ; un paso menos de los 1852
	ld (0e0eeh),hl		;41d8
	ld a,h			;41db
	or l			;41dc
	ret nz			;41dd
	ld (0e133h),a		;41de   ; al agotarse se para el reloj y se pasa de estado
	jp ESPERA_80_Y_PASO		;41e1
DEMO_2_SALE:		; Paso 2: cortinilla, y vuelta al estado 0
	call CORTINILLA		;41e4
	ret p			;41e7
	xor a			;41e8   ; el estado vuelve a 0: la demo termina donde empezo
	ld (0e000h),a		;41e9
...
DATA_tabla_menu:
	defw 041fdh	; 41f5  -> MENU_0_PINTA
	defw 0420eh	; 41f7  -> MENU_1_ROTULO
	defw 04221h	; 41f9  -> MENU_2_PARPADEA
	defw 04251h	; 41fb  -> MENU_3_CORTINILLA
...
MENU_0_PINTA:		; Paso 0: limpia y prepara el rotulo
	call BORRA_SPRITES		;41fd   ; el menu se monta con la pantalla limpia: sprites y nombres
...
MENU_1_ROTULO:		; Paso 1: dibuja el rotulo entero de una vez, sin repartirlo por fotogramas
	call DIBUJA_ROTULO		;420e   ; el rotulo se dibuja poco a poco, y mientras haya acarreo se vuelve
	jr c,MENU_1_ROTULO		;4211
	ld hl,057eeh		;4213
	call ESCRIBE_CADENA		;4216
	ld a,006h		;4219   ; seis parpadeos de la opcion elegida antes de entrar
...
MENU_2_PARPADEA:		; Paso 2: parpadea la linea elegida, ocho fotogramas encendida y ocho apagada
	ld hl,0e003h		;4221
	ld a,(hl)			;4224   ; ocho fotogramas por medio parpadeo: el `and 7` deja pasar uno de cada ocho...
	and 007h		;4225
	ret nz			;4227
	ld a,(hl)			;4228
	bit 3,a		;4229   ; ...y el bit 3 dice si toca encender o apagar
	jr nz,MENU_2_ENCIENDE		;422b
	ld de,03a00h		;422d   ; 0x3A00 es la fila 16 de la tabla de nombres
	ld bc,00020h		;4230   ; 0x20 casillas: la fila entera
	ld a,(0e002h)		;4233   ; Bit 4 de 0xE002: 0 joystick (fila 16), 1 teclado (fila 18)
	and 010h		;4236
	rlca			;4238   ; dos rotaciones: el bit 4 se convierte en 0x40, que son dos filas
	rlca			;4239
	call SUMA_A_DE		;423a   ; Suma 0x00 o 0x40 al destino, o sea dos filas
	ld a,001h		;423d
	call RELLENA_VRAM		;423f   ; el tile 1 es el blanco: apagar la linea es rellenarla con el
	ret			;4242
MENU_2_ENCIENDE:		; La otra mitad del parpadeo: repinta el texto
	ld hl,057eeh		;4243   ; y encenderla es volver a escribir el texto
	call ESCRIBE_CADENA		;4246
	ld hl,0e18dh		;4249   ; 0xE18D cuenta los parpadeos que quedan; agotados, la espera de 0x80 y al paso siguiente
	dec (hl)			;424c   ; y un parpadeo menos
	ret nz			;424d
	jp ESPERA_80_Y_PASO		;424e
MENU_3_CORTINILLA:		; Paso 3: cortinilla y a empezar la partida
	call CORTINILLA		;4251   ; la cortinilla otra vez, y detras el reinicio de la partida
	ret p			;4254   ; el `ret p` vuelve mientras la cortinilla no acabe
...
ESTADO_09_PREPARA:		; Estado 9: saca de la tabla la distancia y el tiempo de la fase que toca
	...
	daa			;427f   ; el `daa` detras de cada resta: el tiempo va en BCD y hay que ajustarlo a mano
	ld c,a			;4280
	ld a,e			;4281
	sub c			;4282   ; y del tiempo de la fase se descuenta lo que sobro
	jr nc,PREPARA_AJUSTA		;4283
	daa			;4285
	dec d			;4286   ; la resta se lleva una centena
...
PREPARA_TIEMPO:
	...
	call PINTA_PANEL		;428f   ; el panel y la fuente se montan antes de que se vea nada
...
ESTADO_10_ENTRA:		; Estado 10: cortinilla, monta la pista y suena la musica de salida
	...
	ret p			;42a0
	call MONTA_LA_FASE		;42a1   ; con la cortinilla ya pasada, la fase se monta entera
	ld a,(0e002h)		;42a4
	bit 6,a		;42a7   ; En la demo no suena
	ld a,08ah		;42a9   ; el sonido 0x8A es el del arranque de la fase
	call nz,PIDE_SONIDO		;42ab
	ld a,001h		;42ae
	ld (0e133h),a		;42b0   ; y el reloj se echa a andar
...
ESTADO_11_PARTIDA:		; Estado 11: la partida. Un paso de juego por fotograma hasta que se acaba el tiempo o se llega a la meta
	ld a,(0e002h)		;42b6   ; el bit 6 de 0xE002 distingue la partida de verdad de la demo
	bit 6,a		;42b9
	...
	ld a,l			;42c6
	ld hl,0e133h		;42c7
	ld (hl),000h		;42ca   ; y el reloj se para
	or a			;42cc
...
PARTIDA_ERA_DEMO:		; Si no habia partida de verdad, vuelve al paso 1 de la demo
	ld hl,00107h		;42d7   ; estado 7, paso 1: la demo se recoge por otro lado
	jr PONE_ESTADO		;42da
ESTADO_12_TIME_OUT:		; Estado 12: se acabo el tiempo
	...
	ld de,00004h		;42e3
	ld b,004h		;42e6   ; cuatro nubes
TIME_OUT_SPRITES:
	ld (hl),0e0h		;42e8   ; la Y 0xE0 deja el sprite fuera de la pantalla
	add hl,de			;42ea
	djnz TIME_OUT_SPRITES		;42eb
	call VUELCA_ATRIBUTOS		;42ed   ; y los atributos se vuelcan ya, sin esperar al fotograma siguiente
	ld (0e0e2h),a		;42f0
	ld a,08ch		;42f3
...
ESTADO_13_FIN:		; Estado 13: espera a que acabe la musica y vuelve a la demo
	...
	or a			;4304
	ret nz			;4305
	ld hl,0e002h		;4306   ; y al soltarse se apaga la partida: lo que venga despues es demo otra vez
...
DATA_tabla_meta:
	defw 04328h	; 4318  -> META_0_FRENA
	defw 0433bh	; 431a  -> META_1_SIGUIENTE
	defw 0437bh	; 431c  -> META_2_ANDA
	defw 04389h	; 431e  -> META_3_LLEGA
	defw 043a6h	; 4320  -> META_4_SALUDA
	defw 043d0h	; 4322  -> META_5_ESPERA
	defw 043d9h	; 4324  -> META_6_BONUS
	defw 04400h	; 4326  -> META_7_CORTINILLA
...
META_0_FRENA:		; Paso 0: espera a que el pinguino termine de frenar
	ld hl,0e0f9h		;4328   ; 0xE0F9 es lo que le queda al salto por frenar
	ld a,(hl)			;432b   ; A cero ya estaba parado: al paso siguiente sin mas
	or a			;432c
	jp z,SIGUIENTE_PASO		;432d
	call SIGUE_SALTO		;4330   ; Un paso mas de frenada; si con el llega a cero, al paso siguiente en este mismo fotograma
	ld a,(0e0f9h)		;4333
	or a			;4336
	ret nz			;4337
...
META_1_SIGUIENTE:		; Paso 1: sube el numero de fase y guarda el tiempo que ha sobrado. Son DOS contadores distintos y conviene no mezclarlos: 0xE0E0 es el numero que se ve en el panel, en BCD y SIN TOPE -sigue subiendo a 11, 12...-, y 0xE0E1 es el indice 0-9 de la base, que DA LA VUELTA al llegar a diez. Esa pareja es la vuelta completa: el juego arranca con 0xE0E1 = 0, que es FRANCE (los valores iniciales estan en 0x4491), y cada llegada lo sube UNO, asi que se va de una base a la siguiente hasta que en la decima se vuelve al 0 y se cierra el circuito en Francia. Como el numero del panel no se reinicia, la vuelta siguiente son las mismas diez bases pero mas dificiles, que es lo que mira 0x76F7
	ld hl,0e0e0h		;433b   ; 0xE0E0 es el numero de fase que se ve, en BCD, y 0xE0E1 el indice de verdad
	ld a,(hl)			;433e
	add a,001h		;433f   ; El numero que se ve, en BCD
	daa			;4341   ; el `daa` ajusta la suma, que el numero va en BCD
	ld (hl),a			;4342
	inc hl			;4343
	ld a,(hl)			;4344   ; Y el indice 0-9, que da la vuelta al llegar a diez
	ld c,a			;4345
	inc a			;4346
	cp 00ah		;4347   ; diez fases y vuelta a empezar
...
META_1_GUARDA:
	...
	ld (hl),a			;435a
	xor a			;435b
	ld (0e00dh),a		;435c   ; el aviso de meta se consume
	ld hl,0e0e8h		;435f   ; La casilla del mapa tambien avanza y da la vuelta
	inc (hl)			;4362
	ld a,(hl)			;4363
	cp 00ah		;4364   ; diez fases y vuelta a empezar
...
META_1_ANIMA:
	...
	ld h,a			;436d
	ld l,001h		;436e
	ld (0e138h),hl		;4370   ; 0xE138 arranca la animacion de la vuelta completa
	ld a,013h		;4373
...
META_2_ANDA:		; Paso 2: el pinguino sigue andando hasta la bandera
	ld c,0ffh		;437b   ; C = 0xFF: que ANDA_HASTA_LA_BASE calcule la X de destino en su primer paso
	call ANDA_HASTA_LA_BASE		;437d   ; el pinguino anda solo hasta la base; hasta que no llega, no se pasa de paso
	ret nz			;4380   ; hasta que no llega, no se pasa de paso
	ld a,00ch		;4381   ; Llegado (dieciseis pasos), el paso 3 arranca con la cuenta en 12: cuatro pasos mas de subida
	ld (0e138h),a		;4383
	jp SIGUIENTE_PASO		;4386
META_3_LLEGA:		; Paso 3: llega, se monta el decorado de la base y suena la musica
	ld c,000h		;4389   ; C = 0: el andar de vuelta no lleva prisa
	ld a,(0e079h)		;438b
	ld h,a			;438e
	call ANDA_HASTA_LA_BASE		;438f
	ret nz			;4392
	call MONTA_SPRITES_BASE		;4393   ; los sprites de la base, el dibujo y el montaje, en ese orden
	call DIBUJA_LA_BASE		;4396
	call MONTA_LA_BASE		;4399
	ld a,08fh		;439c   ; el sonido 0x8F es el de la llegada a la base
...
META_4_SALUDA:		; Paso 4: la escena de la base
	ld a,(0e01ah)		;43a6   ; 0xE01A es el "lo que queda de nota" del segundo canal de sonido: la escena avanza justo cuando la nota va a acabar, al compas de la musica
	dec a			;43a9   ; 0xE01A es el 'lo que queda de nota' del segundo canal: la escena avanza al compas de la musica
	ret nz			;43aa
	call SUBE_LA_BANDERA		;43ab
	ld a,(0e0e1h)		;43ae   ; la bandera solo se sube en la fase 2
	or a			;43b1
...
META_5_ESPERA:		; Paso 5: espera a que se acabe la musica
	ld a,(0e012h)		;43d0   ; no se sigue hasta que el reproductor suelta 0xE012: la musica se oye entera
	or a			;43d3
	ret nz			;43d4
	ld a,010h		;43d5
	jr ESPERA_A_Y_PASO		;43d7
META_6_BONUS:		; Paso 6: cada cuatro fotogramas cambia un segundo que sobra por 100 puntos
	ld hl,0e004h		;43d9   ; 0xE004 es la espera que dejo el paso anterior: se descuenta antes de empezar a canjear
	ld a,(hl)			;43dc   ; una espera con su propio contador
	or a			;43dd
	jr z,BONUS_PASO		;43de
	dec (hl)			;43e0   ; y mientras quede, se descuenta
	ret			;43e1
BONUS_PASO:
	ld a,(0e003h)		;43e2   ; uno de cada cuatro fotogramas: el bonus se descuenta despacio para que se vea
	and 003h		;43e5   ; los dos bits bajos: uno de cada cuatro
	ret nz			;43e7
	ld hl,(0e0e3h)		;43e8   ; Cuando el reloj llega a cero se acabo el bonus
	ld a,h			;43eb   ; los dos bytes del tiempo juntos: a cero, se acabo el bonus
	add a,l			;43ec
	jr z,ESPERA_80_Y_PASO		;43ed
	ld c,000h		;43ef
	call RESTA_UN_SEGUNDO		;43f1   ; un segundo menos de tiempo...
	ld de,00100h		;43f4   ; Cien puntos por segundo
	call SUMA_AL_MARCADOR		;43f7   ; ...y sus puntos al marcador
	ld a,001h		;43fa
...
META_7_CORTINILLA:		; Paso 7: cortinilla y vuelta al estado 9 con la fase siguiente
	...
	ret p			;4403   ; el `ret p` vuelve mientras la cortinilla no acabe
	ld a,008h		;4404   ; el estado 8 con 0x50 fotogramas de espera
	ld (0e000h),a		;4406
...
SIGUIENTE_ESTADO:		; Sube 0xE000 y pone el paso a cero
	ld hl,0e000h		;440e   ; el estado sube uno y el paso vuelve a cero
	inc (hl)			;4411
	xor a			;4412
	ld (0e001h),a		;4413
	ret			;4416
ESPERA_80_Y_PASO:		; Espera 80 fotogramas y pasa al paso siguiente
	ld a,050h		;4417   ; 0x50 fotogramas de espera y un paso mas
...
MUERTA_JUGADOR:		; Codigo muerto: escribe el numero de jugador. No la llama nadie
	...
	and 001h		;4428
	add a,031h		;442a   ; el 0x31 es el tile del "1": el numero de jugador sale de sumarle el bit 0 de 0xE002
	ld de,03933h		;442c   ; 0x3933 es la casilla donde va ese digito
...
REINICIA_PARTIDA:		; Deja a cero el marcador y todas las variables de partida
	...
	ld de,0e044h		;4466   ; 0x100 bytes desde 0xE044 a cero: las fichas de los objetos
	ld bc,00100h		;4469
	ld (hl),000h		;446c
	ldir		;446e
	ld hl,04491h		;4470   ; Los nueve valores iniciales de 0xE0E0 (fase, tiempo, distancia...)
	ld de,0e0e0h		;4473   ; y nueve bytes mas desde 0xE0E0: fase, mapa y tiempo
	ld bc,00009h		;4476
	ldir		;4479
	ld de,00900h		;447b   ; Colores del banco 0 de la VRAM
	ld bc,00100h		;447e
	ld a,0f0h		;4481   ; el tile 0xF0 rellena la pantalla: el fondo de la fase
	call RELLENA_VRAM		;4483
	ld b,00ah		;4486   ; Los diez huecos de tiempo sobrante, uno por fase, a 5
	ld hl,0e0d5h		;4488   ; los diez huecos de tiempo sobrante vuelven a cinco segundos cada uno
REINICIA_HUECOS:
	ld (hl),005h		;448b
	inc hl			;448d
...
; ----------------------------------------------------------------------
; DATOS valores_iniciales: Los nueve bytes que 0x4470 copia a 0xE0E0: fase 1,
;   indice 0, y el resto a cero salvo 0xE0E4=2 y 0xE0E6=0x17. Solo los cinco
;   primeros se usan: 0x425B machaca la distancia y el tiempo en cuanto
;   empieza la fase
...
BORRA_NOMBRES:		; Las 768 casillas de la tabla de nombres
	ld de,03800h		;44b5   ; 0x3800 son las 24 filas de la tabla de nombres, 0x300 bytes
	ld bc,00300h		;44b8
	jr BORRA_VRAM		;44bb
...
PONE_REGISTROS_VDP:		; Copia los ocho registros a 0xE038 y los manda al VDP con WRTVDP
	ld hl,044dfh		;44c3
	ld de,0e038h		;44c6   ; 0xE038 guarda la copia en RAM de los ocho registros, que el VDP no deja leer
	ld bc,00008h		;44c9
	ldir		;44cc
	ld hl,0e038h		;44ce
	ld d,008h		;44d1
...
; ----------------------------------------------------------------------
; DATOS registros_vdp: Los ocho registros del VDP: 02 E2 0E 7F 07 76 03 E4.
;   Colores en 0x0000 y patrones en 0x2000, al reves de lo corriente; nombres
;   en 0x3800, patrones de sprite en 0x1800 y atributos de sprite en 0x3B00.
;   Sprites de 16x16 sin ampliar, y SCREEN 2
...
COPIA_A_VRAM_BUCLE:
	...
	exx			;44f4
	inc hl			;44f5
	dec bc			;44f6   ; El `dec bc` no toca banderas: el `ld a,b / or c` es el "BC == 0" de un contador de 16 bits
	ld a,b			;44f7
	or c			;44f8
...
RELLENA_VRAM:		; Escribe el byte A en BC posiciones de la VRAM desde DE
	...
	res 6,d		;4504
RELLENA_VRAM_BUCLE:
	ld a,h			;4506   ; El byte se relee de H en cada vuelta, porque el `or c` del final gasta A
	exx			;4507
	out (c),a		;4508
	exx			;450a
	dec bc			;450b
...
PINTA_FRANJAS:		; Rellena tiras de la tabla de nombres a partir de una lista (largo, posicion), con el byte que va delante
	ld a,(hl)			;4512   ; el primer byte de la lista es el que se va a escribir; detras vienen las parejas de largo y posicion
	inc hl			;4513
	ld (0e0dfh),a		;4514   ; El byte con el que se rellena, que va el primero de la lista
	ld d,039h		;4517   ; 0x3900 es la tabla de nombres mas 0x100, o sea la fila 8: por ahi empiezan las franjas
FRANJAS_BUCLE:
	ld c,(hl)			;4519
	inc hl			;451a
	xor a			;451b   ; Un largo 0 cierra la lista
	cp c			;451c
	ret z			;451d
	ld b,a			;451e   ; B a cero: el largo cabe en un byte y el otro no se usa
	ld e,(hl)			;451f
	inc hl			;4520
	ld a,e			;4521
	cp 020h		;4522   ; Posicion menor de 0x20: la fila de mas abajo
	jr nc,FRANJAS_PINTA		;4524
	inc d			;4526   ; el `inc d` suma 0x100, que en la tabla de nombres son ocho filas
FRANJAS_PINTA:
	ld a,(0e0dfh)		;4527
	push hl			;452a   ; La lista y el destino se salvan alrededor del relleno, que gasta los seis registros
	push de			;452b
...
DIBUJA_BLOQUE:		; Interprete de los bloques de decorado y de pista
	...
	or a			;4534
	ret z			;4535
	and 0f0h		;4536   ; El nibble alto del primer byte es la columna
	ld c,a			;4538
	ld a,(hl)			;4539
	inc hl			;453a
	and 003h		;453b   ; El nibble bajo (0-3) mas 0x78: el byte alto de la casilla en la tabla de nombres, con el bit de escritura ya puesto
	add a,078h		;453d
	ld d,a			;453f   ; y con eso D queda hecho: el byte alto de la casilla, con el bit de escritura ya dentro
	ld a,c			;4540
BLOQUE_FILA:
	ld b,(hl)			;4541
	inc hl			;4542
	ld a,020h		;4543   ; Una fila mas abajo son 32 casillas: se suman al byte bajo y el acarreo sube el alto
	add a,c			;4545
	ld c,a			;4546
...
BLOQUE_APUNTA:
	ld a,c			;454a
	add a,b			;454b
	sub 0e0h		;454c   ; El byte que cerro la fila hace de desplazamiento de la siguiente: el `sub 0xE0` le quita la marca
	ld e,a			;454e
	call APUNTA_VRAM		;454f
BLOQUE_CASILLA:
	ld a,(hl)			;4552   ; El 0x00 cierra el bloque
	or a			;4553
	ret z			;4554   ; Y un byte de 0xE0 para arriba cierra la fila y abre la siguiente
	cp 0e0h		;4555
	jr nc,BLOQUE_FILA		;4557
	inc hl			;4559   ; Las casillas de la fila salen seguidas por el puerto de datos
	exx			;455a
	out (c),a		;455b   ; El puerto de datos sigue en C', apuntado por APUNTA_VRAM: las casillas de la fila salen seguidas
	exx			;455d
...
DESCOMPRIME_TIRADA_BUCLE:
	...
	out (c),a		;458f
	exx			;4591
	djnz DESCOMPRIME_TIRADA_BUCLE		;4592   ; Este `djnz` vuelve ANTES de leer: un byte nuevo por vuelta, al reves que el bucle de repetir
...
LEE_BYTE_QUIZA_ESPEJADO:		; Lee (HL) y, con el bit 0 de C, le da la vuelta a los bits: espejo horizontal
	ld a,(hl)			;4598
	inc hl			;4599
	bit 0,c		;459a   ; El bit 0 de C lo fijo la entrada del descompresor: sin el, el byte sale tal cual
	ret z			;459c
	push bc			;459d
	ld b,008h		;459e
	ld c,a			;45a0
...
ESCRIBE_CADENA:		; Escribe la cadena de (HL); el destino va en los dos primeros bytes
	ld e,(hl)			;45a8   ; cada cadena trae delante su destino de VRAM, dos bytes
	inc hl			;45a9
	ld d,(hl)			;45aa
	inc hl			;45ab
ESCRIBE_CADENA_EN_DE:		; Igual, pero el destino ya viene en DE
	ld a,(hl)			;45ac
	inc hl			;45ad
	ld b,a			;45ae   ; el byte se mira con dos `inc b` seguidos: el primero caza el 0xFF y el segundo el 0xFE
	inc b			;45af
...
REPITE_4_BYTES:		; Copia C veces los mismos cuatro bytes de (HL) a (DE): un atributo de sprite repetido
	push hl			;45ba   ; cuatro bytes por vuelta, y HL vuelve atras: el mismo grupo repetido C veces
	ld b,004h		;45bb
REPITE_4_BUCLE:
	ld a,(hl)			;45bd
	ld (de),a			;45be
	...
	pop hl			;45c6   ; HL vuelve al principio de los cuatro bytes; DE sigue avanzando y la copia se repite
...
CORTINILLA:		; Borra la pantalla por columnas; devuelve M al terminar
	...
	ld d,038h		;45ce   ; 0x3800 es la tabla de nombres; la cortinilla se pinta ahi
	ld hl,0e004h		;45d0
	ld b,018h		;45d3   ; 0x18 filas: la pantalla entera
	bit 6,(hl)		;45d5   ; Bit 6 de 0xE004: un lado o el otro
	jr nz,CORTINILLA_DERECHA		;45d7
	ld a,01fh		;45d9   ; La columna izquierda es 0x1F menos la cuenta: las dos avanzan a la vez hacia el centro
	sub (hl)			;45db
	ld e,a			;45dc
	set 6,(hl)		;45dd   ; el bit 6 se enciende y se apaga en vueltas alternas: una columna por lado y por fotograma
	jr CORTINILLA_COLUMNA		;45df
CORTINILLA_DERECHA:
	res 6,(hl)		;45e1
	dec (hl)			;45e3   ; y solo se descuenta al cerrar la pareja; con el signo negativo, se acabo
	ret m			;45e4
	ld e,(hl)			;45e5
CORTINILLA_COLUMNA:
	...
	add a,e			;45ef
	ld e,a			;45f0
	dec b			;45f1   ; los dos `dec b` se comen las dos filas del panel
	dec b			;45f2
CORTINILLA_BUCLE:
	xor a			;45f3
	call ESCRIBE_BYTE_VRAM		;45f4
	ld a,020h		;45f7   ; 0x20 casillas para bajar una fila
...
BORRA_SPRITES:		; Deja a cero los 128 bytes de atributos de sprite (VRAM 0x3B00), pasando por 0xE050
	ld hl,0e050h		;4600   ; los sprites se borran en la copia de RAM y luego se vuelca entera
	push hl			;4603
	ld b,080h		;4604   ; 0x80 bytes son los 32 atributos de sprite
BORRA_SPRITES_BUCLE:
	ld (hl),000h		;4606
	inc hl			;4608
	djnz BORRA_SPRITES_BUCLE		;4609
	ld de,03b00h		;460b   ; Y la copia limpia entera a la tabla de atributos: 0xE050 es su espejo en RAM
	pop hl			;460e
	ld bc,00080h		;460f
...
LEE_GATILLOS_NUEVOS:		; Deja en A los gatillos que se acaban de pulsar en este fotograma
	ld a,(0e009h)		;461d   ; 0xE009 es lo pulsado ahora y 0xE008 lo de la vuelta anterior
	ld b,a			;4620
	ld a,(0e008h)		;4621
	and 030h		;4624
	cpl			;4626   ; Lo de antes negado y en AND con lo de ahora: el flanco de los bits 4-5, los gatillos recien pulsados
	ld c,a			;4627
	ld a,b			;4628
	and 030h		;4629
	and c			;462b   ; y el `and` de los dos deja solo lo que acaba de pulsarse
	ret			;462c
SUMA_AL_MARCADOR:		; Suma DE al marcador, en BCD, y actualiza el record
	...
	add a,a			;4630
	ret p			;4631
	ld hl,0e043h		;4632   ; el marcador son tres parejas BCD en 0xE043, y se suma pareja a pareja
	ld a,(hl)			;4635
	add a,e			;4636
	daa			;4637
	ld (hl),a			;4638
	ld e,a			;4639   ; Las dos parejas bajas vuelven a DE: COMPARA_RECORD las compara de golpe con el `sbc hl,de`
	inc hl			;463a
	ld a,(hl)			;463b
	adc a,d			;463c   ; la pareja de en medio va con `adc`: el acarreo viene de la de abajo
	daa			;463d
	ld (hl),a			;463e
	ld d,a			;463f
	inc hl			;4640
	...
	ld a,(hl)			;4643
	adc a,000h		;4644   ; y la de arriba igual, con su `daa`
	daa			;4646
	ld (hl),a			;4647
	jr nc,COMPARA_RECORD		;4648
	ld bc,09999h		;464a   ; Al pasarse de 999999 el record se clava en ese tope
	ld (0e040h),bc		;464d   ; los seis nueves se escriben dos veces, en 0xE040 y 0xE041, que es como se cubre la pareja de en medio
	ld (0e041h),bc		;4651
	jr PINTA_RECORD		;4655
COMPARA_RECORD:
	ld a,(0e042h)		;4657   ; el record se compara de arriba abajo, y solo se mira lo de abajo si lo de arriba empata
	ld b,(hl)			;465a
	sub (hl)			;465b   ; Byte alto contra byte alto: con acarreo hay record nuevo; empatados, deciden las parejas bajas
...
NUEVO_RECORD:
	ld (0e040h),de		;4667   ; el record nuevo son las dos parejas bajas y la alta, en ese orden
	ld a,b			;466b
	ld (0e042h),a		;466c
	jr PINTA_RECORD		;466f
CUENTA_EL_TIEMPO:		; Descuenta un segundo cada 64 fotogramas mientras 0xE133 diga que el reloj corre
	ld a,(0e133h)		;4671   ; 0xE133 apagado es el reloj parado: en el menu y en las cortinillas no corre
	or a			;4674
	ret z			;4675
	...
	ret			;4681
TIEMPO_CADA_64:
	ld a,(0e003h)		;4682   ; uno de cada 64 fotogramas, y 0xE003 sube uno por interrupcion: el "segundo" del juego dura 1,28 s en una maquina PAL y 1,07 en una NTSC, o sea que en las dos es mas largo que el de verdad
	and 03fh		;4685
	ret nz			;4687
	ld c,001h		;4688
RESTA_UN_SEGUNDO:		; Baja el reloj en uno, en BCD, y avisa con un pitido cuando quedan menos de once
	ld hl,0e0e3h		;468a   ; el reloj tambien va en BCD, dos parejas
	ld a,(hl)			;468d
	sub 001h		;468e   ; `sub 1 / daa` en vez de `dec`: el `dec` no deja el acarreo que la pareja alta BCD necesita
	daa			;4690
	ld (hl),a			;4691
	inc hl			;4692
	ld a,(hl)			;4693   ; La pareja alta solo baja si la baja pidio prestamo
...
TIEMPO_MIRA_AVISO:
	...
	push af			;46a6   ; el aviso suena y la vuelta sigue: el `push` guarda las banderas que hacen falta despues
	push hl			;46a7
	ld a,009h		;46a8
...
PINTA_TIEMPO:		; Las cuatro cifras del reloj
	ld b,002h		;46af   ; dos parejas BCD: los cuatro digitos del reloj
	ld de,03827h		;46b1
	ld hl,0e0e4h		;46b4
	jp PINTA_BCD		;46b7
PINTA_PANEL:		; Pinta el panel entero: rotulos, tiempo, distancia, fase, record y marcador
	ld hl,057b0h		;46ba   ; el panel se pinta entero de una vez: rotulos, reloj, distancia, fase, record y marcador
...
PINTA_RECORD:		; Las seis cifras del record
	ld hl,0e042h		;46c9   ; el record va en 0x380F, tres bytes
	ld de,0380fh		;46cc
	call PINTA_TRES_BYTES		;46cf
PINTA_MARCADOR:		; Las seis cifras del marcador
	ld de,03805h		;46d2   ; y el marcador en 0x3805
	ld hl,0e045h		;46d5
PINTA_TRES_BYTES:
	ld b,003h		;46d8
	jr PINTA_BCD		;46da
AVANZA_DISTANCIA:		; Descuenta la distancia que queda al ritmo que marca la velocidad
	...
	ret nz			;46e0
	ld a,(0e100h)		;46e1
	srl a		;46e4   ; la mitad del periodo: la distancia baja al doble de ritmo que la animacion
	dec a			;46e6
	ld (hl),a			;46e7
	ld hl,0e0e6h		;46e8   ; Distancia a cero: 0xE00D avisa de que se ha llegado a la meta
	ld a,(hl)			;46eb   ; los dos bytes de la distancia mirados de una vez con el `or`
	dec hl			;46ec
	or (hl)			;46ed
	...
	ret			;46f4
DISTANCIA_RESTA:
	ld a,(hl)			;46f5   ; un metro por paso, en BCD
	sub 001h		;46f6   ; Un descuento por paso, en BCD, con el prestamo subiendo al byte alto
	daa			;46f8
	ld (hl),a			;46f9
	ld c,a			;46fa   ; C se queda el byte bajo, que DISTANCIA_MIRA consulta
	inc hl			;46fb
...
DISTANCIA_MIRA:
	...
	or (hl)			;4707   ; con el byte bajo a cero y el alto distinto, se acaba de cruzar una centena
	jr z,DISTANCIA_PINTA		;4708
	ld a,(hl)			;470a
	and 003h		;470b   ; y de esas, una de cada cuatro: cada 400 metros
...
PINTA_DISTANCIA:		; Las cuatro cifras de la distancia
	ld b,002h		;4716   ; dos parejas: los cuatro digitos de la distancia
	ld de,0382fh		;4718
	ld hl,0e0e6h		;471b
	jr PINTA_BCD		;471e
PINTA_FASE:		; Las dos cifras del numero de fase
	ld de,0381ch		;4720   ; la fase es un solo byte, dos digitos
	ld hl,0e0e0h		;4723
	ld b,001h		;4726
PINTA_BCD:		; Escribe B bytes BCD de (HL) hacia abajo en la VRAM (DE) hacia arriba, dos cifras por byte
	...
	ld c,a			;472e   ; el nibble bajo se guarda en C mientras se saca el alto
	pop af			;472f   ; El mismo byte otra vez: ahora el nibble alto, bajado con las cuatro rotaciones
	and 0f0h		;4730
	rra			;4732
	rra			;4733
	rra			;4734
	...
	inc de			;473b   ; el alto se escribe primero y el bajo detras, que en pantalla van en ese orden
	ld a,c			;473c
	call ESCRIBE_BYTE_VRAM		;473d
	dec hl			;4740   ; Los bytes BCD se recorren hacia abajo y la pantalla hacia arriba: van al reves
	inc de			;4741
	djnz PINTA_BCD		;4742
	ret			;4744
...
ELIGE_DECORADO:		; Segun la fase y la distancia, deja en 0xE18A y 0xE18B lo que toca dibujar a los lados de la pista
	ld a,(0e0e0h)		;4745   ; la fase, en su nibble bajo, elige la pareja de decorados
	and 00fh		;4748
	ld hl,04787h		;474a
	add a,a			;474d   ; por dos: la tabla son parejas
	call SUMA_A_HL		;474e
	ld a,(0e0e6h)		;4751
...
DECORADO_SEGUNDA:
	ld a,(hl)			;4759
	ld (0e18ah),a		;475a   ; 0xE18A se queda el byte del decorado elegido por la fase y el bit 4 de la distancia
	ld a,(0e0e0h)		;475d
	and 00fh		;4760
	ld hl,047c3h		;4762
	add a,a			;4765
	call SUMA_A_HL		;4766
	ld e,(hl)			;4769   ; El puntero de la fase abre una VENTANA dentro de la tabla de veinte punteros; la distancia elige dentro de ella
	inc hl			;476a
	ld d,(hl)			;476b
	ex de,hl			;476c
	ld a,(0e0e6h)		;476d   ; La distancia, por cuartos
	and 0fch		;4770   ; los dos bits de abajo fuera y dos rotaciones: la distancia partida en cuartos de centena
	rrca			;4772
	rrca			;4773
	res 3,a		;4774
	cp 004h		;4776   ; por debajo de cuatro se coge la entrada tal cual
...
DECORADO_INDICE:
	...
	ld d,(hl)			;4781
	ex de,hl			;4782
	ld (0e18bh),hl		;4783   ; 0xE18B queda apuntando a la lista de ocho bytes que toca
	ret			;4786

; ----------------------------------------------------------------------
; DATOS decorado_por_fase: Dos bytes por fase, diez fases: 0x474A los indexa y
...
;   veinte punteros de 0x47D7, y acaban clavadas donde empieza la tabla de
;   fases
;   0x479b..0x47c3  (40 bytes)
DATA_listas_de_decorado:
	defb 001h,005h,0ffh,000h,012h,005h,0ffh,000h	; 479b  ........
	defb 011h,001h,000h,012h,000h,001h,012h,000h	; 47a3  ........
	defb 000h,0ffh,003h,011h,001h,005h,0ffh,003h	; 47ab  ........
	defb 000h,0ffh,003h,003h,000h,011h,001h,012h	; 47b3  ........
	defb 005h,0ffh,005h,0ffh,003h,012h,005h,0ffh	; 47bb  ........

; ----------------------------------------------------------------------
; DATOS decorado_puntero_por_fase: Diez punteros, uno por fase, que apuntan
;   DENTRO de la tabla de al lado con ventanas que se solapan. 0x4762 lo
;   indexa
;   0x47c3..0x47d7  (20 bytes)
DATA_decorado_puntero_por_fase:
	defw 047f7h,047e5h,047edh,047f7h,047efh,047f1h,047f9h,047e5h,047f1h,047d7h	; 47c3

; ----------------------------------------------------------------------
; DATOS decorado_punteros: Veinte punteros a las cinco listas. Cierra clavado
;   en 0x47FF, donde vuelve a haber codigo
;   0x47d7..0x47ff  (40 bytes)
DATA_decorado_punteros:
	defw 047b3h,0479bh,047b3h,0479bh,047bbh,047abh,0479bh,047abh,047a3h,047b3h	; 47d7
	defw 047a3h,0479bh,047b3h,047a3h,047abh,047a3h,047bbh,047a3h,047bbh,047a3h	; 47eb
...
HAY_SORPRESA:		; Mientras 0xE18E este encendido devuelve C=3, y descuenta 0xE18F hasta apagarlo
	ld a,(0e18eh)		;47ff   ; la sorpresa es la que aparece de tarde en tarde y no en cada tramo
	rra			;4802   ; El bit 0 de 0xE18E al acarreo: apagado, no hay sorpresa
	ret nc			;4803
	ld hl,0e18fh		;4804
	dec (hl)			;4807   ; 0xE18F es la vida que le queda; al agotarse, la sorpresa se apaga sola
	jr nz,SORPRESA_SI		;4808
	xor a			;480a
	ld (0e18eh),a		;480b
SORPRESA_SI:
	ld c,003h		;480e   ; C = 3 al apagarse: es lo que el llamador lee para saber que ya no esta
	ret			;4810
MIRA_SORPRESA:		; Enciende 0xE18E en ciertos multiplos de 100 metros, con la duracion que da la tabla de al lado
	ld a,(0e0e0h)		;4811   ; la tabla de 0x4839 da la vida de la sorpresa, una por fase
	and 00fh		;4814
	ld hl,04839h		;4816
	call SUMA_A_HL		;4819
	ld de,(0e0e5h)		;481c
	...
	ret nz			;4826
	ld a,(0e0e0h)		;4827
	add a,d			;482a   ; y la fase corre el reparto, asi que no salen siempre en los mismos sitios
	and 003h		;482b   ; Una de cada cuatro centenas, corrida segun la fase
	cp 002h		;482d
	ret nz			;482f
	inc a			;4830
	ld (0e18eh),a		;4831   ; encendida, con su vida cargada
	ld a,(hl)			;4834
	ld (0e18fh),a		;4835
	ret			;4838
...
;   enciende 0x4811. La fase 1 lleva 7 y las demas entre 2 y 6
...
PREPARA_ROTULO:		; Limpia la pantalla y pone en blanco los colores del tercio de abajo para el rotulo
	...
	ld bc,00180h		;4849
	ld a,070h		;484c   ; el 0x70 en los colores del banco 2 antes de escribir nada
...
DIBUJA_ROTULO:		; Dibuja una columna del rotulo por llamada, 23 columnas de dos casillas; devuelve C mientras queda
	ld hl,0e00ah		;4864   ; 0xE00A cuenta por que columna va el rotulo
	ld a,(hl)			;4867
	inc (hl)			;4868
	cp 017h		;4869   ; 23 columnas
	jr nc,ROTULO_ESPERA		;486b
	ld de,03885h		;486d   ; 0x3885 es la casilla por la que empieza
	ld c,a			;4870
	add a,e			;4871
	ld e,a			;4872
	ld a,c			;4873
	...
	ld c,a			;4877
	ld b,003h		;4878   ; tres escrituras por columna, una fila cada una
	xor a			;487a
ROTULO_COLUMNA:
	call ESCRIBE_BYTE_VRAM		;487b   ; La primera pasada escribe un 0 -la casilla vacia de encima- y las otras dos, las dos casillas del rotulo, una fila cada una
	ld a,020h		;487e   ; 0x20 casillas para bajar de fila
	call SUMA_A_DE		;4880
	ld a,c			;4883
	inc c			;4884
	djnz ROTULO_COLUMNA		;4885
	scf			;4887   ; El acarreo dice que quedan columnas
	ret			;4888
ROTULO_ESPERA:
	push af			;4889   ; y con las 23 columnas puestas, el copyright debajo
...
SUBE_LOGO:		; Dibuja el logotipo -tres filas de 3, 11 y 12 casillas- una fila mas arriba cada vez, y borra el rastro que deja debajo
	ld hl,(0e00eh)		;4896   ; 0xE00E lleva la altura a la que va el logotipo
	ld de,00020h		;4899
	...
	sbc hl,de		;48a5
	ex de,hl			;48a7
	ld a,044h		;48a8   ; Las 26 casillas van seguidas desde la 0x44; las filas las corta el 0x0E - C de abajo: 3, 11 y 12 de ancho
	ld bc,00303h		;48aa   ; tres filas y tres columnas para empezar
LOGO_FILA:
	push de			;48ad
LOGO_CASILLA:
	call ESCRIBE_BYTE_VRAM		;48ae
	inc de			;48b1   ; A no se reinicia entre filas: las casillas van seguidas por todo el logotipo
	inc a			;48b2
	...
	ex de,hl			;48ba
	ld h,a			;48bb
	ld a,00eh		;48bc   ; B se recarga con 0x0E menos C: la fila de arriba lleva 3 casillas y las dos de abajo 11 y 12
	sub c			;48be
	ld b,a			;48bf
	ld a,h			;48c0
	dec c			;48c1
...
ESCRIBE_BYTE_VRAM:		; Escribe A en la VRAM (DE)
	...
	exx			;48d3   ; El puerto va en C': APUNTA_VRAM lo dejo alli
	out (c),a		;48d4
	exx			;48d6
	ei			;48d7
	ret			;48d8
MUERTA_LEE_BYTE_VRAM:		; Codigo muerto: lee un byte de la VRAM (DE). Gemela de la de escribir, que se usa diez veces
	...
	exx			;48dc
	in a,(c)		;48dd   ; La gemela muerta lee por el `in` del puerto que dejo en C' la otra rutina muerta
	exx			;48df
	ei			;48e0
	ret			;48e1
...
MUERTA_APUNTA_VRAM_LEER:		; Codigo muerto: apunta la VRAM para LEER. Solo la llama la otra rutina muerta
	...
	ret			;48fd
SUMA_A_HL:		; HL = HL + A
	add a,l			;48fe   ; suma A a HL de 16 bits, que el Z80 no tiene `add hl,a`
	ld l,a			;48ff
	ret nc			;4900
	inc h			;4901
	ret			;4902
SUMA_A_DE:		; DE = DE + A
	add a,e			;4903   ; y la misma para DE
	ld e,a			;4904
	ret nc			;4905
	inc d			;4906
	ret			;4907
ESTADO_15_MAPA:		; Estado 15: el mapa, en siete pasos
	ld a,(0e001h)		;4908   ; el mapa tambien va por pasos, con su tabla
	call DESPACHA		;490b
...
DATA_tabla_mapa:
	defw 0491ch	; 490e  -> MAPA_0_PREPARA
	defw 04936h	; 4910  -> MAPA_1_BORDE
	defw 0493dh	; 4912  -> MAPA_2_FILA
	defw 04974h	; 4914  -> MAPA_3_BORDE
	defw 04990h	; 4916  -> MAPA_4_TRAZO
	defw 0499dh	; 4918  -> MAPA_5_TRAZA
	defw 049f4h	; 491a  -> MAPA_6_ESPERA
...
MAPA_0_PREPARA:		; Paso 0: prepara los punteros y pinta de blanco los colores del mapa
	...
	ld (0e0f0h),hl		;4925
	ld de,01080h		;4928   ; los colores del banco 2 a 0xF4 antes de dibujar el mapa
	ld bc,00180h		;492b
	ld a,0f4h		;492e
	call RELLENA_VRAM		;4930
	jp SIGUIENTE_PASO		;4933
MAPA_1_BORDE:		; Paso 1: la linea de arriba del marco
	ld de,03883h		;4936   ; y el borde de arriba con el tile 0x92
	ld a,092h		;4939
	jr MAPA_BORDE		;493b
MAPA_2_FILA:		; Paso 2: una fila del mapa cada dos fotogramas, hasta el 0x00 que cierra los datos
	ld a,(0e003h)		;493d   ; una fila cada dos fotogramas: el mapa se dibuja a la vista
	rra			;4940
	ret c			;4941
	ld hl,(0e0f0h)		;4942
	...
	pop de			;4957
	inc de			;4958   ; y por dentro el 4, 0x16 casillas: el marco deja una a cada lado
	ld a,004h		;4959
	ld c,016h		;495b
...
MAPA_3_BORDE:		; Paso 3: la linea de abajo del marco
	ld de,03aa3h		;4974   ; el borde de abajo: esquina, tramo de 24 y la otra esquina
	ld a,091h		;4977
MAPA_BORDE:
	call ESCRIBE_BYTE_VRAM		;4979
	inc de			;497c
	ld bc,00018h		;497d
	add a,004h		;4980   ; Tres casillas: la esquina (A), el tramo (A + 4, 24 veces) y la otra esquina (A + 2)
	push af			;4982
	call RELLENA_VRAM		;4983
	pop af			;4986
	sub 002h		;4987
	exx			;4989
	out (c),a		;498a   ; El puerto sigue apuntado tras el relleno: la esquina derecha sale por el `out` directo
	exx			;498c
...
MAPA_5_TRAZA:		; Paso 5: un paso del recorrido cada dos fotogramas
	ld a,(0e003h)		;499d   ; tambien uno de cada dos fotogramas
	rra			;49a0
	ret c			;49a1
	ld hl,0e0f6h		;49a2
	ld a,(hl)			;49a5
...
TRAZO_PARTE:
	...
	scf			;49de   ; el acarreo dice que quedan pasos por trazar
	ret			;49df
...
PASO_ARRIBA:		; Una fila menos: -0x20
	ld a,0e0h		;49e1   ; arriba es restar 0x20, o sea sumar 0xE0
	jr PASO_ARRIBA_SUMA		;49e3
PASO_DERECHA:		; Una casilla mas
	ld a,001h		;49e5   ; y a la derecha, una casilla
	jr PASO_SUMA		;49e7
...
MAPA_6_ESPERA:		; Paso 6: espera y pone 0xE000 a 9, pero ESPERA_80_Y_ESTADO lo sube: cae en el estado 10, el que monta la fase (el 9 ya corrio antes del mapa)
	...
	dec (hl)			;49f7
	ret nz			;49f8
	ld a,009h		;49f9   ; El 9 que ESPERA_80_Y_ESTADO subira a 10
	ld (0e000h),a		;49fb
...
DATA_mapa_dibujo:
	...
	defb 029h,023h,021h,004h,004h,01ah,02bh,02fh,02eh,021h,02dh,029h,0ffh,0ffh,000h	; 4aa1  )#!...+/.!-)...
...
;   casilla que se dibuja. El 0x20 de 0x4AD8 lo cierra
;   0x4ab0..0x4ad9  (41 bytes)
DATA_mapa_recorrido:
	defb 0c4h,0c4h,0c0h,00bh,002h,002h,0c5h,00ch,0c5h,0c5h,0c6h,086h,087h,0c5h,002h,00ch	; 4ab0  ................
	defb 00ah,009h,048h,043h,00ch,00ch,001h,045h,045h,045h,042h,085h,047h,042h,082h,082h	; 4ac0  ..HC...EEEB.GB..
	defb 085h,04bh,082h,082h,08bh,0c4h,082h,08bh,020h	; 4ad0  .K...... 
...
;   en 0x4B01, donde vuelve a haber codigo. Salen 1500 m/100 s, 1700/120,
;   1100/80, 1200/80, 1200/80, 500/40, 2600/165, 1200/90, 1500/100 y 1200/90
;   0x4ad9..0x4b01  (40 bytes)
DATA_tabla_de_fases:
	defb 015h,000h,000h,001h	; 4ad9
	defb 017h,003h,020h,001h	; 4add
	defb 011h,008h,080h,000h	; 4ae1
	defb 012h,00ch,080h,000h	; 4ae5
	defb 012h,010h,080h,000h	; 4ae9
	defb 005h,015h,040h,000h	; 4aed
	defb 026h,016h,065h,001h	; 4af1
	defb 012h,01dh,090h,000h	; 4af5
	defb 015h,022h,000h,001h	; 4af9
	defb 012h,025h,090h,000h	; 4afd
...
MONTA_LA_FASE:		; Prepara la fase entera: borra las variables de pista, carga los graficos y monta el decorado
	...
	ld de,0e0f1h		;4b04
	ld bc,00130h		;4b07
	ld (hl),000h		;4b0a   ; el `ld (hl),0` mas el `ldir` es el borrado clasico: se siembra el primero y se arrastra
	ldir		;4b0c
	ld a,010h		;4b0e
	ld h,a			;4b10
	ld l,a			;4b11
	ld (0e100h),hl		;4b12   ; Periodo inicial 0x10, cerca del tope lento: cada fase arranca despacio
	ld (0e110h),a		;4b15   ; 0xE110 es el otro periodo, el del decorado
	ld a,008h		;4b18   ; 0xE149 arranca en 8
	ld (0e149h),a		;4b1a
	ld a,005h		;4b1d
	ld (0e0e9h),a		;4b1f   ; y 0xE0E9 en 5, que es lo que tarda el primer descuento de distancia
	ld hl,03030h		;4b22   ; Segun sea la fase par o impar, una pareja de casillas u otra
	ld a,(0e0e0h)		;4b25   ; el bit 0 de la fase: par o impar
	rra			;4b28
...
FASE_CASILLAS:
	...
	xor a			;4b45
	ld (0e13bh),a		;4b46   ; montada la fase, se levanta el cerrojo
	ret			;4b49
...
PASO_DE_PARTIDA:		; Todo lo que pasa en un fotograma de juego
	call PINTA_VELOCIMETRO		;4b4a   ; el paso de partida empieza por lo que se ve: el velocimetro y el pez
	...
	call AJUSTA_DIFICULTAD		;4b5e   ; la dificultad se ajusta antes de leer el mando
	call MUEVE_PINGUINO		;4b61   ; Los mandos
	call ARRASTRA_PINGUINO		;4b64
	call MIRA_EL_PEZ		;4b67   ; El pez
	call MIRA_EL_BORDE		;4b6a   ; y el borde se mira despues de moverse, no antes
	ld a,(0e140h)		;4b6d   ; si se ha caido al agua, la pista ya no avanza en este paso
	or a			;4b70
	ret nz			;4b71
	...
	call CREA_OBSTACULO		;4b7e   ; y por ultimo los obstaculos nuevos y las nubes
	jp LAS_NUBES		;4b81

; ----------------------------------------------------------------------
; DATOS poses_del_pinguino: Diez poses de cuatro bytes: los cuatro patrones de
;   sprite que forman el pinguino en cada postura. 0x4BD5 las reparte de
;   cuatro en cuatro por los atributos
;   0x4b84..0x4bac  (40 bytes)
DATA_poses_del_pinguino:
	defb 000h,004h,008h,00ch	; 4b84
	defb 010h,014h,018h,01ch	; 4b88
	defb 020h,024h,028h,02ch	; 4b8c
	defb 000h,004h,030h,034h	; 4b90
	defb 038h,03ch,040h,044h	; 4b94
	defb 060h,064h,068h,06ch	; 4b98
	defb 020h,048h,04ch,050h	; 4b9c
	defb 054h,014h,058h,05ch	; 4ba0
	defb 010h,0a8h,018h,0ach	; 4ba4
	defb 0b0h,024h,0b4h,02ch	; 4ba8
...
MUEVE_PINGUINO:		; Lee los mandos y mueve al pinguino, o sigue el salto si ya estaba saltando
	...
	ld a,b			;4bba   ; B trae el mando: en el salto solo importan los bits de los lados
...
PINGUINO_A_VRAM:		; Vuelca los cuatro atributos del pinguino a la VRAM
	...
	ld bc,00010h		;4bcc   ; 0x10 bytes: cuatro atributos de cuatro
...
PONE_POSE:		; Copia los cuatro patrones de la pose A a los atributos, saltando de cuatro en cuatro
	exx			;4bd5   ; la pose se escribe en el juego alterno para no gastar los registros del llamador
	ld hl,04b84h		;4bd6   ; la tabla de 0x4B84 son los cuatro patrones de cada pose
	call SUMA_A_HL		;4bd9
	ld de,0e07ah		;4bdc
	ld b,004h		;4bdf
POSE_BUCLE:
	ld a,(hl)			;4be1
	ld (de),a			;4be2
	ld a,004h		;4be3   ; De cuatro en cuatro: el campo de patron de cada uno de los cuatro atributos
	add a,e			;4be5
	ld e,a			;4be6
	inc hl			;4be7
...
COLOCA_SPRITES:		; Reparte HL (Y en L, X en H) por los cuatro sprites, sumando 16 a la derecha y abajo
	ld d,h			;4bec   ; El pinguino son cuatro sprites de 16x16 en cuadro: 0xE078, 0xE07C, 0xE080 y 0xE084
	ld (0e078h),hl		;4bed
	ld a,h			;4bf0
	add a,010h		;4bf1   ; la pareja de la derecha, 16 pixeles mas alla
	ld h,a			;4bf3
	ld (0e07ch),hl		;4bf4
	ld a,l			;4bf7
	add a,010h		;4bf8   ; Con la Y bajada 16, la pareja de abajo
	ld l,a			;4bfa
	ld e,a			;4bfb
	ld (0e080h),de		;4bfc
	ld (0e084h),hl		;4c00
...
EMPIEZA_SALTO:		; Gatillo: suena el salto y se apunta hacia donde va
	...
	ld a,b			;4c09   ; el mando se mira una sola vez, al arrancar el salto: lo que se pulse despues ya no cambia el rumbo
...
SIGUE_SALTO:		; Un paso de salto cada cuatro fotogramas
	ld a,(0e003h)		;4c18   ; el salto avanza uno de cada cuatro fotogramas
	and 003h		;4c1b
	ret nz			;4c1d
SALTO_PASO:
	ld a,(hl)			;4c1e
	inc (hl)			;4c1f   ; el paso sube y el valor leido es el de antes
	cp 00bh		;4c20   ; Once pasos y vuelta a cero
	jr nz,SALTO_POSE		;4c22
	ld (hl),000h		;4c24
SALTO_POSE:
	push af			;4c26
	ld c,000h		;4c27   ; Tres poses: al rematar (paso 11) la de parado (0); durante el salto alternan la 0x10 y la 0x0C con el bit 0 del paso
	cp 00bh		;4c29
...
SALTO_COLOCA:
	...
	call SUMA_A_HL		;4c3c
	ld a,(hl)			;4c3f
	ld de,(0e078h)		;4c40   ; la altura se le suma a la Y del pinguino
	add a,e			;4c44
	ld e,a			;4c45
	ld hl,0e0fbh		;4c46
	ld a,(hl)			;4c49
...
SALTO_TERMINA:
	...
	ld a,(0e0f9h)		;4c54   ; si el salto se acabo en este paso, no se miran choques
	or a			;4c57
	ret nz			;4c58
	call MIRA_CHOQUES		;4c59   ; La sombra
	ld a,(0e140h)		;4c5c   ; ni en el agua ni cayendose se puntua el salto
	ld hl,0e142h		;4c5f
	add a,(hl)			;4c62
	ret nz			;4c63
	ld hl,0e132h		;4c64   ; 0xE132 evita cobrar el mismo salto dos veces
	cp (hl)			;4c67
	ret z			;4c68
	ld (hl),a			;4c69
...
SALTO_A_LA_IZQUIERDA:
	call MUEVE_IZQUIERDA		;4c70   ; el salto lateral mueve DOS casillas, no una: por eso las dos llamadas seguidas
...
MUEVE_IZQUIERDA:		; Una columna a la izquierda; el borde esta en X=0x14
	ld a,d			;4c9c   ; con la columna por debajo de 0x14 no se puede ir mas a la izquierda
	cp 014h		;4c9d
	ret c			;4c9f
	dec d			;4ca0
	set 0,(hl)		;4ca1   ; Los bits 0 y 1 de 0xE0FA apuntan el ultimo sentido: izquierda enciende el 0 y apaga el 1
	res 1,(hl)		;4ca3
	ret			;4ca5
MANTIENE_SENTIDO:		; Con las dos direcciones metidas, la primera vez INVIERTE el sentido que llevaba y lo deja apuntado (bit 7 de 0xE0FA); las siguientes repite el nuevo
	ld a,(hl)			;4ca6   ; 0xE0FA a cero: no habia sentido anterior que mantener
	or a			;4ca7
	ret z			;4ca8
	bit 7,a		;4ca9   ; El bit 7 marca el empate ya resuelto: la primera vez se invierte el sentido (bit 1: venia de la derecha) y las siguientes se repite el nuevo (bit 0)
...
MUEVE_DERECHA:		; Una columna a la derecha; el borde esta en X=0xCC
	ld a,d			;4cb9
	cp 0cch		;4cba   ; y por la derecha el tope es 0xCC
	ret nc			;4cbc
	set 1,(hl)		;4cbd   ; El espejo de la izquierda: enciende el bit 1 y apaga el 0
	res 0,(hl)		;4cbf
	inc d			;4cc1
	ret			;4cc2
ANIMA_ANDAR:		; Las tres poses de andar, una cada ocho fotogramas. La llama la interrupcion, no el paso de partida
	...
	or (hl)			;4cc9
	ret nz			;4cca
	ld a,(0e003h)		;4ccb   ; uno de cada ocho fotogramas: una zancada
	and 007h		;4cce
	ret nz			;4cd0
ANDAR_PASO:
	ld hl,0e0f8h		;4cd1
	inc (hl)			;4cd4   ; 0xE0F8 cuenta las zancadas
	ld a,(hl)			;4cd5
	ld c,000h		;4cd6   ; Los bits 0-1 de la cuenta reparten el ciclo de poses 0, 4, 0, 8: el paso central entre cada zancada
	rra			;4cd8
...
COLOCA_SOMBRA:		; Los dos sprites de debajo del pinguino; en el salto y en la caida las dos mitades se MONTAN una sobre otra y la sombra se estrecha cuanto mas alto va
	ld hl,(0e078h)		;4ce9
	ld a,l			;4cec   ; la sombra va 0x1E pixeles mas abajo y 0x10 a la derecha del pinguino
	add a,01eh		;4ced
	ld l,a			;4cef
	ld c,a			;4cf0
	ld a,h			;4cf1
...
SOMBRA_SEPARA:
	ex de,hl			;4d07   ; la tabla se recorre con el paso de la caida
	call SUMA_A_HL		;4d08
	ld l,(hl)			;4d0b
	ld a,d			;4d0c   ; La separacion se SUMA a la mitad izquierda y se RESTA a la derecha: las medias sombras se montan y la sombra se estrecha
	add a,l			;4d0d
	ld d,a			;4d0e
	ld a,b			;4d0f
	sub l			;4d10
	ld b,a			;4d11
	ld e,0aeh		;4d12   ; La Y de la sombra se clava en 0xAE, el suelo: no acompana al pinguino
	ld c,0aeh		;4d14
	ex de,hl			;4d16
SOMBRA_GUARDA:
	ld (0e0a0h),hl		;4d17   ; las dos mitades de la sombra van a 0xE0A0 y 0xE0A4
	ld (0e0a4h),bc		;4d1a
SOMBRA_A_VRAM:
	ld hl,0e0a0h		;4d1e
	ld de,03b50h		;4d21   ; Sprites 20 y 21
	ld bc,00008h		;4d24   ; ocho bytes: los dos atributos de la sombra
	jp COPIA_A_VRAM		;4d27

; ----------------------------------------------------------------------
; DATOS arco_del_salto: Diez alturas, indexadas de 1 a 10 desde 0x4D29: lo que
;   se montan las dos medias sombras en cada paso del salto (la izquierda
;   avanza eso y la derecha retrocede lo mismo)
...
; ----------------------------------------------------------------------
DATA_arco_del_salto:
	defb 001h,002h,002h,003h,003h,003h,003h,003h,002h,002h	; 4d2a  ..........

; ----------------------------------------------------------------------
; DATOS arco_de_la_caida: Veintiuna alturas, indexadas de 1 a 21 desde 0x4D33
;   con 0xE143, que es el contador de la caida. Cierra clavada en 0x4D49
;   0x4d34..0x4d49  (21 bytes)
DATA_arco_de_la_caida:
	defb 001h,001h,002h,002h,003h,002h,002h,001h,000h,001h,002h,002h,002h,001h,000h,001h	; 4d34  ................
	defb 002h,002h,002h,001h,000h	; 4d44
...
MIRA_CHOQUES:		; Con el pinguino en el suelo: recorre las fichas y mira si alguna esta en el paso 13 a su altura
	ld a,(0e0f9h)		;4d49   ; saltando no se choca con nada: se pasa por encima
	or a			;4d4c
	ret nz			;4d4d
	ld b,004h		;4d4e   ; cuatro fichas de obstaculo, cinco a partir de la fase 5
...
CHOQUES_FICHA:
	...
	ld d,(hl)			;4d69
	ex de,hl			;4d6a
	dec a			;4d6b   ; el tipo se compara con 5: los de mas arriba son objetos que se cogen
	cp c			;4d6c
...
CHOQUES_AGUJERO:
	ld c,(hl)			;4d7a   ; el hueco tiene dos zonas: el agua por dentro y el borde por fuera
	dec c			;4d7b
...
CHOQUES_NADA:
	...
	xor a			;4d8e
CHOQUES_SIGUIENTE:
	inc a			;4d8f   ; seis bytes de una ficha a la siguiente
	call SUMA_A_HL		;4d90
	djnz CHOQUES_FICHA		;4d93
	ret			;4d95
MIRA_CHOQUES_SALTANDO:		; Lo mismo, pero en el aire: solo mira una lista propia, la de 0x4DC8
	ld a,(0e0f9h)		;4d96   ; esta es la gemela de arriba para cuando SI se esta saltando
	or a			;4d99
	ret z			;4d9a
	ld b,005h		;4d9b   ; aqui siempre son cinco fichas
	ld hl,0e112h		;4d9d
SALTANDO_FICHA:
	ld a,(hl)			;4da0
	inc hl			;4da1
	cp 00dh		;4da2   ; Solo las fichas que van por el paso 0x0D
	ld a,005h		;4da4   ; El 5 mas el `inc hl` de arriba: seis bytes por ficha
	jr nz,SALTANDO_SIGUIENTE		;4da6
	ex de,hl			;4da8
	ld a,(de)			;4da9
	cp 005h		;4daa   ; el `cp 5` y el `add a,a` juntos: el tipo por dos, que la tabla son parejas
	add a,a			;4dac
	ld hl,04dc8h		;4dad
	call SUMA_A_HL		;4db0
	ld a,(0e079h)		;4db3   ; La X del pinguino contra el par (posicion, ancho): dentro del ancho, se ha saltado por encima
	sub (hl)			;4db6
	inc hl			;4db7
	cp (hl)			;4db8
	...
	ex de,hl			;4dbb
SALTANDO_SIGUIENTE:
	call SUMA_A_HL		;4dbc   ; y a la ficha siguiente
...
MIRA_EL_PEZ:		; Si el pinguino pisa el pez, suena, se lo lleva y suma 300 puntos
	ld a,(0e142h)		;4dd2   ; ni en el agua ni cayendose se puede coger el pez
	ld hl,0e140h		;4dd5
	add a,(hl)			;4dd8
	ret nz			;4dd9
	ld de,(0e188h)		;4dda   ; 0xE188 es donde esta el pez
	ld a,e			;4dde
	...
	ret nc			;4de9
	ld a,013h		;4dea   ; la caja del pez no es un rectangulo: el ancho depende de la diferencia de altura, asi que es un rombo
	add a,e			;4dec
	ld l,a			;4ded
	ld a,e			;4dee
	add a,a			;4def
	...
	ld hl,0e08ch		;4dfd   ; cogido el pez, se le quita de la pantalla y se avanza su ciclo
	ld de,0e183h		;4e00
...
MIRA_EL_BORDE:		; Choque con lo que haya en 0xE090 cuando esta abajo del todo
	ld hl,(0e090h)		;4e0f   ; la foca solo pilla cuando esta asomada del todo
	ld a,l			;4e12
	cp 08fh		;4e13   ; Solo con la Y de 0xE090 en 0x8F: la foca asomada del todo
	ret nz			;4e15
	ld a,(0e079h)		;4e16
	ld l,a			;4e19
	ld a,h			;4e1a
	sub l			;4e1b
	push af			;4e1c
	sub 018h		;4e1d   ; La pareja `sub`/`add` deja el acarreo con la diferencia de X entre -11 y +23: el ancho del choque
	add a,023h		;4e1f
	jp c,CHOCA		;4e21
	pop af			;4e24
	ret			;4e25
TROPIEZA:		; El pinguino tropieza y rueda hacia el lado por el que iba
	...
	or a			;4e29
	ret nz			;4e2a
	ld a,003h		;4e2b   ; el sonido 3 es el tropiezo
	call PIDE_SONIDO		;4e2d   ; Sonido 3
	ld hl,00101h		;4e30   ; el bit 0 de 0xE0FA -el ultimo sentido- decide hacia donde se cae
	ld a,(0e0fah)		;4e33
	cpl			;4e36
	rra			;4e37
	jr PONE_CAIDA		;4e38
CHOCA:		; Choque de frente: se cae hacia atras
	ld hl,00101h		;4e3a   ; y esta es la caida al agua, con su sonido 8
	ld (0e136h),hl		;4e3d
	ld a,008h		;4e40
	call PIDE_SONIDO		;4e42   ; Sonido 8
	ld hl,00102h		;4e45
	ld a,(0e0f9h)		;4e48   ; cayendo desde un salto la cuenta empieza un paso mas alla
	or a			;4e4b
...
SIGUE_CAIDA:		; Un paso de caida cada cuatro fotogramas: rueda de lado y baja
	ld a,(0e003h)		;4e69   ; la caida avanza uno de cada cuatro fotogramas, como el salto
	and 003h		;4e6c
	ret nz			;4e6e
	ld hl,0e142h		;4e6f
	ld a,(hl)			;4e72
	...
	inc hl			;4e78
	ld a,(hl)			;4e79   ; y su propio contador de paso
	inc (hl)			;4e7a
	ld hl,RET_COMPARTIDO		;4e7b   ; El desplazamiento de este paso, apuntado un byte antes
	call SUMA_A_HL		;4e7e
	ld c,(hl)			;4e81   ; C se queda el desplazamiento de este paso
	ld de,(0e078h)		;4e82
CAIDA_LADO:
	ld hl,0e0d0h		;4e86
	ld a,(0e144h)		;4e89
	...
	ld hl,0e142h		;4e94   ; Mientras 0xE142 pase de 1, otra tanda: el tropiezo entra con 1, el choque con 2 y el choque saltando con 3 tandas de tres columnas
	ld a,(hl)			;4e97
	dec a			;4e98
...
CAIDA_COLOCA:
	ex de,hl			;4e9e
	ld a,l			;4e9f   ; el desplazamiento se le suma a la Y y se recolocan los cuatro sprites
	add a,c			;4ea0
	ld l,a			;4ea1
...
CAIDA_ABAJO:
	...
	call PISTA_GRUPO_B		;4eb1   ; al tocar el suelo se cambia de grupo de pista y se pasa a la siguiente
	call PISTA_SIGUIENTE		;4eb4
	xor a			;4eb7
	ld b,a			;4eb8
	ld hl,0e136h		;4eb9   ; Si 0xE136 (el choque de frente) estaba puesto, se apaga y el repintado corre con 0xE135 levantado
	cp (hl)			;4ebc
	...
	ld (0e135h),a		;4ec1
CAIDA_REPINTA:
	call MUEVE_OBSTACULOS		;4ec4   ; los obstaculos se mueven con el repintado especial levantado
	xor a			;4ec7
	ld (0e135h),a		;4ec8
CAIDA_CUENTA:
	ld hl,0e143h		;4ecb
	...
	ret nz			;4ed1
	ld (hl),a			;4ed2
	dec hl			;4ed3   ; los tres contadores de la caida a cero
	ld (hl),a			;4ed4
	ld (0e137h),a		;4ed5
	ret			;4ed8
TRES_A_LA_DERECHA:		; Tres columnas de golpe
	call MUEVE_DERECHA		;4ed9   ; tres a la derecha de una vez: la caida arrastra tres columnas por tanda
	call MUEVE_DERECHA		;4edc
	jp MUEVE_DERECHA		;4edf
TRES_A_LA_IZQUIERDA:
	call MUEVE_IZQUIERDA		;4ee2   ; y su espejo, tres a la izquierda
...
;   20 desde 0x4EEC con 0xE143. Son tres tramos casi iguales: cada vuelta el
...
CAIDA_TERCER_PASO:		; El tercer paso de la caida, que ya es levantarse
	ld hl,0e0f9h		;4f01   ; levantarse son once pasos, contados con el mismo byte que el salto
	ld a,(hl)			;4f04   ; 0xE0F9 vuelve a contar como en el salto: once pasos de levantarse
	inc (hl)			;4f05
	cp 00bh		;4f06
	jr nz,CAIDA_LEVANTA		;4f08
	ld (hl),000h		;4f0a   ; y a los once vuelve a cero
CAIDA_LEVANTA:
	push af			;4f0c
	ld a,(0e144h)		;4f0d   ; la pose de levantarse sale de 0xE144, el lado por el que se cayo
	ld c,a			;4f10
	call PONE_POSE		;4f11
	pop af			;4f14
	...
	add a,e			;4f20
	ld e,a			;4f21
	bit 2,c		;4f22   ; El bit 2 de la pose (en C) dice el lado: se sigue rodando tres columnas por paso mientras se levanta
	ld hl,0e0d0h		;4f24
	...
	or a			;4f34
	ret nz			;4f35
	ld a,001h		;4f36   ; el remate levanta 0xE135 para que el repintado sepa que es especial
	ld (0e135h),a		;4f38
	call CAIDA_ABAJO		;4f3b   ; El remate reutiliza CAIDA_ABAJO para repintar la fila y los obstaculos donde ha quedado
	xor a			;4f3e
	ld (0e135h),a		;4f3f
	dec hl			;4f42   ; y deja la cuenta de la caida como se la encontro
	inc a			;4f43
	ld (hl),a			;4f44
	ld a,004h		;4f45
...
CAE_AL_AGUA:		; Se cae por el agujero: ocho sprites y a esperar
	ld hl,00001h		;4f4b   ; caer al agua enciende 0xE140 y apaga la caida
	ld (0e140h),hl		;4f4e   ; 0xE140: esta en el agua
	xor a			;4f51
	ld (0e142h),a		;4f52
	ld a,0ffh		;4f55   ; 0xE0F8 a 0xFF: el ciclo de zancadas se reinicia al salir
	ld (0e0f8h),a		;4f57
	ld a,005h		;4f5a
	call PIDE_SONIDO		;4f5c   ; Sonido 5
	ld hl,0e068h		;4f5f
	ld bc,004b6h		;4f62   ; los cuatro sprites del chapoteo, de cuatro en cuatro bytes
AGUA_SPRITES:
	ld (hl),c			;4f65   ; el 0xB6 es el primer patron del chapoteo
	ld a,004h		;4f66
	call SUMA_A_HL		;4f68
	djnz AGUA_SPRITES		;4f6b   ; cuatro sprites
...
DIBUJA_EN_EL_AGUA:		; Coloca al pinguino asomando por el agujero, en Y=0x9F, y le PONE LAS PATAS AMARILLAS
	ld hl,(0e078h)		;4f6d
	ld l,09fh		;4f70   ; la Y se clava en 0x9F: la altura del agua
	call COLOCA_SPRITES		;4f72
	ld a,010h		;4f75
	call PONE_POSE		;4f77
	ld a,0e0h		;4f7a   ; la Y 0xE0 deja el sprite fuera de la pantalla
...
VUELCA_OCHO_SPRITES:		; Los ocho atributos de 0xE068 a la VRAM, sprites 6 a 13
	ld hl,0e068h		;4f85
	ld de,03b18h		;4f88   ; 0x3B18 son los sprites 6 a 13
	ld bc,00020h		;4f8b   ; 0x20 bytes: los ocho atributos
	call COPIA_A_VRAM		;4f8e
	jp SOMBRA_A_VRAM		;4f91
SIGUE_EN_EL_AGUA:		; Manotea hasta que se pulsa el gatillo
	ld hl,0e141h		;4f94   ; 0xE141 cuenta lo que se lleva en el agua
	inc (hl)			;4f97
	res 7,(hl)		;4f98   ; el bit 7 se apaga cada vuelta: la cuenta no pasa de 0x7F
	ld a,(hl)			;4f9a
	...
	bit 3,c		;4fb2   ; los bits 3 y 4 del contador reparten los tres dibujos
...
AGUA_ANIMA:		; Mueve las patas: el patron va cambiando entre 0x70, 0x74 y 0x78, y la posicion los sigue
	ld hl,(0e078h)		;4fc6   ; A trae el ajuste de X, B la Y del chapoteo y DE la pareja pose (D) y patron de las patas (E)
	ld l,b			;4fc9
	add a,h			;4fca
	ld c,a			;4fcb
	ld a,b			;4fcc
	ld b,e			;4fcd
	ld (0e0a1h),bc		;4fce
	add a,010h		;4fd2   ; La Y del sprite de las patas queda 16 por debajo del cuerpo, que sube y baja con el manoteo
	ld (0e0a0h),a		;4fd4
	push de			;4fd7
	call COLOCA_SPRITES		;4fd8   ; los cuatro sprites del pinguino se recolocan con la Y del manoteo
	pop af			;4fdb
	call PONE_POSE		;4fdc
...
SALE_DEL_AGUA:		; Con el gatillo se sale, y el periodo vuelve a 0x13: se sale del agua a la minima velocidad
	xor a			;4fe2   ; salir del agua apaga las dos banderas y reinicia las zancadas
	ld (0e140h),a		;4fe3
	ld (0e0f8h),a		;4fe6
	ld hl,00313h		;4fe9   ; 0xE100 vuelve al periodo 0x13: del agua se sale a la minima velocidad
	ld (0e100h),hl		;4fec
	ld a,(0e079h)		;4fef
	push af			;4ff2
	ld hl,066f0h		;4ff3   ; El atributo de 0x66F0, cuatro veces sobre los sprites 6 a 9: los que encendio el agua se recogen
	ld de,0e068h		;4ff6
	ld c,004h		;4ff9   ; cuatro sprites con el mismo atributo
	call REPITE_4_BYTES		;4ffb
	ld b,004h		;4ffe   ; cuatro juegos mas
SALIDA_SPRITES:
	ld c,(hl)			;5000   ; La lista sigue con cuatro juegos mas: un byte de cuantas veces y los cuatro del atributo
	inc hl			;5001
	push bc			;5002
	call REPITE_4_BYTES		;5003
	pop bc			;5006
	djnz SALIDA_SPRITES		;5007   ; tantos juegos como diga B
	pop hl			;5009
	ld l,090h		;500a   ; y el pinguino vuelve a la Y de andar, 0x90
	call COLOCA_SPRITES		;500c
	ld hl,004a0h		;500f   ; El patron 0xA0 y el color 4: la sombra vuelve a ser sombra
	ld (0e0a2h),hl		;5012
...
COGE_OBJETO:		; LAS BANDERAS DE LA PISTA. Los tipos 5 y 6 son las dos banderas que hay plantadas en el hielo -una inclinada a cada lado- y NO se esquivan: se recogen. Al tocarlas suena, se borra la ficha, se repinta el hueco y suman 500 puntos. Lo dice tambien el reparto de 0x4D6B: los tipos 0 a 4 se van al choque y solo el 5 y el 6 caen aqui. OJO, no confundirlas con la bandera que sube por el mastil de la base, que es otra cosa y va por sprites (0x55AD)
	ex de,hl			;501c   ; el dibujo se lee hacia ATRAS: los bytes que van delante del puntero
	dec hl			;501d
	dec hl			;501e
	ld d,(hl)			;501f
	dec hl			;5020
	...
	ld hl,0e1a0h		;5037
	call DIBUJA_BLOQUE		;503a   ; y de ahi lo pinta el interprete de bloques normal
...
MONTA_LA_PISTA:		; Monta la pista de la fase: colores, cielo, hielo, decorados y la lista de lo que va saliendo
	...
	call SUMA_A_HL		;504a
	ld a,007h		;504d
	bit 0,(hl)		;504f   ; el bit 0 elige entre la casilla 7 y la 9 para el cielo
...
PISTA_COLORES:
	...
	ld de,06263h		;5065
PISTA_DESCOMPRIME:
	push de			;5068   ; Las dos tiradas del descompresor -a 0x4588 y a 0x4F78- van con el juego de flujos del tipo de fase
	ld de,04588h		;5069   ; y se descomprimen en dos sitios: la tabla de colores y los patrones
	call DESCOMPRIME_DE		;506c
	pop hl			;506f
	ld de,04f78h		;5070
	call DESCOMPRIME_DE		;5073
	ld de,03860h		;5076   ; El cielo, con la casilla 7 o la 9
	ld bc,000e0h		;5079   ; 0xE0 casillas: siete filas de cielo
	ld a,(0e10ch)		;507c
	...
	ld (0e102h),a		;50a9
	ld (0e108h),a		;50ac
	ld hl,07249h		;50af   ; Los punteros de los dos grupos de trozos arrancan en 0x7249 y 0x7286
	ld (0e103h),hl		;50b2
	ld hl,07286h		;50b5
	ld (0e105h),hl		;50b8
...
SIGUIENTE_DECORADO:		; Coge el siguiente decorado de la lista de la fase; 0xFF quiere decir que ya no hay mas
	...
	ld hl,(0e10ah)		;50c7
	call SUMA_A_HL		;50ca
	ld a,(hl)			;50cd   ; El 0xFF de la lista: ya no hay mas decorados en esta fase
	cp 0ffh		;50ce
	ret z			;50d0
	ld (0e109h),a		;50d1
	ld bc,0e103h		;50d4   ; El bit 0 del decorado elige la pareja de punteros: 0xE103 (grupo A) o 0xE105 (grupo B)
	bit 0,a		;50d7
...
DECORADO_GRUPO:
	...
	ld a,(hl)			;50e4
	ld e,a			;50e5
	ld (bc),a			;50e6   ; El puntero elegido se apunta en la pareja y queda tambien en DE, que se dibuja ya
	inc hl			;50e7
	inc bc			;50e8
	ld a,(hl)			;50e9
	ld d,a			;50ea
	ld (bc),a			;50eb
	ex de,hl			;50ec
	ld a,008h		;50ed   ; Los ocho primeros bytes del grupo son sus cuatro punteros de trozos; el dibujo empieza en el byte 8
	call SUMA_A_HL		;50ef
...
PINTA_FILA_DE_PISTA:		; Escribe la fila compuesta en 0xE14E, que va siempre a la fila 9
	ld de,03920h		;510b   ; La cabecera 0x3920 (la fila 9) y el 0xFF de cierre arropan la fila compuesta, y ESCRIBE_CADENA la lleva a pantalla
	ld (0e14eh),de		;510e
	ld a,0ffh		;5112
	ld (0e170h),a		;5114
	ld hl,0e14eh		;5117
...
AVANZA_DECORADO:		; Cada 400 metros toca decorado nuevo
	...
	ld hl,0e107h		;5122   ; 0xE107 lo enciende el marcador de la distancia (0x4710)
	ld a,(hl)			;5125
	dec a			;5126
	ret nz			;5127
	ld a,(0e102h)		;5128   ; Y solo con el contador de trozos en 1, para no pisar un trozo a medias
	dec a			;512b
	ret nz			;512c
	ld (hl),a			;512d   ; el aviso se consume al atenderlo
	call SIGUIENTE_DECORADO		;512e
	or a			;5131
	ret nz			;5132
	ld hl,(0e103h)		;5133
	ld a,(0e109h)		;5136
	bit 0,a		;5139   ; el bit 0 de 0xE109 elige entre los dos decorados apuntados
...
DATA_decorados_por_fase:
	defb 002h,003h,000h,001h,077h,077h,077h,077h	; 5145  ....wwww
	defb 003h,002h,001h,000h,077h,077h,077h,077h	; 514d  ....wwww
	defb 003h,0ffh,001h,077h,077h,077h,077h,077h	; 5155  ...wwwww
	defb 0ffh,002h,000h,077h,077h,077h,077h,077h	; 515d  ...wwwww
	defb 003h,0ffh,001h,077h,077h,077h,077h,077h	; 5165  ...wwwww
	defb 0ffh,077h,077h,077h,077h,077h,077h,077h	; 516d  .wwwwwww
	defb 002h,003h,000h,002h,001h,000h,0ffh,077h	; 5175  .......w
	defb 002h,0ffh,000h,077h,077h,077h,077h,077h	; 517d  ...wwwww
	defb 002h,000h,003h,001h,077h,077h,077h,077h	; 5185  ....wwww
	defb 0ffh,003h,001h,077h,077h,077h,077h,077h	; 518d  ...wwwww

; ----------------------------------------------------------------------
; DATOS color_por_fase: Un byte por fase: con 0 el cielo es la casilla 7 y con
;   1 la 9. Cierra clavada en 0x519F, donde vuelve a haber codigo
...
AVANZA_LA_PISTA:		; Al ritmo de la velocidad, va sacando los trozos de decorado y moviendo los obstaculos
	ld hl,0e100h		;519f   ; 0xE100 es el periodo y 0xE101 su cuenta atras: los trabajos del dibujo se reparten dentro del periodo (3 el decorado, 1 el grupo B, 0 la recarga y el grupo A)
	ld c,(hl)			;51a2
	inc hl			;51a3
	dec (hl)			;51a4
...
PISTA_DIBUJA:		; Dibuja el trozo A de la tabla que apunta HL
	add a,a			;51c3   ; A por dos: la tabla del grupo es de punteros y el trozo se dibuja con el interprete de bloques
	call SUMA_A_HL		;51c4
	ld e,(hl)			;51c7
	inc hl			;51c8
	ld d,(hl)			;51c9
...
PISTA_MIRA:
	...
	inc b			;51d4
	srl c		;51d5   ; A media cuenta (periodo/2 contra la cuenta) se entra con B = 1
	ld a,(hl)			;51d7
	cp c			;51d8
	ret nz			;51d9
...
OBSTACULO_FICHA:
	...
	ld a,(hl)			;51ec
	cp 00bh		;51ed   ; A media cuenta solo avanzan las fichas del paso 0x0B en adelante: lo cercano corre el doble, que es la perspectiva
	ld a,006h		;51ef
...
OBSTACULO_DIBUJA:
	...
	push de			;5207
	push bc			;5208
	call DIBUJA_BLOQUE		;5209   ; el bloque se pinta antes de avanzar el puntero
	pop bc			;520c
	pop de			;520d
	inc hl			;520e   ; El puntero de dibujo se guarda avanzado: cada paso dibuja el trozo siguiente de la secuencia
	ex de,hl			;520f
	ld (hl),d			;5210
	dec hl			;5211
	ld (hl),e			;5212
	ld a,004h		;5213   ; cuatro bytes por ficha de obstaculo
OBSTACULO_SIGUIENTE:
	call SUMA_A_HL		;5215
	djnz OBSTACULO_FICHA		;5218
	call SUELTA_EL_PEZ		;521a   ; Detras del barrido van el pez, la foca y los dos choques: una vez por paso de pista
...
CREA_CUENTA:
	...
	dec (hl)			;523a
	ret nz			;523b
	ld (hl),a			;523c   ; Al dispararse, la cuenta se recarga con el periodo de 0xE10E y se busca ficha libre
	ld hl,0e112h		;523d
	ld b,003h		;5240   ; Tres fichas donde elegir (cuatro desde la fase 5); la de 0xE12A no entra: es la reservada de la pareja
	ld a,(0e0e0h)		;5242
	cp 005h		;5245
...
CREA_BUSCA_FICHA:
	...
	ld a,006h		;524e   ; El 6 salta a la ficha siguiente
	call SUMA_A_HL		;5250
...
CREA_EN_ESTA:
	...
	inc hl			;5257
	ex de,hl			;5258
	ld hl,0e111h		;5259   ; 0xE111 cuenta 0-7 (el `res 3` lo pliega): la posicion en la lista de ocho del decorado
	inc (hl)			;525c
	res 3,(hl)		;525d
	ld a,(hl)			;525f
...
CREA_RELLENA:
	...
	ld a,(0e190h)		;5290   ; Si 0xE190 quedo levantado, la pareja: el complemento del lado en 0xE191 y la ficha reservada de 0xE12A
	rra			;5293
	ret nc			;5294
	ld a,(0e191h)		;5295
	cpl			;5298
	and 003h		;5299   ; los dos bits bajos son el lado
	ld c,a			;529b
	ld hl,0e12ah		;529c
	ld a,(hl)			;529f   ; Solo si la reservada esta libre; si no, la pareja se pierde
	or a			;52a0
	jr nz,CREA_PAREJA_FIN		;52a1
	inc (hl)			;52a3   ; la ficha reservada queda ocupada
	inc hl			;52a4
...
RELLENA_FICHA:		; Copia a la ficha el tipo, el puntero de dibujo y el de choque, sacados de la tabla de al lado
	...
	ld de,052cbh		;52b4
	ld a,c			;52b7
	add a,a			;52b8   ; Tipo por seis -el doble, el cuadruple y la suma-: lo que ocupa cada entrada de la tabla
	ld c,a			;52b9
	add a,a			;52ba
	add a,c			;52bb
	call SUMA_A_DE		;52bc
	ld a,(de)			;52bf
	ld (hl),a			;52c0   ; Los dos primeros bytes de la entrada, el puntero al primer trozo de dibujo
	inc de			;52c1
	inc hl			;52c2
	ld a,(de)			;52c3
	ld (hl),a			;52c4
	inc de			;52c5
	inc hl			;52c6
	ld (hl),e			;52c7   ; Y como puntero de choque se guarda DE mismo: los cuatro bytes de pares estan ahi, en la propia tabla
	inc hl			;52c8
	ld (hl),d			;52c9
	ret			;52ca
...
;   dentro de los 92 trozos de 0x6BE9-0x7241, que es lo que confirma para que
;   son. Cierra clavada en 0x52F5
;   0x52cb..0x52f5  (42 bytes)
DATA_tabla_de_obstaculos:
	defb 019h,06fh,001h,053h,03ah,000h	; 52cb
	defb 0d2h,06fh,001h,013h,03bh,000h	; 52d1
	defb 091h,070h,001h,092h,03bh,000h	; 52d7
	defb 0e9h,06bh,02bh,05bh,010h,090h	; 52dd
	defb 085h,06dh,064h,053h,048h,088h	; 52e3
	defb 0c8h,071h,080h,02ch,000h,000h	; 52e9
	defb 050h,071h,02eh,02ch,000h,000h	; 52ef
...
MIRA_LA_CURVA:		; Cada 200 del marcador (la distancia va en BCD: dispara con las centenas impares y el resto en 82) mira en la tabla de nibbles que curva toca y monta el cambio
	ld hl,(0e0e5h)		;52f5
	ld a,h			;52f8   ; La distancia va en BCD: dispara con las centenas impares y el resto en 82, cada 200 del marcador
	and 001h		;52f9
	ret z			;52fb
	ld a,l			;52fc
	cp 082h		;52fd
	ret nz			;52ff
	ld hl,0e0e2h		;5300   ; 0xE0E2 es el indice de curva, que solo avanza aqui
	ld a,(hl)			;5303
	inc (hl)			;5304
...
CURVA_MONTA:
	ld c,a			;5316
	and 003h		;5317   ; El nibble: los bits 0-1 son el rumbo de 0xE194 (11: recto, nada que hacer) y el bit 3 enciende el "va torcida" (bit 1)
	cp 003h		;5319
	ret z			;531b
	bit 3,c		;531c
...
CURVA_GUARDA:
	...
	ld (hl),a			;5325
	inc hl			;5326
	bit 2,c		;5327   ; El bit 2 del nibble alarga la curva: 0xE195 a 2
...
PINTA_HORIZONTE:		; Dibuja el horizonte que corresponde a la curva
	ld hl,05402h		;533f
PINTA_HORIZONTE_HL:
	ld a,(0e194h)		;5342   ; 0xE194 por dos indexa los punteros: cada rumbo tiene su dibujo de horizonte
	add a,a			;5345
	call SUMA_A_HL		;5346
	ld e,(hl)			;5349
	inc hl			;534a
...
DESPLAZA_PINTA:
	...
	ld hl,0e197h		;5380
	inc (hl)			;5383   ; 0xE197 cuenta los desplazamientos: cada dieciseis se consume una tanda de 0xE195
	ld a,(hl)			;5384
	and 00fh		;5385   ; los cuatro bits bajos: una tanda cada dieciseis pasos
	jr nz,CURVA_ENDEREZA		;5387
	dec hl			;5389
	dec hl			;538a
	cp (hl)			;538b
	jr z,CURVA_ENDEREZA		;538c
	dec (hl)			;538e   ; y una tanda menos
	jr nz,CURVA_ENDEREZA		;538f
	dec hl			;5391   ; Agotadas las tandas, el rumbo (bit 0 de 0xE194) se INVIERTE y se repinta el horizonte: la curva se deshace girando al otro lado
	ld a,(hl)			;5392
	xor 001h		;5393
	ld (hl),a			;5395
...
CURVA_ENDEREZA:
	...
	ld hl,0e197h		;53a4
	ld a,(hl)			;53a7
	and 00fh		;53a8   ; Solo con la cuenta de 0xE197 redonda, para no cortar una tanda a medias
	ret nz			;53aa
	dec hl			;53ab   ; 0xE196 a cero -la pista deja de girar- y el horizonte pasa a los dibujos de la llegada (0x540A)
	ld (hl),a			;53ac
	ld hl,0540ah		;53ad
...
ARRASTRA_PINGUINO:		; En las curvas el pinguino se va de lado solo, al ritmo de la velocidad
	ld hl,0e196h		;53b6
	ld a,(hl)			;53b9   ; Sin curva en marcha (0xE196 a cero) no hay arrastre
	or a			;53ba
	ret z			;53bb
	inc hl			;53bc
	inc hl			;53bd
	dec (hl)			;53be
	ret nz			;53bf
	ld a,(0e100h)		;53c0   ; El mismo ritmo que el desplazamiento de la pista: periodo/4, contado en 0xE198
	srl a		;53c3
	srl a		;53c5
	ld (hl),a			;53c7
	ld hl,0e0d1h		;53c8   ; HL apunta a 0xE0D1, un byte de paso: el arrastre usa MUEVE_* sin tocar los bits de sentido de 0xE0FA
	ld de,(0e078h)		;53cb
	ld a,(0e194h)		;53cf
	rra			;53d2
...
;   nibble por curva: 0x5305 elige la mitad alta o la baja. Cierra con el 0xFF
;   de 0x5401, justo delante de los punteros
...
;   (dos apuntan al mismo). Cierra clavada en 0x5412, que es el primero de
;   ellos
;   0x5402..0x5412  (16 bytes)
DATA_punteros_del_horizonte:
	defw 05412h,05423h,05445h,05464h,05434h,05434h,054a2h,05483h	; 5402
...
;   a la base. Todos escriben en las filas 10 y 11. Acaba clavado en 0x54C1
;   0x5412..0x54c1  (175 bytes)
DATA_dibujos_del_horizonte:
	defb 049h,039h,014h,014h,013h,013h,015h,030h,030h,031h,010h,010h,010h,032h,033h,023h	; 5412  I9.....001...23#
	defb 0ffh,049h,039h,023h,074h,032h,010h,010h,010h,031h,030h,030h,015h,013h,013h,014h	; 5422  .I9#t2...100....
	defb 014h,0ffh,049h,039h,015h,014h,013h,012h,052h,010h,00fh,00fh,010h,011h,012h,013h	; 5432  ..I9....R.......
	defb 014h,015h,0ffh,049h,039h,014h,014h,013h,013h,015h,030h,030h,031h,010h,010h,010h	; 5442  ...I9.....001...
	defb 041h,047h,053h,053h,054h,054h,054h,054h,054h,054h,054h,054h,0feh,072h,039h,00fh	; 5452  AGSSTTTTTTTT.r9.
	defb 03eh,0ffh,040h,039h,054h,054h,054h,054h,054h,054h,054h,054h,053h,053h,088h,082h	; 5462  >.@9TTTTTTTTSS..
	defb 010h,010h,010h,031h,030h,030h,015h,013h,013h,014h,014h,0feh,06ch,039h,07fh,00fh	; 5472  ...100......l9..
	defb 0ffh,040h,039h,004h,004h,004h,004h,004h,004h,004h,004h,004h,004h,004h,004h,004h	; 5482  .@9.............
	defb 07dh,07ah,00fh,00fh,010h,011h,012h,013h,014h,015h,0feh,06ch,039h,079h,078h,0ffh	; 5492  }z.........l9yx.
	defb 049h,039h,015h,014h,013h,012h,052h,010h,00fh,00fh,039h,03ch,004h,004h,004h,004h	; 54a2  I9....R...9<....
	defb 004h,004h,004h,004h,004h,004h,004h,004h,004h,0feh,072h,039h,037h,038h,0ffh	; 54b2  ..........r978.
...
RETOCA_LADO:
	ld a,(0e194h)		;54c9
	bit 1,a		;54cc   ; El bit 1 de 0xE194 dice que la pista va torcida; el bit 0 (al acarreo por el `rra`) elige el juego de casillas, 0x10 mas abajo en un lado
	ret z			;54ce
	rra			;54cf
	ld a,(hl)			;54d0
...
BASE_MULTIPLICA:
	...
	ld a,h			;54f6   ; Las ocho rotaciones en dos tandas montan HL/16: el nibble bajo de H y el alto de L
	rlca			;54f7
	rlca			;54f8
	rlca			;54f9
	rlca			;54fa
	...
	and 00fh		;5503
	or e			;5505
	add a,c			;5506   ; Sumado a la X de partida: la interpolacion lineal, paso/16 del camino hasta la bandera
	ld h,a			;5507
BASE_ANDA:
	ld a,(0e078h)		;5508
...
DIBUJA_LA_BASE_PASO:		; Alterna los dos bloques de la escena de la base
	...
	ld a,(hl)			;5522
	inc (hl)			;5523
	ld hl,0554ch		;5524   ; El bit 0 de la cuenta 0xE13A elige bloque: pares el A (0x554C), impares el B (0x5560)
	rra			;5527
...
DIBUJA_EL_POLO:		; El remate del POLO SUR: descomprime cuatro sprites mas (0x6B81 -> VRAM 0x1F80, o sea los patrones 0xF0, 0xF4, 0xF8 y 0xFC), copia sus cuatro atributos de 0x6746 encima de los numeros 7 a 10 -dos en amarillo y dos en negro- y dibuja un tercer bloque de casillas. Los cuatro completan al pinguino inclinado que dibujan las casillas: el pico y la mancha de la barriga en amarillo, el ala y el lomo en negro
	ld hl,06b81h		;5531   ; 0x6B81: los patrones comprimidos de los cuatro sprites del remate (0xF0-0xFC)
	call DESCOMPRIME		;5534
	ld hl,06746h		;5537
	ld de,0e06ch		;553a   ; Los dieciseis bytes de 0x6746, los atributos, encima de los sprites 7 a 10
	ld bc,00010h		;553d
	ldir		;5540
...
DATA_bloque_base_a:
	...
	defb 0c4h,0c5h,0c6h,000h	; 555c

; ----------------------------------------------------------------------
; DATOS bloque_base_b: El otro
;   0x5560..0x556a  (10 bytes)
DATA_bloque_base_b:
	defb 002h,0eeh,0c2h,0eeh,0bdh,0c1h,0eeh,0c7h,0c8h,000h	; 5560  ..........

; ----------------------------------------------------------------------
; DATOS bloque_base_polo: El tercero, el del remate de 0x5531
;   0x556a..0x557f  (21 bytes)
DATA_bloque_base_polo:
	defb 0e1h,0eeh,0d2h,0d5h,0d8h,0eeh,0d3h,0d6h,0d9h,0dbh,0eeh,0d4h,0d7h,0dah,0dch,0eeh	; 556a  ................
	defb 0ddh,0deh,0dfh,00fh,000h	; 557a
...
MONTA_LA_BASE:		; Escribe el nombre de la base y descomprime su bandera en los patrones de sprite
	...
	ld a,(0e0e1h)		;558b
	ld c,a			;558e   ; El nombre que toca sale de la tabla de las diez fases
	add a,a			;558f
	call SUMA_A_HL		;5590
	ld e,(hl)			;5593
	inc hl			;5594
	...
	ld a,(0e0e0h)		;559d   ; La bandera se indexa aparte, con 0xE0E0, que es el numero del panel
	and 00fh		;55a0
	add a,a			;55a2
	call SUMA_A_HL		;55a3
	ld e,(hl)			;55a6
...
SUBE_LA_BANDERA:		; Sube la bandera dos pixeles por llamada hasta el tope del mastil. OJO CON DONDE SE PARA: el `cp 036h / ret z` se vuelve SIN GUARDAR el valor nuevo, asi que la bandera nunca llega a Y=0x36; se queda en 0x38. Medido en la llegada a Francia de la partida grabada: 0x50, 0x48, 0x3E y 0x38, y ahi se queda clavada
	...
	cp 036h		;55c0
	ret z			;55c2
	ld (0e060h),a		;55c3   ; La misma Y en los tres atributos: los tres sprites de la bandera van montados en el mismo sitio
	ld (0e064h),a		;55c6
	ld (0e068h),a		;55c9
...
;   las bases. Cierra clavada en 0x55ED, que es la primera cadena; con ocho,
;   nueve, once o doce entradas no cierra
;   0x55d9..0x55ed  (20 bytes)
DATA_punteros_de_las_bases:
	defw 05605h,05620h,05649h,05620h,05620h,05628h,05636h,055edh,055f7h,055f7h	; 55d9
...
;   ZEALAND (0x5610..0x561F) NO LA VISITA NADIE: no esta en la tabla, ninguna
...
DATA_nombres_de_las_bases:
	defb 0cdh,03ah,020h,02ah,021h,030h,021h,02eh,020h,0ffh,0cbh,03ah,020h,021h,035h,033h	; 55ed  .: *!0!. ..: !53
	defb 034h,032h,021h,02ch,029h,021h,020h,0ffh,0cch,03ah,020h,0c9h,032h,021h,02eh,023h	; 55fd  42!,)! ..: .2!.#
	defb 025h,020h,0ffh,0cah,03ah,020h,02eh,025h,0cah,00fh,0cbh,025h,021h,02ch,021h,02eh	; 560d  % ..: .%...%!,!.
	defb 024h,020h,0ffh,0ceh,03ah,020h,035h,033h,021h,020h,0ffh,0cbh,03ah,020h,021h,032h	; 561d  $ ..: 53! ..: !2
	defb 027h,025h,02eh,034h,029h,02eh,021h,020h,0ffh,0c8h,03ah,020h,035h,02eh,029h,034h	; 562d  '%.4).! ..: 5.)4
	defb 025h,024h,00fh,02bh,029h,02eh,027h,024h,02fh,02dh,020h,0ffh,0c8h,03ah,020h,034h	; 563d  %$.+).'$/- ..: 4
	defb 028h,025h,00fh,033h,02fh,035h,034h,028h,00fh,030h,02fh,02ch,025h,020h,0ffh	; 564d  (%.3/54(.0/,% .
...
DATA_punteros_de_banderas:
	defw 05689h,056b8h,056f1h,05775h,056f1h,056f1h,05715h,05738h,05670h,05689h	; 565c
...
DATA_banderas_comprimidas:
	defb 002h,000h,082h,003h,007h,003h,00fh,082h,007h,003h,009h,000h,082h,080h,0c0h,003h	; 5670  ................
	defb 0e0h,082h,0c0h,080h,027h,000h,000h,006h,00fh,087h,0cch,06dh,00ch,0ffh,00ch,06dh	; 5680  ....'......m...m
	defb 0cch,009h,000h,087h,0c0h,080h,000h,0c0h,000h,080h,0c0h,009h,000h,007h,000h,002h	; 5690  ................
	defb 0ffh,002h,0fbh,001h,0ffh,004h,000h,089h,03fh,03bh,03fh,03dh,02fh,03bh,03fh,0ffh	; 56a0  ........?;?=/;?.
	defb 0f7h,003h,0ffh,004h,000h,000h,006h,00dh,010h,000h,00ch,03fh,004h,000h,00ch,0f8h	; 56b0  ...........?....
	defb 014h,000h,000h,006h,004h,087h,0cch,06dh,00ch,0ffh,00ch,06dh,0cch,009h,000h,087h	; 56c0  .......m...m....
	defb 0c0h,080h,000h,0c0h,000h,080h,0c0h,009h,000h,007h,000h,005h,0ffh,004h,000h,08ch	; 56d0  ................
	defb 03fh,03fh,037h,03fh,03bh,02fh,03fh,0ffh,0ffh,0f7h,0ffh,0ffh,004h,000h,000h,006h	; 56e0  ??7?;/?.........
	defb 00dh,007h,000h,085h,0ffh,000h,0ffh,000h,0ffh,005h,000h,08bh,0ffh,000h,0ffh,000h	; 56f0  ................
	defb 0ffh,000h,0ffh,000h,0ffh,000h,0ffh,004h,000h,086h,055h,0aah,055h,0aah,055h,0aah	; 5700  ..........U.U.U.
	defb 01ah,000h,000h,006h,004h,004h,000h,084h,001h,003h,003h,001h,00ch,000h,084h,080h	; 5710  ................
	defb 0c0h,0c0h,080h,008h,000h,004h,0ffh,004h,000h,004h,0ffh,004h,000h,004h,0ffh,004h	; 5720  ................
	defb 000h,004h,0ffh,004h,000h,000h,00ah,007h,08ch,061h,031h,019h,00dh,001h,0ffh,0ffh	; 5730  .........a1.....
	defb 001h,00dh,019h,031h,061h,004h,000h,08ch,086h,08ch,098h,0b0h,080h,0ffh,0ffh,080h	; 5740  ...1a...........
	defb 0b0h,098h,08ch,086h,004h,000h,084h,00ch,084h,0c0h,0e0h,004h,000h,084h,0e0h,0c0h	; 5750  ................
	defb 084h,00ch,004h,000h,084h,030h,021h,003h,007h,004h,000h,084h,007h,003h,021h,030h	; 5760  .....0!.......!0
	defb 004h,000h,000h,008h,005h,08bh,003h,004h,00ah,00ch,02ch,03eh,018h,008h,008h,00ch	; 5770  ..........,>....
	defb 007h,005h,000h,08bh,0c0h,020h,050h,010h,030h,078h,01ch,014h,010h,030h,0e0h,005h	; 5780  ..... P.0x...0..
	defb 000h,085h,000h,000h,002h,001h,003h,003h,000h,083h,000h,000h,018h,005h,000h,085h	; 5790  ................
	defb 000h,000h,040h,080h,0c0h,003h,000h,083h,000h,000h,018h,005h,000h,000h,001h,00ah	; 57a0  ..@.............
	defb 00ch,038h,028h,029h,020h,0feh,016h	; 57b0

; ----------------------------------------------------------------------
; DATOS rotulos: Los rotulos de pantalla, en el formato de las cadenas: el
;   panel (1P, HI, STAGE, TIME), (c)KONAMI 1984, PLAY SELECT con JOYSTICK y
;   KEYBOARD, y TIME OUT
;   0x57b7..0x5839  (130 bytes)
DATA_rotulos:
	defb 038h,033h,034h,021h,027h,025h,020h,0feh,022h,038h,034h,029h,02dh,025h,020h,0feh	; 57b7  834!'% ."84)-% .
	defb 02ch,038h,038h,03ah,03bh,000h,000h,000h,000h,040h,041h,0feh,036h,038h,026h,031h	; 57c7  ,88:;....@A.68&1
	defb 037h,0feh,002h,038h,011h,030h,020h,0ffh,00ah,039h,01ah,02bh,02fh,02eh,021h,02dh	; 57d7  7..8.0 ..9.+/.!-
	defb 029h,000h,011h,019h,018h,014h,0ffh,0abh,039h,030h,02ch,021h,039h,000h,033h,025h	; 57e7  ).......90,!9.3%
	defb 02ch,025h,023h,034h,0feh,006h,03ah,011h,020h,03ch,03dh,000h,000h,030h,02ch,021h	; 57f7  ,%#4..:. <=..0,!
	defb 039h,000h,03eh,03fh,000h,02ah,02fh,039h,033h,034h,029h,023h,02bh,0feh,046h,03ah	; 5807  9.>?.*/934)#+.F:
	defb 012h,020h,03ch,03dh,000h,000h,030h,02ch,021h,039h,000h,03eh,03fh,000h,02bh,025h	; 5817  . <=..0,!9.>?.+%
	defb 039h,022h,02fh,021h,032h,024h,0ffh,0ech,038h,034h,029h,02dh,025h,000h,02fh,035h	; 5827  9"/!2$..84)-%./5
	defb 034h,0ffh	; 5837
...
;   sitios (VRAM 0x394A y 0x396C) y el flujo se acaba en 0x584A
...
;   bytes, uno cada 32 fotogramas: 0x41BA los apunta y 0x4103 los va leyendo.
...
;   inteligencia, va grabada. Cierra clavada en 0x588A, la primera instruccion
;   de MONTA_LA_FUENTE
;   0x584a..0x588a  (64 bytes)
DATA_mandos_de_la_demo:
	defb 000h,000h,000h,000h,000h,000h,000h,000h,000h,000h,000h,001h,009h,001h,001h,011h	; 584a  ................
	defb 005h,005h,009h,009h,001h,006h,004h,010h,001h,001h,011h,010h,001h,001h,009h,009h	; 585a  ................
	defb 001h,005h,015h,009h,019h,001h,001h,005h,011h,001h,001h,001h,011h,001h,001h,001h	; 586a  ................
	defb 011h,001h,000h,018h,019h,009h,001h,011h,001h,001h,001h,001h,001h,001h,001h,001h	; 587a  ................
...
MONTA_LA_FUENTE:		; Monta la fuente y los colores en los tres bancos de la pantalla. OJO: que la fuente se escriba tres veces NO quiere decir que los tres bancos acaben iguales. Cada escena descomprime luego sus dibujos ENCIMA, banco por banco, y comparando los tres en una VRAM de verdad solo quedan iguales DIECINUEVE casillas: la 0x00-0x0F, que son los cuadrados de color liso, y la 0xFD-0xFF, que estan vacias. Cada tercio de la pantalla tiene su propio juego de 256 casillas, y por eso render_tiles.py saca una hoja por banco y no una para todo
	ld de,00000h		;588a   ; Los tres bancos de colores de SCREEN 2 (0x0000, 0x0800 y 0x1000), uno por llamada, con el `jp` haciendo de tercera
...
DATA_fuente_comprimida:
	...
	defb 051h,001h,041h,000h,008h,019h,000h	; 5deb
...
;   otros 29 de 0x6246-0x6262 -que son los del primer tipo y caen dentro del
;   rango de arriba-, y 0x5044 elige entre las dos parejas mirando el bit 0 de
;   la tabla de 0x5195 con la fase: o 0x5DE4 y 0x6246, o 0x5DEF y 0x6263. EL
;   PUNTERO NO SE VE MIRANDO LAS INSTRUCCIONES DE AL LADO, y por eso el
;   reconstructor se saltaba estos bytes: 0x5065 hace `ld de,06263h`, 0x5068
;   lo GUARDA EN LA PILA y quien lo usa es el `pop hl` de 0x506F, dos
;   descompresiones despues. Es el mismo truco que la fuente en 0x58CF. Cierra
;   clavado en 0x6280, donde empieza el remate del banco 1
;   0x6263..0x6280  (29 bytes)
DATA_colores_de_pista_b:
	defb 017h,019h,001h,01fh,008h,0f9h,007h,0f9h,001h,0f4h,005h,0f9h,003h,0f4h,004h,0f9h	; 6263  ................
	defb 004h,0f4h,004h,0f9h,004h,0f4h,003h,0f9h,005h,0f4h,028h,0f9h,000h	; 6273  ..........(..

; ----------------------------------------------------------------------
; DATOS dibujos_banco1_resto: El remate del banco 1
;   0x6280..0x628f  (15 bytes)
DATA_dibujos_banco1_resto:
	defb 098h,04ah,004h,04fh,001h,041h,003h,044h,003h,04fh,001h,041h,004h,044h,000h	; 6280  .J.O.A.D.O.A.D.
...
CARGA_BANCO_2:		; Descomprime los dibujos del banco 2
	ld hl,062c5h		;628f
	call DESCOMPRIME		;6292   ; Dos llamadas seguidas sin recargar HL: la segunda sigue el flujo donde acabo la primera, que cada bloque trae su destino
	call DESCOMPRIME		;6295
	ld hl,05c2ch		;6298
	call DESCOMPRIME_SIGUE		;629b
	ld hl,062c7h		;629e
	ld de,072b0h		;62a1   ; La entrada con espejo: el mismo dibujo con los bits del reves
...
SPRITES_BUCLE:
	...
	ld c,a			;66dd   ; Un byte de cuantos y los cuatro del atributo; el cero cierra y se vuelca todo de una vez
...
;   (0x6709 y 0x670A, la Y y la X) para sacarlo al cielo, aparece un sol
...
;   barridos de la tabla entera -el ldir de 0x446E, el copiador de cuatro
;   bytes de 0x45BE, BORRA_SPRITES en 0x4606 y el borrado previo de 0x66D1-, y
...
;   que el 14 desde 0x45BE, que son las tres salidas del agua por cuatro
;   bytes, o sea la cadena que rehace los sprites parandose justo antes del 14
;   0x66ef..0x672c  (61 bytes)
DATA_atributos_de_partida:
	defb 00ah,0e0h,000h,07ch,000h	; 66ef
	defb 001h,090h,070h,000h,001h	; 66f4
	defb 001h,090h,080h,004h,001h	; 66f9
	defb 001h,0a0h,070h,008h,001h	; 66fe
	defb 001h,0a0h,080h,00ch,001h	; 6703
	defb 001h,0e0h,000h,0d4h,00ah	; 6708
	defb 001h,0e0h,000h,000h,008h	; 670d
	defb 001h,0e0h,000h,07ch,001h	; 6712
	defb 003h,0e0h,000h,07ch,006h	; 6717
	defb 001h,0aeh,070h,0a0h,004h	; 671c
	defb 001h,0aeh,080h,0a4h,004h	; 6721
	defb 008h,008h,000h,070h,000h	; 6726
	defb 000h	; 672b

; ----------------------------------------------------------------------
; DATOS atributos_de_base: La misma lista para la escena de la base, pero de
;   OCHO entradas en vez de treinta: 0x66CB pone los 128 bytes a cero antes de
;   aplicarla, asi que del atributo 8 en adelante no queda nada. Cierra
;   clavada en 0x6756, donde vuelve a haber codigo. Sus bytes de 0x6746 los
;   copia ademas 0x5537. Y AQUI ESTA EL UNICO SPRITE DEL PINGUINO QUE SE GIRA
...
DATA_atributos_de_base:
	defb 004h,04fh,080h,07ch,000h	; 672c
	defb 001h,052h,080h,0e8h,000h	; 6731
	defb 001h,052h,080h,0ech,000h	; 6736
	defb 001h,052h,080h,0e4h,00fh	; 673b
	defb 001h,07fh,078h,0d0h,00ah	; 6740
	defb 000h,07fh,070h,0f0h,00ah	; 6745
	defb 087h,078h,0f4h,00ah,077h	; 674a
	defb 070h,0f8h,001h,077h,080h	; 674f
	defb 0fch,001h	; 6754
...
DATA_sprites_comprimidos:
	...
	defb 080h,080h,080h,080h,000h,000h,000h,000h,000h,000h,000h,000h,000h	; 6bdc  .............
...
;   paso, y asi va creciendo lo que se acerca. Los siete obstaculos de 0x52CB
;   empiezan cada uno en uno de estos trozos
;   0x6be9..0x7241  (1624 bytes)
DATA_trozos_de_pista:
	defb 000h,041h,0efh,093h,000h,041h,0eeh,0a1h,095h,0a2h,000h,041h,0eeh,00fh,00fh,00fh	; 6be9  .A...A.....A....
	defb 0eeh,098h,098h,0a3h,000h,061h,0eeh,00fh,00fh,00fh,0edh,099h,09ah,09ah,09bh,000h	; 6bf9  .....a..........
	defb 081h,0edh,00fh,00fh,00fh,00fh,0ech,0a4h,09dh,09dh,09dh,09dh,0a5h,000h,0a1h,0ech	; 6c09  ................
	defb 00fh,00fh,00fh,00fh,00fh,00fh,0eah,0a8h,0aah,09fh,09fh,09fh,09fh,09fh,0abh,0a7h	; 6c19  ................
	defb 000h,0c1h,0eah,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,0e9h,070h,082h,06ch	; 6c29  .............p.l
	defb 06ch,06ch,06ch,06ch,06ch,083h,071h,000h,0e1h,0e9h,00fh,00fh,00fh,00fh,00fh,00fh	; 6c39  lllll.q.........
	defb 00fh,00fh,00fh,00fh,0e8h,0e7h,072h,073h,084h,08bh,06dh,06dh,06dh,06dh,06dh,06dh	; 6c49  ......rs..mmmmmm
	defb 08eh,086h,075h,000h,022h,0e7h,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6c59  ..u."...........
	defb 00fh,00fh,00fh,0e6h,072h,073h,084h,090h,06eh,06eh,06eh,06eh,06eh,06eh,06eh,091h	; 6c69  ....rs..nnnnnnn.
	defb 004h,074h,078h,0e5h,079h,07ah,08ah,085h,08ch,06fh,06fh,06fh,06fh,06fh,06fh,06fh	; 6c79  .tx.yz...ooooooo
	defb 08dh,06fh,07bh,07ch,07dh,000h,042h,0e6h,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6c89  .o{|}.B.........
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,0e5h,072h,073h,084h,090h,06eh,06eh,06eh,06eh	; 6c99  ........rs..nnnn
	defb 06eh,06eh,06eh,06eh,06eh,092h,086h,075h,00fh,0e4h,079h,07ah,08ah,085h,08ch,06fh	; 6ca9  nnnnn..u..yz...o
	defb 06fh,06fh,06fh,06fh,06fh,06fh,06fh,06fh,08ch,087h,07eh,07fh,000h,062h,0e5h,00fh	; 6cb9  oooooooo..~..b..
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,0e4h	; 6cc9  ................
	defb 072h,073h,084h,090h,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,091h,004h	; 6cd9  rs..nnnnnnnnnn..
	defb 074h,078h,0e3h,079h,07ah,08ah,085h,08ch,06fh,06fh,06fh,06fh,06fh,06fh,06fh,06fh	; 6ce9  tx.yz...oooooooo
	defb 06fh,06fh,08dh,06fh,07bh,07ch,07dh,000h,082h,0e4h,00fh,00fh,00fh,00fh,00fh,00fh	; 6cf9  oo.o{|}.........
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,0e3h,072h,073h,084h	; 6d09  .............rs.
	defb 090h,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,092h,086h,075h	; 6d19  .nnnnnnnnnnnn..u
	defb 00fh,0e2h,079h,07ah,08ah,085h,08ch,06fh,06fh,06fh,06fh,06fh,06fh,06fh,06fh,06fh	; 6d29  ..yz...ooooooooo
	defb 06fh,06fh,06fh,08ch,087h,07eh,07fh,000h,0a2h,0e3h,00fh,00fh,00fh,00fh,00fh,00fh	; 6d39  ooo..~..........
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,0e2h,072h,073h	; 6d49  ..............rs
	defb 084h,090h,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,091h	; 6d59  ..nnnnnnnnnnnnn.
	defb 004h,074h,078h,000h,0c2h,0e2h,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6d69  .tx.............
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,000h,000h,041h,0f0h,093h	; 6d79  .............A..
	defb 000h,041h,0efh,094h,095h,096h,000h,041h,0efh,00fh,00fh,00fh,0efh,097h,098h,098h	; 6d89  .A.....A........
	defb 000h,061h,0efh,00fh,00fh,00fh,0efh,099h,09ah,09ah,09bh,000h,081h,0efh,00fh,00fh	; 6d99  .a..............
	defb 00fh,00fh,0eeh,09ch,09dh,09dh,09dh,09dh,09eh,000h,0a1h,0eeh,00fh,00fh,00fh,00fh	; 6da9  ................
	defb 00fh,00fh,0edh,0a6h,0aah,09fh,09fh,09fh,09fh,09fh,0abh,0a7h,000h,0c1h,0edh,00fh	; 6db9  ................
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,0edh,070h,082h,06ch,06ch,06ch,06ch,06ch	; 6dc9  .........p.lllll
	defb 06ch,083h,077h,000h,0e1h,0edh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6dd9  l.w.............
	defb 0edh,0ech,076h,089h,088h,06dh,06dh,06dh,06dh,06dh,06dh,06dh,08eh,086h,075h,000h	; 6de9  ..v..mmmmmmm..u.
	defb 022h,0ech,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,0ech	; 6df9  "...............
	defb 076h,089h,08fh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,091h,004h,074h,078h,0ebh,080h	; 6e09  v..nnnnnnn..tx..
	defb 081h,093h,085h,06fh,06fh,06fh,06fh,06fh,06fh,06fh,08dh,06fh,07bh,07ch,07dh,000h	; 6e19  ...ooooooo.o{|}.
	defb 042h,0ech,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6e29  B...............
	defb 0ebh,072h,073h,084h,090h,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,091h,004h,074h	; 6e39  .rs..nnnnnnnn..t
	defb 078h,0eah,079h,07ah,08ah,085h,08ch,06fh,06fh,06fh,06fh,06fh,06fh,06fh,06fh,08dh	; 6e49  x.yz...oooooooo.
	defb 06fh,07bh,07ch,07dh,000h,062h,0ebh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6e59  o{|}.b..........
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,0eah,00fh,076h,089h,08fh,06eh,06eh,06eh,06eh	; 6e69  .........v..nnnn
	defb 06eh,06eh,06eh,06eh,06eh,06eh,091h,004h,074h,078h,0eah,080h,081h,093h,08dh,06fh	; 6e79  nnnnnn..tx.....o
	defb 06fh,06fh,06fh,06fh,06fh,06fh,06fh,06fh,06fh,08dh,06fh,07bh,07ch,07dh,000h,082h	; 6e89  ooooooooo.o{|}..
	defb 0ebh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6e99  ................
	defb 00fh,00fh,0eah,072h,073h,084h,090h,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh	; 6ea9  ...rs..nnnnnnnnn
	defb 06eh,06eh,091h,004h,074h,078h,0e9h,079h,07ah,08ah,085h,08ch,06fh,06fh,06fh,06fh	; 6eb9  nn..tx.yz...oooo
	defb 06fh,06fh,06fh,06fh,06fh,06fh,06fh,08dh,06fh,07bh,07ch,07dh,000h,0a2h,0eah,00fh	; 6ec9  ooooooo.o{|}....
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh	; 6ed9  ................
	defb 00fh,00fh,0e9h,00fh,076h,089h,08fh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh,06eh	; 6ee9  ....v..nnnnnnnnn
	defb 06eh,06eh,06eh,06eh,091h,004h,077h,078h,000h,0c2h,0eah,00fh,00fh,00fh,00fh,00fh	; 6ef9  nnnn..wx........
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,000h	; 6f09  ................
	defb 000h,041h,0efh,0afh,0b0h,000h,041h,0efh,094h,0a2h,000h,041h,0efh,00fh,00fh,0efh	; 6f19  .A....A....A....
	defb 0bfh,0c0h,000h,061h,0efh,00fh,00fh,0efh,0b7h,0b8h,000h,081h,0efh,00fh,00fh,0efh	; 6f29  ...a............
	defb 0bch,0bdh,000h,0a1h,0efh,00fh,00fh,0efh,0c1h,0c2h,000h,0c1h,0efh,00fh,00fh,0eeh	; 6f39  ................
	defb 094h,095h,095h,096h,000h,0e1h,0eeh,00fh,00fh,00fh,00fh,0ffh,0eeh,097h,098h,098h	; 6f49  ................
	defb 099h,000h,022h,0eeh,00fh,00fh,00fh,00fh,0eeh,09ah,098h,098h,09bh,0eeh,0abh,0aah	; 6f59  ..".............
	defb 0aah,0ach,000h,042h,0eeh,00fh,00fh,00fh,00fh,0edh,09ch,09dh,098h,098h,09eh,09fh	; 6f69  ...B............
	defb 0edh,0a3h,0a4h,0a1h,0a1h,0a5h,0a6h,000h,062h,0edh,00fh,00fh,00fh,00fh,00fh,00fh	; 6f79  ........b.......
	defb 0edh,09ah,098h,098h,098h,098h,09bh,0edh,0abh,0a1h,0a8h,0a8h,0a1h,0ach,000h,082h	; 6f89  ................
	defb 0edh,00fh,00fh,00fh,00fh,00fh,00fh,0ech,09ch,09dh,098h,098h,098h,098h,09eh,09fh	; 6f99  ................
	defb 0ech,0a3h,0a4h,0a8h,0a9h,0a9h,0a9h,0a5h,0a6h,000h,0a2h,0ech,00fh,00fh,00fh,00fh	; 6fa9  ................
	defb 00fh,00fh,00fh,00fh,0ech,09ah,098h,098h,098h,098h,098h,098h,09bh,000h,0c2h,0ech	; 6fb9  ................
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,00fh,000h,000h,041h,0efh,0b2h,000h,041h,0eeh	; 6fc9  ..........A...A.
	defb 0b4h,00fh,000h,041h,0eeh,00fh,0edh,0bfh,0b6h,000h,061h,0edh,00fh,00fh,0edh,0bah	; 6fd9  ...A......a.....
	defb 0bbh,000h,081h,0edh,00fh,00fh,0ech,0beh,0beh,000h,0a1h,0ech,00fh,00fh,0ebh,0c1h	; 6fe9  ................
	defb 0c3h,0c2h,000h,0c1h,0ebh,00fh,00fh,00fh,0e9h,094h,095h,095h,095h,096h,000h,0e1h	; 6ff9  ................
	defb 0e9h,00fh,00fh,00fh,00fh,00fh,0ffh,0e8h,097h,098h,098h,098h,099h,000h,022h,0e8h	; 7009  ..............".
	defb 00fh,00fh,00fh,00fh,00fh,0e7h,09ah,098h,098h,098h,09bh,0e7h,0abh,0aah,0aah,0aah	; 7019  ................
	defb 0ach,000h,042h,0e7h,00fh,00fh,00fh,00fh,00fh,0e6h,09ah,098h,098h,098h,09eh,09fh	; 7029  ..B.............
	defb 0e6h,0a0h,0a1h,0a1h,0a1h,0a5h,0a6h,000h,062h,0e6h,00fh,00fh,00fh,00fh,00fh,00fh	; 7039  ........b.......
	defb 0e5h,09ah,098h,098h,098h,098h,09bh,00fh,0e5h,0a0h,0a1h,0a8h,0a8h,0a1h,0a2h,000h	; 7049  ................
	defb 082h,0e5h,00fh,00fh,00fh,00fh,00fh,00fh,0e4h,09ah,098h,098h,098h,098h,09eh,09fh	; 7059  ................
	defb 0e4h,0a0h,0a1h,0a8h,0a8h,0a1h,0a2h,0a6h,000h,0a2h,0e4h,00fh,00fh,00fh,00fh,00fh	; 7069  ................
	defb 00fh,00fh,0e3h,09ah,098h,098h,098h,098h,098h,098h,09bh,00fh,000h,0c2h,0e3h,00fh	; 7079  ................
	defb 00fh,00fh,00fh,00fh,00fh,00fh,00fh,000h,000h,041h,0f0h,0b1h,000h,041h,0f0h,00fh	; 7089  .........A...A..
	defb 0b3h,000h,041h,0f1h,00fh,0f1h,0b5h,0c0h,000h,061h,0f1h,00fh,00fh,0f1h,0b9h,0bah	; 7099  ..A......a......
	defb 000h,081h,0f1h,00fh,00fh,0f2h,0beh,0beh,000h,0a1h,0f2h,00fh,00fh,0f2h,0c1h,0c3h	; 70a9  ................
	defb 0c2h,000h,0c1h,0f2h,00fh,00fh,00fh,0f2h,094h,095h,095h,095h,096h,000h,0e1h,0f2h	; 70b9  ................
	defb 00fh,00fh,00fh,00fh,00fh,0ffh,0f3h,097h,098h,098h,098h,099h,000h,022h,0f3h,00fh	; 70c9  ............."..
	defb 00fh,00fh,00fh,00fh,0f4h,09ah,098h,098h,098h,09bh,0f4h,0abh,0aah,0aah,0aah,0ach	; 70d9  ................
	defb 000h,042h,0f4h,00fh,00fh,00fh,00fh,00fh,0f4h,09ch,09dh,098h,098h,098h,09eh,0f4h	; 70e9  .B..............
	defb 0a3h,0a4h,0a1h,0a1h,0a1h,0a2h,000h,062h,0f4h,00fh,00fh,00fh,00fh,00fh,00fh,0f4h	; 70f9  .......b........
	defb 00fh,09ah,098h,098h,098h,098h,09bh,0f5h,0a0h,0a1h,0a8h,0a8h,0a1h,0a2h,000h,082h	; 7109  ................
	defb 0f5h,00fh,00fh,00fh,00fh,00fh,00fh,0f5h,09ch,09dh,098h,098h,098h,098h,09eh,0f5h	; 7119  ................
	defb 0a3h,0a4h,0a8h,0a9h,0a8h,0a1h,0a2h,000h,0a2h,0f5h,00fh,00fh,00fh,00fh,00fh,00fh	; 7129  ................
	defb 00fh,0f5h,00fh,09ah,098h,098h,098h,098h,098h,098h,09bh,000h,0c2h,0f6h,00fh,00fh	; 7139  ................
	defb 00fh,00fh,00fh,00fh,00fh,00fh,000h,000h,000h,041h,0efh,0c6h,000h,041h,0efh,0c7h	; 7149  .........A...A..
	defb 000h,041h,0efh,00fh,0efh,0c9h,000h,061h,0efh,00fh,0eeh,0ceh,000h,081h,0edh,0c8h	; 7159  .A.....a........
	defb 0cah,0edh,0cfh,0cbh,000h,081h,0edh,00fh,00fh,0edh,0cch,00fh,0ech,0a1h,0cdh,000h	; 7169  ................
	defb 0a1h,0edh,00fh,0ech,00fh,00fh,0ech,003h,0adh,0ebh,0b5h,0b1h,000h,0e1h,0ech,00fh	; 7179  ................
	defb 00fh,0ebh,0aeh,0aeh,0ebh,003h,003h,0eah,07fh,0b0h,000h,000h,002h,0ebh,00fh,00fh	; 7189  ................
	defb 0ebh,00fh,00fh,0e9h,0afh,003h,003h,0e9h,0afh,003h,003h,0e8h,07fh,0b2h,000h,000h	; 7199  ................
	defb 042h,0e9h,00fh,00fh,00fh,0e9h,00fh,00fh,00fh,0e8h,00fh,00fh,0e5h,003h,003h,003h	; 71a9  B...............
	defb 0e5h,003h,003h,003h,000h,0a2h,0e5h,00fh,00fh,00fh,0e5h,00fh,00fh,00fh,000h,000h	; 71b9  ................
	defb 000h,041h,0f0h,0c6h,000h,041h,0f0h,0c8h,000h,041h,0f0h,00fh,0f1h,0c9h,000h,061h	; 71c9  .A...A...A.....a
	defb 0f1h,00fh,0f1h,0ceh,000h,081h,0f1h,0c8h,0cah,0f1h,0cfh,0cbh,000h,081h,0f1h,00fh	; 71d9  ................
	defb 00fh,0f1h,00fh,0cch,0f1h,0a1h,0cdh,000h,0a1h,0f2h,00fh,0f1h,00fh,00fh,0f2h,0afh	; 71e9  ................
	defb 003h,0f2h,0b2h,000h,0e1h,0f2h,00fh,00fh,0f2h,00fh,0aeh,0aeh,0f3h,003h,003h,0f2h	; 71f9  ................
	defb 07fh,0b0h,000h,000h,002h,0f3h,00fh,00fh,0f3h,00fh,00fh,0f2h,00fh,0afh,003h,003h	; 7209  ................
	defb 0f3h,0afh,003h,003h,0f2h,07fh,0b2h,000h,000h,042h,0f3h,00fh,00fh,00fh,0f3h,00fh	; 7219  .........B......
	defb 00fh,00fh,0f2h,00fh,00fh,0f8h,003h,003h,003h,0f8h,003h,003h,003h,000h,0a2h,0f8h	; 7229  ................
	defb 00fh,00fh,00fh,0f8h,00fh,00fh,00fh,000h	; 7239  ........

; ----------------------------------------------------------------------
; DATOS arbol_de_decorados: Cuatro punteros en 0x7241 llevan a cuatro grupos,
;   cada uno con otros cuatro, y los dieciseis bloques de abajo embaldosan
;   0x732D-0x7518 sin dejar hueco. Pasados por el interprete de 0x4533 dibujan
;   los bordes de la pista
;   0x7241..0x7519  (728 bytes)
DATA_arbol_de_decorados:
	defw 07249h,07286h,072c3h,072f8h	; 7241
	defw 0732dh,07355h,0736dh,0738eh	; 7249
	defw 00f0fh,00e51h,00d72h,00b93h	; 7251
	defw 00ab5h,009d6h,008f7h,00618h	; 7259
	defw 0053ah,0035bh,0027dh,0019eh	; 7261
	defb 0bfh,000h,051h,039h,00fh,010h,011h,012h,013h,014h,015h,0ffh,060h,000h,000h,000h	; 7269  ..Q9........`...
	defb 0f3h,0f4h,0f3h,0f7h,0f5h,0f6h,0f4h,0f3h,0f7h,0f5h,0f6h,000h,000h,0a6h,073h,0ceh	; 7279  ..............s.
	defb 073h,0e6h,073h,007h,074h,00fh,00fh,040h,00eh,060h,00dh,080h,00bh,0a0h,00ah,0c0h	; 7289  s.s.t..@.`......
	defb 009h,0e0h,008h,000h,006h,020h,005h,040h,003h,060h,002h,080h,001h,0a0h,000h,048h	; 7299  ..... .@.`.....H
	defb 039h,015h,014h,013h,012h,052h,010h,00fh,0ffh,050h,0f3h,0f5h,0f6h,0f4h,0f5h,0f7h	; 72a9  9....R...P......
	defb 0f6h,0f4h,0f4h,0f3h,0f5h,0f6h,0f4h,0f5h,0f6h,000h,01fh,074h,040h,074h,061h,074h	; 72b9  ...........t@tat
	defb 07fh,074h,004h,00dh,053h,00ch,074h,00ah,096h,009h,0b7h,007h,0d9h,006h,0fah,005h	; 72c9  .t..S.t.........
	defb 01bh,003h,03dh,000h,051h,039h,039h,03ch,0feh,072h,039h,037h,038h,0ffh,060h,000h	; 72d9  ..=.Q99<.r978.`.
	defb 000h,000h,000h,0f8h,0fch,0f9h,0fbh,0fch,0f9h,0f9h,0f9h,0fbh,0fah,000h,000h,09ch	; 72e9  ................
	defb 074h,0bdh,074h,0deh,074h,0fch,074h,004h,00dh,040h,00ch,060h,00ah,080h,009h,0a0h	; 72f9  t.t.t.t..@.`....
	defb 007h,0c0h,006h,0e0h,005h,000h,003h,020h,000h,04dh,039h,07dh,07ah,0feh,06ch,039h	; 7309  ....... .M9}z.l9
	defb 079h,078h,0ffh,050h,000h,000h,000h,0f8h,0fbh,0f9h,0fch,0fbh,0f9h,0fbh,0fch,0fah	; 7319  yx.P............
	defb 000h,000h,000h,000h,021h,0f8h,013h,015h,012h,012h,012h,014h,014h,014h,0f5h,016h	; 7329  ....!...........
	defb 017h,018h,019h,019h,01ah,01bh,01ch,01ch,01ch,01ch,0f7h,01dh,01eh,01fh,01fh,01fh	; 7339  ................
	defb 020h,021h,022h,023h,0fah,00fh,024h,025h,026h,026h,026h,000h,021h,0fah,015h,0f5h	; 7349   !"#..$%&&&.!...
	defb 027h,028h,029h,029h,019h,02ah,0f7h,02bh,02bh,01eh,01fh,028h,029h,019h,02dh,0fah	; 7359  '()).*.++..().-.
	defb 02eh,026h,026h,000h,021h,0f8h,015h,015h,015h,012h,012h,012h,0f5h,016h,017h,018h	; 7369  .&&.!...........
	defb 019h,019h,02fh,01bh,01ch,022h,022h,0f7h,01dh,01eh,01fh,01fh,01fh,020h,021h,022h	; 7379  ../..""...... !"
	defb 0fah,00fh,024h,025h,000h,021h,0fah,012h,0f5h,027h,028h,029h,029h,019h,02dh,0f7h	; 7389  ..$%.!...'()).-.
	defb 02bh,02bh,01eh,01fh,02ch,029h,019h,02dh,0fah,02eh,026h,026h,000h,021h,0e0h,014h	; 7399  ++..,).-..&&.!..
	defb 014h,014h,012h,012h,012h,015h,013h,0e0h,05dh,05dh,05dh,05dh,05ch,05bh,05ah,05ah	; 73a9  ........]]]]\[ZZ
	defb 059h,058h,057h,0e0h,064h,063h,062h,061h,060h,060h,060h,05fh,05eh,0e0h,067h,067h	; 73b9  YXW.dcba```_^.gg
	defb 067h,066h,065h,00fh,000h,021h,0e5h,014h,0e5h,06bh,05ah,06ah,06ah,069h,068h,0e1h	; 73c9  gfe..!...kZjjih.
	defb 06eh,05ah,06ah,069h,060h,05fh,06ch,06ch,0e3h,067h,067h,06fh,000h,021h,0e2h,012h	; 73d9  nZji`_ll.ggo.!..
	defb 012h,012h,015h,015h,015h,0e1h,063h,063h,05dh,05ch,070h,05ah,05ah,059h,058h,057h	; 73e9  ......cc]\pZZYXW
	defb 0e1h,063h,062h,061h,060h,060h,060h,05fh,05eh,0e3h,066h,065h,00fh,000h,021h,0e5h	; 73f9  .cba```_^.fe..!.
	defb 012h,0e5h,06eh,05ah,06ah,06ah,069h,068h,0e1h,06eh,05ah,06ah,06dh,060h,05fh,06ch	; 7409  ..nZjjih.nZjm`_l
	defb 06ch,0e3h,067h,067h,06fh,000h,061h,0f3h,049h,043h,036h,0f5h,037h,048h,0f6h,03bh	; 7419  l.ggo.a.IC6.7H.;
	defb 042h,036h,0f8h,037h,038h,0f8h,00fh,00fh,054h,0fah,050h,047h,004h,0fbh,042h,048h	; 7429  B6.78...T.PG..BH
	defb 004h,004h,004h,0feh,042h,043h,000h,061h,0f3h,00fh,045h,004h,0f6h,038h,0f6h,04ah	; 7439  ....BC.a..E..8.J
	defb 04ch,004h,0f7h,037h,044h,038h,0fah,040h,041h,0fah,00fh,042h,043h,0fbh,00fh,051h	; 7449  L..7D8.@A..BC..Q
	defb 0fdh,044h,045h,004h,0feh,046h,04dh,000h,061h,0f4h,04fh,0f5h,040h,03dh,0f6h,00fh	; 7459  .DE..FM.a.O.@=..
	defb 035h,04dh,0f7h,04bh,04eh,004h,0f9h,04ah,04bh,0ffh,0fch,00fh,040h,041h,0fdh,00fh	; 7469  5M.KN..JK...@A..
	defb 042h,052h,0feh,04eh,053h,000h,061h,0f4h,03fh,036h,0f5h,046h,03ah,0f8h,036h,0f7h	; 7479  BR.NS.a.?6.F:.6.
	defb 00fh,037h,050h,0f8h,04fh,055h,045h,004h,0fah,046h,04ch,049h,0ffh,0ffh,043h,0feh	; 7489  .7P.OUE..FLI..C.
	defb 00fh,00fh,000h,061h,0eah,077h,084h,08ah,0e9h,089h,078h,0e7h,077h,083h,07ch,0e6h	; 7499  ...a.w....x.w.|.
	defb 079h,078h,0e5h,06ah,00fh,00fh,0e3h,004h,05dh,066h,0e0h,004h,004h,004h,05eh,058h	; 74a9  yx.j....]f....^X
	defb 0e0h,059h,058h,000h,061h,0eah,004h,086h,00fh,0e9h,079h,0e7h,004h,08dh,08bh,0e6h	; 74b9  .YX.a.....y.....
	defb 079h,085h,078h,0e4h,057h,056h,0e3h,059h,058h,00fh,0e3h,067h,00fh,0e0h,004h,05bh	; 74c9  y.x.WV.YX..g...[
	defb 05ah,0e0h,063h,05ch,000h,061h,0ebh,090h,0e9h,07eh,081h,0e7h,08eh,076h,00fh,0e6h	; 74d9  Z.c\.a...~...v..
	defb 004h,08fh,08ch,0e5h,061h,060h,0ffh,0e1h,057h,056h,00fh,0e0h,068h,058h,00fh,0e0h	; 74e9  ....a`..WV..hX..
	defb 069h,064h,000h,061h,0eah,077h,080h,0e9h,07bh,087h,0e7h,077h,0e6h,091h,078h,00fh	; 74f9  id.a.w..{..w..x.
	defb 0e4h,004h,05bh,06bh,065h,0e3h,05fh,062h,05ch,0ffh,0e0h,059h,0e0h,00fh,00fh,000h	; 7509  ..[ke._b\..Y....
...
DIBUJA_LA_META:		; En los ultimos 100 metros, cada 32 dibuja un trozo mas de la llegada
	...
	ret nz			;7522
	ld a,l			;7523
	rlca			;7524   ; Las tres rotaciones y el doble convierten 0x00/0x20/0x40/0x60/0x80 en 0, 2, 4, 6 y 8: la palabra de la tabla que toca
	rlca			;7525
	rlca			;7526
	add a,a			;7527   ; el `add a,a` remata el por 16
	ld hl,0755fh		;7528
	call SUMA_A_HL		;752b
	ld e,(hl)			;752e
	inc hl			;752f
	...
	ex de,hl			;7531
	ld a,(hl)			;7532
	and 0f0h		;7533   ; el nibble alto es el caracter
	ld c,a			;7535
	ld a,(hl)			;7536
	inc hl			;7537
	and 003h		;7538   ; y los dos bits bajos, el tercio
	add a,078h		;753a
	ld d,a			;753c
	ld a,c			;753d
META_FILA:
	ld b,(hl)			;753e
	inc hl			;753f
	ld a,020h		;7540   ; La misma cuenta que el interprete de bloques: 32 casillas por fila y el cierre 0xE0 de desplazamiento
	add a,c			;7542
	ld c,a			;7543
...
META_CASILLA:
	ld a,(hl)			;754f
	or a			;7550   ; El 0x00 cierra el bloque, igual que en DIBUJA_BLOQUE
	ret z			;7551
	cp 0e0h		;7552
...
;   del final. Cierra clavada en 0x7569, que es el primero de ellos
;   0x755f..0x7569  (10 bytes)
DATA_punteros_de_la_meta:
	defw 075a4h,07581h,07573h,0756eh,07569h	; 755f

; ----------------------------------------------------------------------
; DATOS bloques_de_la_meta: Los cinco bloques que va dibujando 0x7519
;   0x7569..0x75ef  (134 bytes)
DATA_bloques_de_la_meta:
	defb 021h,0efh,090h,091h,000h,021h,0efh,092h,093h,000h,001h,0efh,0afh,0eeh,094h,096h	; 7569  !....!..........
	defb 096h,098h,0eeh,095h,097h,097h,09ah,000h,0e0h,0efh,0afh,0efh,0b1h,0b2h,0edh,09dh	; 7579  ................
	defb 09bh,09ch,09ch,09ch,09bh,0edh,0c8h,09eh,0a4h,0a6h,0a8h,0a1h,0edh,0c8h,09fh,0a5h	; 7589  ................
	defb 0a7h,0a9h,0c9h,0edh,0a3h,0a0h,0a0h,0a0h,0adh,0a0h,000h,0c0h,0efh,071h,0efh,0b0h	; 7599  .............q..
	defb 0efh,0b1h,0b2h,0ebh,09dh,09dh,09bh,09bh,09bh,09ch,09ch,09ch,09ch,09bh,0ebh,0c8h	; 75a9  ................
	defb 0c8h,0c9h,0c9h,0c9h,0c9h,0c9h,0a2h,0a2h,0c9h,0ebh,0c8h,0c8h,0c9h,0aah,0c9h,0aah	; 75b9  ................
	defb 0c9h,099h,0c9h,0c9h,0ebh,0c8h,0c8h,0c9h,0abh,0c9h,0abh,0c9h,099h,0c9h,0c9h,0ebh	; 75c9  ................
	defb 0c8h,0c8h,0c9h,0c9h,0c9h,0c9h,0c9h,0aeh,0c9h,0c9h,0ebh,0a3h,0a3h,0ach,0a0h,0a0h	; 75d9  ................
	defb 0ach,0ach,09ah,0a0h,0ach,000h	; 75e9
...
PEZ_SIGUIENTE:
	...
	ret			;760e
PEZ_SALE:
	ld (0e181h),de		;760f   ; 0xE181 se queda apuntando al agujero; DE, a su tipo
	inc de			;7613
	ld a,(0e18ah)		;7614
	ld c,a			;7617
	...
	ld a,(0e009h)		;761e
	and 00ch		;7621   ; Con izquierda o derecha pulsadas el pez sale mirando alli; sin mando, alterna con 0xE185
	jr z,PEZ_LADO		;7623
	bit 2,a		;7625
	jr PEZ_MONTA		;7627
PEZ_LADO:
...
PEZ_SALTO_MEDIO:
	ld b,064h		;764f
PEZ_GUARDA:
	ld (hl),c			;7651   ; C traia el 0x7A del `ld bc`: la Y de salida; B la X del salto y D el patron que quedo antes
	inc hl			;7652
	ld (hl),b			;7653
	inc hl			;7654
	ld (hl),d			;7655
...
PEZ_TARDE:		; Ya no da tiempo: se marca el agujero segun su tipo
	xor a			;7657
	ld (0e192h),a		;7658   ; El agujero queda marcado en los bits 5-7 de 0xE183 segun su tipo: de ahi saldra la foca
	ld a,(de)			;765b
	cp 001h		;765c
...
PEZ_PASO:		; Lo coloca, lo copia a la VRAM y le lleva el arco del salto
	...
	ld bc,00004h		;767c   ; cuatro bytes: el atributo entero del sprite
	call COPIA_A_VRAM		;767f
	ld de,0e183h		;7682
	ld a,(de)			;7685
	and 003h		;7686   ; los dos bits bajos: la fase del vuelo del pez
	ret z			;7688
	ld hl,0e08eh		;7689
	call PEZ_GIRA		;768c
	ld a,(de)			;768f
	...
	dec (hl)			;7694   ; dos pixeles por paso al subir
	dec (hl)			;7695
	jr PEZ_ARCO		;7696
PEZ_ARCO_SUBE:
	inc (hl)			;7698
...
PEZ_ARCO:
	push hl			;769a
	ld hl,0e184h		;769b   ; 0xE184 cuenta el vuelo: sube hasta el 8, plano hasta el 0x10, cae desde ahi y en el 0x22 se recoge
	inc (hl)			;769e
	ld a,(hl)			;769f
	pop hl			;76a0
	dec hl			;76a1
	cp 008h		;76a2   ; hasta el 8 sube
	jr c,PEZ_SUBE		;76a4
	cp 010h		;76a6   ; del 8 al 0x10 vuela plano
	ret c			;76a8
	jr z,PEZ_CAE		;76a9
	cp 022h		;76ab
	jr nc,QUITA_EL_PEZ		;76ad
	ld c,005h		;76af   ; Cinco por paso al caer, y siete desde el 0x1A
	cp 01ah		;76b1
...
PEZ_CAE:		; Al llegar al paso 0x10 del arco le SUMA 8 al byte del patron (0xE08E): el pez cambia al dibujo grande. Con eso y el bit 2 que voltea 0x76CD salen OCHO dibujos, cuatro por lado: 0x80-0x8C mirando a un lado y 0x90-0x9C al otro. Los ocho estan MEDIDOS en la partida grabada (work/sprites_medidos.txt), siempre en el color del atributo 15
	...
	inc hl			;76bf
	ld a,(hl)			;76c0
	add a,008h		;76c1   ; Ocho arriba en el patron: el dibujo grande
	ld (hl),a			;76c3
	ret			;76c4
QUITA_EL_PEZ:		; Lo saca de la pantalla poniendole Y=0xE0
	ld (hl),0e0h		;76c5
	xor a			;76c7   ; Y borra estado y paso (0xE183/0xE184): el agujero queda libre
	ld (de),a			;76c8
	inc de			;76c9
	ld (de),a			;76ca
	jr PEZ_PASO		;76cb
PEZ_GIRA:		; Cada 16 fotogramas le da la vuelta al BIT 2 del patron y le deja los dos de abajo a cero: eso es lo que anima al pez. Los tres `srl` mas el `ccf` mas los tres `rla` dejan (patron & 0xF8) | (bit2 invertido) << 2
	ld a,(0e003h)		;76cd
	and 00fh		;76d0   ; Uno de cada dieciseis fotogramas
	ret nz			;76d2
	ld a,(hl)			;76d3
	srl a		;76d4   ; Tres a la derecha, el acarreo invertido y tres a la izquierda: el bit 2 da la vuelta y los bits 0-1 quedan a cero
	srl a		;76d6
	srl a		;76d8
	ccf			;76da
	rla			;76db
...
; ----------------------------------------------------------------------
; DATOS tabla_velocidad: Los CUATRO destinos, del CALL de 0x7730: nada,
;   acelerar, frenar, nada
;   0x7733..0x773b  (8 bytes)
DATA_tabla_velocidad:
	defw 07769h	; 7733  -> NI_UNA_COSA_NI_OTRA
	defw 0773bh	; 7735  -> ACELERA
	defw 07753h	; 7737  -> FRENA
	defw 07769h	; 7739  -> NI_UNA_COSA_NI_OTRA
...
ACELERA:		; Un escalon MENOS de periodo cada doce fotogramas, hasta 8: el tope de velocidad
	ld hl,0e0fdh		;773b
	xor a			;773e   ; El contador del freno se pone a cero: cambiar de idea empieza la cuenta de nuevo
	ld (hl),a			;773f
	inc hl			;7740
	inc hl			;7741
	ld (hl),a			;7742
	...
	inc (hl)			;7744
	ld a,(hl)			;7745
	sub 00ch		;7746   ; 0x0C fotogramas para subir un escalon de velocidad
	ret nz			;7748
	ld (hl),a			;7749
	ld hl,0e100h		;774a
	ld a,(hl)			;774d
	cp 009h		;774e   ; Nunca por debajo de 8, que es todo lo rapido que va el juego
	ret c			;7750
	dec (hl)			;7751
	ret			;7752
FRENA:		; Un escalon MAS cada cuatro fotogramas, hasta 0x13
	ld hl,0e0fdh		;7753
	xor a			;7756   ; Y este apaga el del acelerador: cuatro fotogramas contra doce, se frena tres veces mas deprisa
	ld (hl),a			;7757
	inc hl			;7758
	ld (hl),a			;7759
	inc hl			;775a
	inc (hl)			;775b
	ld a,(hl)			;775c
	sub 004h		;775d   ; cuatro fotogramas para bajar un escalon de velocidad
	ret nz			;775f
	ld (hl),a			;7760
	ld hl,0e100h		;7761
	ld a,(hl)			;7764
	cp 013h		;7765   ; El tope lento es 0x13
	ret nc			;7767
	inc (hl)			;7768   ; y se baja un escalon
NI_UNA_COSA_NI_OTRA:
	ret			;7769
PINTA_VELOCIMETRO:		; Las seis casillas del velocimetro, en la fila 0
	...
	ld a,(0e100h)		;7776
	ld b,a			;7779
	and 001h		;777a   ; el bit 0 elige entre las dos medias casillas
	add a,042h		;777c
	ld c,a			;777e
	ld a,b			;777f
	rra			;7780   ; La media vuelta y el `cpl` invierten el periodo -barra larga, periodo corto- y el bit 0 elige la media casilla (0x42/0x43)
	cpl			;7781
	and 00fh		;7782   ; los cuatro bits bajos son la longitud de la barra
	sub 006h		;7784
...
NUBE_NUEVA:
	ld a,(hl)			;77b3
	or a			;77b4
	ld a,004h		;77b5   ; El 4 es el salto de ficha para el `SUMA_A_DE` de abajo: cuatro bytes de atributo por nube
	jr nz,NUBE_SIGUIENTE		;77b7
	push hl			;77b9
	inc (hl)			;77ba
	ld hl,07838h		;77bb   ; Donde empieza cada uno
	ld a,b			;77be
	add a,a			;77bf   ; dos bytes por entrada
	call SUMA_A_HL		;77c0
	ld a,(hl)			;77c3
	ld (de),a			;77c4
	inc hl			;77c5
	...
	ld (de),a			;77c8
	inc de			;77c9
	ld a,0e0h		;77ca   ; El patron de arranque es el 0xE0 y el color el blanco 0x0F
	ld (de),a			;77cc
	inc de			;77cd
	ld a,00fh		;77ce
	ld (de),a			;77d0
	ld a,001h		;77d1   ; y la nube queda encendida
	pop hl			;77d3
NUBE_SIGUIENTE:
	call SUMA_A_DE		;77d4
	inc hl			;77d7
	...
	ld (hl),a			;77e2
	ld b,000h		;77e3   ; B a cero: ahora hace de indice de nube para los desplazamientos
	ld hl,0e14ah		;77e5
	ld de,0e0b8h		;77e8
MUEVE_LAS_NUBES:
	ld a,(hl)			;77eb
...
NUBE_PASO:
	...
	inc (hl)			;77fc
	ex de,hl			;77fd
	dec (hl)			;77fe   ; El paso: la Y sube uno y la X se corre lo que diga la tabla de 0x7836 para esa nube
	push de			;77ff
	ld de,07836h		;7800   ; La tabla de 0x7836 lleva un byte con signo por nube: -1, +1, -2 y +2
	ld a,b			;7803
	call SUMA_A_DE		;7804
	ld a,(de)			;7807
	inc hl			;7808
	...
	ld (hl),a			;780a
	ex de,hl			;780b
	pop hl			;780c   ; La Y de la nube, que el `dec (hl)` acaba de subir un pixel
	ld a,(hl)			;780d
	cp 00ch		;780e   ; En los pasos 0x0C y 0x18 el patron pasa a 0xDC y 0xD8: la nube crece al acercarse
	ld a,0dch		;7810
...
NUBE_CRECE:
	inc de			;781b   ; El patron nuevo, al tercer byte del atributo
	ld (de),a			;781c
NUBE_RECUPERA:
	pop de			;781d
NUBE_AVANZA:
	ld a,004h		;781e   ; El 4 salta al atributo siguiente
	call SUMA_A_DE		;7820
	inc hl			;7823
	ld a,004h		;7824   ; Cuatro nubes, y el volcado de los cuatro atributos de una vez (sprites 26 a 29)
	inc b			;7826
	cp b			;7827
...
ANIMA_LA_FOCA:		; Saca la foca del agujero: ocho pasos, del 7 al 14, con su fotograma sacado de la tabla de 0x78C1. Dibujada, se la reconoce: primero asoma un filo, luego la cabeza, y del paso 10 en adelante el cuerpo entero con las dos aletas
	ld a,(0e183h)		;7842
	and 0e0h		;7845   ; Los bits 5-7 de 0xE183 los dejo PEZ_TARDE: sin ellos no hay foca
	ret z			;7847
...
FOCA_FOTOGRAMA:		; Coge el fotograma del paso en que va
	...
	add a,008h		;785e
	ld b,a			;7860
	add a,a			;7861   ; dos bytes por entrada
	call SUMA_A_HL		;7862
	ld e,(hl)			;7865
	inc hl			;7866
	ld d,(hl)			;7867
	ld a,b			;7868
	ld b,004h		;7869   ; cuatro sprites para la foca
	cp 006h		;786b   ; En los pasos 13 y 14, con 0xE137 apagado, se enciende 0xE192: la foca pasa a los sprites 0-3, delante de todo
	jr c,FOCA_PASO		;786d
	ld hl,0e137h		;786f
	bit 0,(hl)		;7872
...
FOCA_PASO:
	cp 003h		;787b
	ex de,hl			;787d
	ld d,00ch		;787e   ; D es el tamano de cada variante: 12 bytes (cuatro sprites) o 6 (dos, en los tres primeros pasos)
	jr nc,FOCA_AVANZA		;7880
	ld d,006h		;7882
	ld b,002h		;7884
FOCA_AVANZA:
	ld a,(0e183h)		;7886
	cp 040h		;7889   ; 0x40 (tipo 0): la primera variante; 0x20: una mas alla; 0x80: dos, saltando D bytes por variante
...
FOCA_BYTE:		; Copia Y, X y patron, y se SALTA el cuarto byte del atributo: el color. Por eso el color de la foca no esta en el fotograma sino en la lista de atributos de 0x66EF, que le deja el primer sprite en NEGRO y los otros tres en ROJO OSCURO. Dibujada asi es una foca con la cara oscura, porque el negro es el atributo 16 y en un MSX el numero mas bajo va delante
	...
	inc de			;78a4   ; El `inc de` extra salta el byte de color del atributo, que no se toca
...
;   0x785B los indexa con paso-7, no con el tipo de obstaculo: leido de la
;   otra manera salen punteros que se van fuera del cartucho. Cierra clavada
;   en 0x78D3, que es el primero de ellos
;   0x78c1..0x78d1  (16 bytes)
DATA_punteros_de_la_foca:
	defw 078d3h,078e5h,078f7h,07909h,0792dh,07951h,07975h,07999h	; 78c1

; ----------------------------------------------------------------------
; DATOS fotogramas_de_la_foca: Los ocho fotogramas, cada uno con TRES
;   variantes que elige 0x7886 con el bit que 0x7657 encendio en 0xE183. Los
...
DATA_fotogramas_de_la_foca:
	...
	defb 0a1h,032h,0c0h,0b1h,02ah,0c4h,0a1h,032h,0c8h,0b1h,03ah,0cch	; 79b1  .2..*..2..:.

; ----------------------------------------------------------------------
; DATOS foca_escondida: El fotograma del paso 15, con las cuatro Y a 0xE0 para
;   sacarla de la pantalla. Cierra clavado en 0x79C9, donde vuelve a haber
;   codigo
;   0x79bd..0x79c9  (12 bytes)
DATA_foca_escondida:
	defb 0e0h,000h,000h,0e0h,000h,000h,0e0h,000h,000h,0e0h,000h,000h	; 79bd  ............
...
PIDE_SONIDO:		; Pone en marcha el sonido A, si su numero manda mas que el que ya suena
	di			;79c9   ; Con las interrupciones paradas: ATIENDE_SONIDO corre en la interrupcion y toca estos mismos bloques
	push hl			;79ca
	push de			;79cb
	push bc			;79cc
	push af			;79cd
	call PIDE_SONIDO_SIN_GUARDAR		;79ce   ; La entrada sin salvar registros es la que usan el rearranque y el encadenado
	pop af			;79d1
	pop bc			;79d2
	pop de			;79d3
	pop hl			;79d4
...
SONIDO_PRIORIDAD:
	...
	call SUMA_A_DE		;79f5
SONIDO_MONTA_CANAL:
	dec hl			;79f8   ; Los dos `dec` bajan del +2 del canal al +0: un fotograma de arranque, duracion 1 y el numero, que es lo que marca el canal ocupado
	dec hl			;79f9
	ld (hl),001h		;79fa   ; la cuenta de arranque, a uno
	inc hl			;79fc
	ld (hl),001h		;79fd
	inc hl			;79ff
	ld a,c			;7a00
	...
	ld a,(de)			;7a07
	ld (hl),a			;7a08
	ld a,008h		;7a09   ; Ocho mas alla del +4 es el +2 del canal siguiente; los flujos de un sonido de varios canales van seguidos en la tabla
	call SUMA_A_HL		;7a0b
	inc de			;7a0e
	djnz SONIDO_MONTA_CANAL		;7a0f   ; tantos canales como diga B
SONIDO_NO:
	ret			;7a11
SONIDO_REPITE:		; El 0xFE: repite el trozo las veces que diga el byte de detras
	inc hl			;7a12
	ld a,(hl)			;7a13
	inc a			;7a14   ; El 0xFF (el `inc a` lo caza) repite siempre; si no, el +9 cuenta las vueltas
	jr z,SONIDO_ENCADENA		;7a15
	inc (ix+009h)		;7a17
	dec a			;7a1a
	cp (ix+009h)		;7a1b
	jr nz,SONIDO_ENCADENA		;7a1e
	xor a			;7a20   ; Cumplidas las vueltas, el +9 a cero y el canal calla
	ld (ix+009h),a		;7a21
	jp CALLA_CANAL		;7a24
SONIDO_ENCADENA:		; Al acabarse un flujo puede arrancar otro
	ld a,(ix+002h)		;7a27   ; Rearrancar es volver a pedir el numero del +2: como empata consigo mismo, la prioridad lo deja pasar
	push bc			;7a2a
...
SONIDO_CANAL:
	exx			;7a49
	ld a,(ix+002h)		;7a4a   ; El +2 es el numero del sonido: a cero, el canal esta libre
	or a			;7a4d
	call nz,PASO_DE_CANAL		;7a4e
	inc c			;7a51   ; C sube de dos en dos (1, 3, 5): el registro ALTO del par de periodo de cada canal del PSG
	inc c			;7a52
	exx			;7a53
	add ix,de		;7a54
...
LEE_NOTA:		; Saca del flujo la nota que toca
	...
	bit 7,(ix+002h)		;7a6e   ; El bit 7 del NUMERO del sonido separa la musica (NOTA_CONTROL) del efecto
	jp nz,NOTA_CONTROL		;7a72
	and 0f0h		;7a75
	cp 020h		;7a77   ; Un evento 0x2n de efecto trae duracion nueva en el nibble bajo
...
NOTA_NORMAL:
	ld a,(hl)			;7a82
	and 0f0h		;7a83   ; Nibble alto el volumen; el bajo y el byte siguiente, el periodo de 12 bits
	ld b,a			;7a85
	xor (hl)			;7a86
	ld d,a			;7a87
	inc hl			;7a88
	ld e,(hl)			;7a89
	inc hl			;7a8a
	ld (ix+003h),l		;7a8b   ; Se apunta por donde va el flujo antes de tocar el PSG
	ld (ix+004h),h		;7a8e
	ex de,hl			;7a91
...
NOTA_VOLUMEN:
	ld h,a			;7a9c
	ld a,(ix+001h)		;7a9d   ; La duracion pasa del +1 al +0, y el +8 arranca 3 por encima: la cuenta del decaimiento
	ld (ix+000h),a		;7aa0
	add a,003h		;7aa3
	ld (ix+008h),a		;7aa5
...
NOTA_DECAE:		; Mientras dura la nota le va bajando el volumen
	dec (ix+000h)		;7ab1
	jr z,LEE_NOTA		;7ab4
	dec (ix+008h)		;7ab6   ; El +8 arranco 3 por encima del +0; mientras no se igualen baja DOS por vuelta (esta y DECAE_MAS) y el volumen cae un paso
	ld a,(ix+008h)		;7ab9
	cp (ix+000h)		;7abc
...
DECAE_VOLUMEN:
	...
	ld h,a			;7ad1
ESCRIBE_VOLUMEN:
	ld a,c			;7ad2   ; La media vuelta convierte C (1, 3, 5) en 0x80, 0x81 y 0x82, y el `add 0x88` los deja en 8, 9 y 10: el bit alto se va por el acarreo
	rrca			;7ad3
	add a,088h		;7ad4
	ld e,h			;7ad6
...
NOTA_CONTROL:		; El 0xFD: cambia la octava y el decaimiento
	...
	inc hl			;7ade
	ld a,(hl)			;7adf
	and 007h		;7ae0   ; Los tres bits bajos son la octava (+5); el `xor (hl)` deja los altos y las rotaciones los bajan: el volumen de arranque (+6)
	ld (ix+005h),a		;7ae2
	xor (hl)			;7ae5
	rrca			;7ae6
	rrca			;7ae7
	rrca			;7ae8
	ld (ix+006h),a		;7ae9
	inc hl			;7aec   ; Detras del 0xFD viene ya la nota, sin evento propio
	ld a,(hl)			;7aed
NOTA_MONTA:
	and 00fh		;7aee
	ld b,a			;7af0
	...
	ld (ix+003h),l		;7af3
	ld (ix+004h),h		;7af6
	rrca			;7af9   ; Cuatro rotaciones bajan el nibble alto: el indice en la tabla de duraciones de 0x7B44
	rrca			;7afa
	rrca			;7afb
	rrca			;7afc
	ld hl,07b44h		;7afd
	...
	ld (ix+001h),a		;7b04
	ld a,b			;7b07
	sub 00ch		;7b08   ; Restar 12 mira si la nota era el silencio y de paso deja el volumen a CERO, que es justo lo que toca si lo era; si no, 0x7B0F lo repone del +6
	ld (ix+007h),a		;7b0a
	jr z,NOTA_PERIODO		;7b0d
	ld a,(ix+006h)		;7b0f
	ld (ix+007h),a		;7b12
NOTA_PERIODO:
	call NOTA_VOLUMEN		;7b15   ; Primero el volumen y las cuentas; B ha guardado la nota para el periodo
	ld a,b			;7b18
	ld hl,07b38h		;7b19
	call SUMA_A_HL		;7b1c
	ld l,(hl)			;7b1f
	ld h,000h		;7b20
	ld a,(ix+005h)		;7b22   ; La octava del +5 a cero deja el periodo tal cual: la tabla es la octava mas aguda
	or a			;7b25
	jr z,ESCRIBE_PERIODO		;7b26
	ld b,a			;7b28
NOTA_OCTAVA:		; Cada doblado del periodo BAJA una octava: el +5 cuenta octavas hacia lo grave desde la tabla, que es la mas aguda
	add hl,hl			;7b29
...
DATA_tabla_de_notas:
	defb 06ah,064h,05fh,059h,054h,050h,04bh,047h,043h,03fh,03ch,038h	; 7b38  jd_YTPKGC?<8
...
DATA_tabla_de_duraciones:
	defb 008h,010h,020h,030h,040h,060h,005h,00ah,00fh,014h,064h,01eh,018h,03ch	; 7b44  .. 0@`....d..<

; ----------------------------------------------------------------------
; DATOS punteros_de_sonido: Veinticuatro punteros a los flujos. Cierra clavada
;   en 0x7B82, que es el primero. El del sonido 0 apunta fuera de la ROM
;   porque no se pide nunca, y los tres ultimos apuntan al 0xFF de 0x7B82: el
...
DATA_punteros_de_sonido:
	defw 02850h,07d46h,07d20h,07d7eh,07d86h,07d6ah,07d5eh,07d4ch	; 7b52
	defw 07e75h,07d2eh,07b83h,07c03h,07e22h,07e3fh,07e62h,07cd9h	; 7b62
	defw 07cf7h,07d0eh,07d8eh,07dc0h,07df3h,07b82h,07b82h,07b82h	; 7b72

; ----------------------------------------------------------------------
; DATOS flujos_de_sonido: Los veintiun flujos de musica y efectos
;   0x7b82..0x7eb7  (821 bytes)
DATA_flujos_de_sonido:
	defb 0ffh,0fdh,05ah,03bh,0fdh,059h,022h,014h,054h,030h,024h,016h,056h,039h,027h,0fdh	; 7b82  ..Z;.Y".T0$.V9'.
	defb 05ah,01bh,0fdh,059h,032h,020h,0fdh,05ah,01bh,03bh,039h,047h,0fdh,059h,002h,007h	; 7b92  Z..Y2 .Z.;9G.Y..
	defb 004h,007h,002h,007h,004h,007h,002h,007h,004h,007h,002h,007h,004h,007h,012h,006h	; 7ba2  ................
	defb 00ch,006h,00ch,012h,006h,00ch,006h,00ch,002h,009h,004h,009h,002h,009h,004h,009h	; 7bb2  ................
	defb 002h,009h,004h,009h,012h,007h,00ch,007h,00ch,012h,007h,00ch,007h,00ch,002h,007h	; 7bc2  ................
	defb 006h,007h,002h,007h,002h,007h,005h,007h,002h,007h,000h,007h,004h,007h,000h,007h	; 7bd2  ................
	defb 000h,007h,003h,007h,000h,007h,0fdh,05ah,00bh,0fdh,059h,007h,002h,007h,0fdh,05ah	; 7be2  .......Z..Y....Z
	defb 00bh,0fdh,059h,007h,000h,006h,002h,006h,000h,006h,017h,01ch,016h,017h,02ch,0feh	; 7bf2  ..Y...........,.
	defb 0ffh,0fdh,05bh,017h,0fdh,05ah,012h,012h,0fdh,05bh,017h,0fdh,05ah,012h,012h,0fdh	; 7c02  ..[..Z...[..Z...
	defb 05bh,017h,0fdh,05ah,010h,010h,0fdh,05bh,017h,0fdh,05ah,010h,010h,0fdh,05bh,017h	; 7c12  [..Z...[..Z...[.
	defb 0fdh,05ah,014h,014h,0fdh,05bh,017h,0fdh,05ah,014h,014h,0fdh,05bh,016h,0fdh,05ah	; 7c22  .Z...[..Z...[..Z
	defb 012h,012h,0fdh,05bh,016h,0fdh,05ah,012h,012h,0fdh,05bh,010h,019h,019h,017h,0fdh	; 7c32  ...[..Z...[.....
	defb 05ah,012h,012h,0fdh,05bh,017h,0fdh,05ah,012h,012h,0fdh,05bh,019h,0fdh,05ah,012h	; 7c42  Z...[..Z...[..Z.
	defb 012h,0fdh,05bh,017h,0fdh,05ah,012h,012h,0fdh,05bh,019h,0fdh,05ah,012h,012h,0fdh	; 7c52  ..[..Z...[..Z...
	defb 05bh,017h,0fdh,05ah,012h,0fdh,05bh,01bh,027h,01ch,01bh,0fdh,05ah,012h,012h,0fdh	; 7c62  [..Z..[.'...Z...
	defb 05bh,01bh,0fdh,05ah,012h,012h,0fdh,05bh,016h,0fdh,05ah,012h,012h,0fdh,05bh,016h	; 7c72  [..Z...[..Z...[.
	defb 0fdh,05ah,012h,012h,0fdh,05bh,019h,0fdh,05ah,012h,012h,0fdh,05bh,019h,0fdh,05ah	; 7c82  .Z...[..Z...[..Z
	defb 012h,012h,0fdh,05bh,017h,0fdh,05ah,012h,012h,0fdh,05bh,017h,0fdh,05ah,012h,012h	; 7c92  ...[..Z...[..Z..
	defb 0fdh,05bh,012h,017h,01bh,017h,01bh,0fdh,05ah,012h,0fdh,05bh,017h,0fdh,05ah,010h	; 7ca2  .[......Z..[..Z.
	defb 014h,0fdh,05bh,017h,0fdh,05ah,010h,014h,0fdh,05bh,017h,0fdh,05ah,012h,01ch,0fdh	; 7cb2  ..[..Z...[..Z...
	defb 05bh,019h,0fdh,05ah,012h,01ch,012h,01ch,0fdh,05bh,01bh,0fdh,05ah,002h,000h,0fdh	; 7cc2  [..Z.....[..Z...
	defb 05bh,00bh,009h,007h,00ch,0feh,0ffh,0fdh,059h,090h,080h,060h,090h,0fdh,05ah,08bh	; 7cd2  [.......Y..`..Z.
	defb 069h,097h,094h,097h,094h,072h,074h,075h,077h,079h,077h,079h,07bh,0fdh,061h,090h	; 7ce2  i....rtuwywy{.a.
	defb 080h,060h,090h,0ffh,0ffh,0fdh,05bh,097h,097h,097h,09ch,097h,097h,097h,09ch,095h	; 7cf2  .`....[.........
	defb 092h,097h,0fdh,05ch,097h,0fdh,063h,090h,097h,097h,0ffh,0ffh,0fdh,05bh,090h,090h	; 7d02  ...\..c......[..
	defb 090h,09ch,090h,090h,090h,09ch,0ach,0fdh,05ah,084h,064h,094h,0ffh,0ffh,022h,0d0h	; 7d12  ........Z.d...".
	defb 07fh,0b0h,070h,0b0h,077h,0a0h,062h,090h,050h,080h,043h,0ffh,023h,090h,060h,090h	; 7d22  ..p.w.b.P.C.#.`.
	defb 040h,090h,060h,090h,040h,090h,060h,090h,040h,090h,060h,090h,040h,090h,060h,090h	; 7d32  @.`.@.`.@.`.@.`.
	defb 040h,090h,060h,0ffh,021h,0a0h,025h,0a0h,027h,0ffh,021h,0c0h,0ddh,0c0h,0bbh,0b0h	; 7d42  @.`.!.%.'.!.....
	defb 0aah,0b0h,099h,0a0h,088h,0a0h,077h,090h,066h,090h,055h,0ffh,022h,0c0h,055h,0c0h	; 7d52  ......w.f.U.".U.
	defb 066h,0c0h,055h,0b0h,044h,0a0h,033h,0ffh,022h,0e0h,0a5h,0c0h,0b5h,0a0h,0c5h,090h	; 7d62  f.U.D.3.".......
	defb 0d5h,080h,0e5h,070h,0f5h,061h,005h,051h,025h,051h,045h,0ffh,021h,0c1h,003h,0c1h	; 7d72  ...p.a.Q%QE.!...
	defb 00dh,0c1h,006h,0ffh,021h,0c1h,043h,0c1h,04dh,0c1h,046h,0ffh,0fdh,05ah,07bh,0fdh	; 7d82  ....!.C.M.F..Z{.
	defb 059h,072h,074h,072h,097h,076h,074h,0b2h,0fdh,05ah,07bh,097h,067h,069h,06bh,0fdh	; 7d92  Yrtr.vt..Z{.gik.
	defb 059h,060h,0fdh,05ah,07bh,0fdh,059h,072h,074h,072h,097h,076h,074h,062h,064h,062h	; 7da2  Y`.Z{.Yrtr.vtbdb
	defb 060h,0fdh,05ah,06bh,0fdh,059h,060h,0fdh,05ah,06bh,069h,097h,09ch,0ffh,0fdh,05ah	; 7db2  `.Zk.Y`.Zki....Z
	defb 077h,07bh,0fdh,059h,070h,0fdh,05ah,07bh,0fdh,059h,092h,070h,070h,0fdh,05ah,0bbh	; 7dc2  w{.Yp.Z{.Y.pp.Z.
	defb 077h,092h,09ch,077h,07bh,0fdh,059h,070h,0fdh,05ah,07bh,0fdh,059h,092h,070h,070h	; 7dd2  w..w{.Yp.Z{.Y.pp
	defb 0fdh,05ah,06bh,0fdh,059h,060h,0fdh,05ah,06bh,069h,067h,069h,067h,066h,092h,09ch	; 7de2  .Zk.Y`.Zkigigf..
	defb 0ffh,0fdh,05bh,077h,076h,074h,072h,070h,0fdh,05ch,07bh,079h,077h,0fdh,05bh,077h	; 7df2  ..[wvtrp.\{yw.[w
	defb 076h,074h,072h,070h,0fdh,05ch,07bh,079h,077h,0fdh,05bh,077h,076h,074h,072h,070h	; 7e02  vtrp.\{yw.[wvtrp
	defb 0fdh,05ch,07bh,079h,077h,0fdh,05bh,072h,0fdh,05ch,072h,074h,076h,077h,09ch,0ffh	; 7e12  .\{yw.[r.\rtvw..
	defb 0fdh,059h,094h,074h,074h,094h,072h,070h,0b5h,0fdh,05ah,075h,0b5h,0fdh,059h,075h	; 7e22  .Y.tt.rp..Zu..Yu
	defb 094h,070h,074h,092h,0fdh,05ah,079h,07bh,0fdh,059h,0d0h,01ch,0ffh,0fdh,05bh,090h	; 7e32  .pt..Zy{.Y....[.
	defb 070h,070h,090h,0fdh,05ah,07bh,077h,0fdh,059h,0b0h,0fdh,05ah,070h,0b0h,0fdh,059h	; 7e42  pp..Z{w.Y..Zp..Y
	defb 070h,090h,0fdh,05ah,077h,0fdh,059h,070h,0fdh,05ah,09bh,075h,077h,0d7h,01ch,0ffh	; 7e52  p..Zw.Yp.Z.uw...
	defb 0fdh,05bh,097h,094h,097h,094h,099h,095h,099h,095h,097h,094h,097h,095h,097h,097h	; 7e62  .[..............
	defb 097h,09ch,0ffh,022h,0d1h,0eeh,0d1h,0cch,0c1h,0eeh,0b1h,0ffh,0a1h,099h,091h,088h	; 7e72  ..."............
	defb 081h,077h,071h,066h,061h,077h,051h,088h,041h,099h,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh	; 7e82  .wqfawQ.A.......
	defb 0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh	; 7e92  ................
	defb 0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh,0ffh	; 7ea2  ................
	defb 0ffh,0ffh,0ffh,0ffh,0ffh	; 7eb2

; ----------------------------------------------------------------------
; DATOS relleno_final: Lo que sobra del cartucho hasta los 16 KB, que son 306
;   bytes de 0xFF... salvo LOS ULTIMOS VEINTITRES (0x7FE9-0x7FFF), que no son
;   0xFF, no son texto descifrable y no los apunta nadie. Las otras dos
;   versiones tienen ahi 0xFF hasta el final, asi que es cosa de esta. SIN
;   IDENTIFICAR
