...
ESTADO_06_CORTINILLA:		; Estado 6: borra la pantalla por columnas; sale cuando la cortinilla vuelve negativa
	call CORTINILLA		;417e   ; la cortinilla devuelve el signo: mientras sea negativo, sigue
	ret p			;4181
...
ESTADO_07_DEMO:		; Estado 7: la partida de demostracion, en tres pasos
	ld a,(0e001h)		;4185   ; 0xE001 es el paso dentro del estado, y tiene su propia tabla
	call DESPACHA		;4188
...
DEMO_0_ARRANCA:		; Paso 0: pone a cero la partida y arranca la grabacion
	call REINICIA_PARTIDA		;4191   ; la demo empieza como una partida: se reinicia todo igual
	ld hl,0e002h		;4194
...
DEMO_1_CORRE:		; Paso 1: mientras corre la demo, escribe el aviso y descuenta
	...
	ld hl,(0e0eeh)		;41b9
	dec hl			;41bc   ; un paso menos de los 1852
	ld (0e0eeh),hl		;41bd
	ld a,h			;41c0
	or l			;41c1
	ret nz			;41c2
	ld (0e133h),a		;41c3   ; al agotarse se para el reloj y se pasa de estado
	jp ESPERA_80_Y_PASO		;41c6
DEMO_2_SALE:		; Paso 2: cortinilla, y vuelta al estado 0
	call CORTINILLA		;41c9
	ret p			;41cc
	xor a			;41cd   ; el estado vuelve a 0: la demo termina donde empezo
	ld (0e000h),a		;41ce
...
MENU_0_PINTA:		; Paso 0: limpia y prepara el rotulo
	call BORRA_SPRITES		;41e2   ; el menu se monta con la pantalla limpia: sprites y nombres
...
MENU_1_ROTULO:		; Paso 1: dibuja el rotulo entero de una vez, sin repartirlo por fotogramas
	call DIBUJA_ROTULO		;41f3   ; el rotulo se dibuja poco a poco, y mientras haya acarreo se vuelve
	jr c,MENU_1_ROTULO		;41f6
	ld hl,05782h		;41f8
	call ESCRIBE_CADENA		;41fb
	ld a,006h		;41fe   ; seis parpadeos de la opcion elegida antes de entrar
...
MENU_2_PARPADEA:		; Paso 2: parpadea la linea elegida, ocho fotogramas encendida y ocho apagada
	ld hl,0e003h		;4206
	ld a,(hl)			;4209   ; ocho fotogramas por medio parpadeo: el `and 7` deja pasar uno de cada ocho...
	and 007h		;420a
	ret nz			;420c
	ld a,(hl)			;420d
	bit 3,a		;420e   ; ...y el bit 3 dice si toca encender o apagar
	jr nz,MENU_2_ENCIENDE		;4210
	ld de,03a00h		;4212   ; 0x3A00 es la fila 16 de la tabla de nombres
	ld bc,00020h		;4215   ; 0x20 casillas: la fila entera
	ld a,(0e002h)		;4218   ; Bit 4 de 0xE002: 0 joystick (fila 16), 1 teclado (fila 18)
	and 010h		;421b
	rlca			;421d   ; dos rotaciones: el bit 4 se convierte en 0x40, que son dos filas
	rlca			;421e
	call SUMA_A_DE		;421f   ; Suma 0x00 o 0x40 al destino, o sea dos filas
	ld a,001h		;4222
	call RELLENA_VRAM		;4224   ; el tile 1 es el blanco: apagar la linea es rellenarla con el
	ret			;4227
MENU_2_ENCIENDE:		; La otra mitad del parpadeo: repinta el texto
	ld hl,05782h		;4228   ; y encenderla es volver a escribir el texto
	call ESCRIBE_CADENA		;422b
	ld hl,0e18dh		;422e   ; 0xE18D cuenta los parpadeos que quedan; agotados, la espera de 0x80 y al paso siguiente
	dec (hl)			;4231   ; y un parpadeo menos
	ret nz			;4232
	jp ESPERA_80_Y_PASO		;4233
MENU_3_CORTINILLA:		; Paso 3: cortinilla y a empezar la partida
	call CORTINILLA		;4236   ; la cortinilla otra vez, y detras el reinicio de la partida
	ret p			;4239   ; el `ret p` vuelve mientras la cortinilla no acabe
...
ESTADO_09_PREPARA:		; Estado 9: saca de la tabla la distancia y el tiempo de la fase que toca
	...
	daa			;4264   ; el `daa` detras de cada resta: el tiempo va en BCD y hay que ajustarlo a mano
	ld c,a			;4265
	ld a,e			;4266
	sub c			;4267   ; y del tiempo de la fase se descuenta lo que sobro
	jr nc,PREPARA_AJUSTA		;4268
	daa			;426a
	dec d			;426b   ; la resta se lleva una centena
...
PREPARA_TIEMPO:
	...
	call PINTA_PANEL		;4274   ; el panel y la fuente se montan antes de que se vea nada
...
ESTADO_10_ENTRA:		; Estado 10: cortinilla, monta la pista y suena la musica de salida
	...
	ret p			;4285
	call MONTA_LA_FASE		;4286   ; con la cortinilla ya pasada, la fase se monta entera
	ld a,(0e002h)		;4289
	bit 6,a		;428c   ; En la demo no suena
	ld a,08ah		;428e   ; el sonido 0x8A es el del arranque de la fase
	call nz,PIDE_SONIDO		;4290
	ld a,001h		;4293
	ld (0e133h),a		;4295   ; y el reloj se echa a andar
...
ESTADO_11_PARTIDA:		; Estado 11: la partida. Un paso de juego por fotograma hasta que se acaba el tiempo o se llega a la meta
	ld a,(0e002h)		;429b   ; el bit 6 de 0xE002 distingue la partida de verdad de la demo
	bit 6,a		;429e
	...
	ld a,l			;42ab
	ld hl,0e133h		;42ac
	ld (hl),000h		;42af   ; y el reloj se para
	or a			;42b1
...
PARTIDA_ERA_DEMO:		; Si no habia partida de verdad, vuelve al paso 1 de la demo
	ld hl,00107h		;42bc   ; estado 7, paso 1: la demo se recoge por otro lado
	jr PONE_ESTADO		;42bf
ESTADO_12_TIME_OUT:		; Estado 12: se acabo el tiempo
	...
	ld de,00004h		;42c8
	ld b,004h		;42cb   ; cuatro nubes
TIME_OUT_SPRITES:
	ld (hl),0e0h		;42cd   ; la Y 0xE0 deja el sprite fuera de la pantalla
	add hl,de			;42cf
	djnz TIME_OUT_SPRITES		;42d0
	call VUELCA_ATRIBUTOS		;42d2   ; y los atributos se vuelcan ya, sin esperar al fotograma siguiente
	ld (0e0e2h),a		;42d5
	ld a,08ch		;42d8
...
ESTADO_13_FIN:		; Estado 13: espera a que acabe la musica y vuelve a la demo
	...
	or a			;42e9
	ret nz			;42ea
	ld hl,0e002h		;42eb   ; y al soltarse se apaga la partida: lo que venga despues es demo otra vez
...
META_0_FRENA:		; Paso 0: espera a que el pinguino termine de frenar
	ld hl,0e0f9h		;430d   ; 0xE0F9 es lo que le queda al salto por frenar
	ld a,(hl)			;4310   ; A cero ya estaba parado: al paso siguiente sin mas
	or a			;4311
	jp z,SIGUIENTE_PASO		;4312
	call SIGUE_SALTO		;4315   ; Un paso mas de frenada; si con el llega a cero, al paso siguiente en este mismo fotograma
	ld a,(0e0f9h)		;4318
	or a			;431b
	ret nz			;431c
...
META_1_SIGUIENTE:		; Paso 1: sube el numero de fase y guarda el tiempo que ha sobrado. Son DOS contadores distintos y conviene no mezclarlos: 0xE0E0 es el numero que se ve en el panel, en BCD y SIN TOPE -sigue subiendo a 11, 12...-, y 0xE0E1 es el indice 0-9 de la base, que DA LA VUELTA al llegar a diez. Esa pareja es la vuelta completa: el juego arranca con 0xE0E1 = 0, que es FRANCE (los valores iniciales estan en 0x4479), y cada llegada lo sube UNO, asi que se va de una base a la siguiente hasta que en la decima se vuelve al 0 y se cierra el circuito en Francia. Como el numero del panel no se reinicia, la vuelta siguiente son las mismas diez bases pero mas dificiles, que es lo que mira 0x769E
	ld hl,0e0e0h		;4320   ; 0xE0E0 es el numero de fase que se ve, en BCD, y 0xE0E1 el indice de verdad
	ld a,(hl)			;4323
	add a,001h		;4324   ; El numero que se ve, en BCD
	daa			;4326   ; el `daa` ajusta la suma, que el numero va en BCD
	ld (hl),a			;4327
	inc hl			;4328
	ld a,(hl)			;4329   ; Y el indice 0-9, que da la vuelta al llegar a diez
	ld c,a			;432a
	inc a			;432b
	cp 00ah		;432c   ; diez fases y vuelta a empezar
...
META_1_GUARDA:
	...
	ld (hl),a			;433f
	xor a			;4340
	ld (0e00dh),a		;4341   ; el aviso de meta se consume
	ld hl,0e0e8h		;4344   ; La casilla del mapa tambien avanza y da la vuelta
	inc (hl)			;4347
	ld a,(hl)			;4348
	cp 00ah		;4349   ; diez fases y vuelta a empezar
...
META_1_ANIMA:
	...
	ld h,a			;4352
	ld l,001h		;4353
	ld (0e138h),hl		;4355   ; 0xE138 arranca la animacion de la vuelta completa
	ld a,013h		;4358
...
META_2_ANDA:		; Paso 2: el pinguino sigue andando hasta la bandera
	ld c,0ffh		;4360   ; C = 0xFF: que ANDA_HASTA_LA_BASE calcule la X de destino en su primer paso
	call ANDA_HASTA_LA_BASE		;4362   ; el pinguino anda solo hasta la base; hasta que no llega, no se pasa de paso
	ret nz			;4365   ; hasta que no llega, no se pasa de paso
	ld a,00ch		;4366   ; Llegado (dieciseis pasos), el paso 3 arranca con la cuenta en 12: cuatro pasos mas de subida
	ld (0e138h),a		;4368
	jp SIGUIENTE_PASO		;436b
META_3_LLEGA:		; Paso 3: llega, se monta el decorado de la base y suena la musica
	ld c,000h		;436e   ; C = 0: el andar de vuelta no lleva prisa
	ld a,(0e079h)		;4370
	ld h,a			;4373
	call ANDA_HASTA_LA_BASE		;4374
	ret nz			;4377
	call MONTA_SPRITES_BASE		;4378   ; los sprites de la base, el dibujo y el montaje, en ese orden
	call DIBUJA_LA_BASE		;437b
	call MONTA_LA_BASE		;437e
	ld a,08fh		;4381   ; el sonido 0x8F es el de la llegada a la base
...
META_4_SALUDA:		; Paso 4: la escena de la base
	ld a,(0e01ah)		;438b   ; 0xE01A es el "lo que queda de nota" del segundo canal de sonido: la escena avanza justo cuando la nota va a acabar, al compas de la musica
	dec a			;438e   ; 0xE01A es el 'lo que queda de nota' del segundo canal: la escena avanza al compas de la musica
	ret nz			;438f
	call SUBE_LA_BANDERA		;4390
	ld a,(0e0e1h)		;4393   ; la bandera solo se sube en la fase 2
	or a			;4396
...
META_5_ESPERA:		; Paso 5: espera a que se acabe la musica
	ld a,(0e012h)		;43b5   ; no se sigue hasta que el reproductor suelta 0xE012: la musica se oye entera
	or a			;43b8
	ret nz			;43b9
	ld a,010h		;43ba
	jr ESPERA_A_Y_PASO		;43bc
META_6_BONUS:		; Paso 6: cada cuatro fotogramas cambia un segundo que sobra por 100 puntos
	ld hl,0e004h		;43be   ; 0xE004 es la espera que dejo el paso anterior: se descuenta antes de empezar a canjear
	ld a,(hl)			;43c1   ; una espera con su propio contador
	or a			;43c2
	jr z,BONUS_PASO		;43c3
	dec (hl)			;43c5   ; y mientras quede, se descuenta
	ret			;43c6
BONUS_PASO:
	ld a,(0e003h)		;43c7   ; uno de cada cuatro fotogramas: el bonus se descuenta despacio para que se vea
	and 003h		;43ca   ; los dos bits bajos: uno de cada cuatro
	ret nz			;43cc
	ld hl,(0e0e3h)		;43cd   ; Cuando el reloj llega a cero se acabo el bonus
	ld a,h			;43d0   ; los dos bytes del tiempo juntos: a cero, se acabo el bonus
	add a,l			;43d1
	jr z,ESPERA_80_Y_PASO		;43d2
	ld c,000h		;43d4
	call RESTA_UN_SEGUNDO		;43d6   ; un segundo menos de tiempo...
	ld de,00100h		;43d9   ; Cien puntos por segundo
	call SUMA_AL_MARCADOR		;43dc   ; ...y sus puntos al marcador
	ld a,001h		;43df
...
META_7_CORTINILLA:		; Paso 7: cortinilla y vuelta al estado 9 con la fase siguiente
	...
	ret p			;43e8   ; el `ret p` vuelve mientras la cortinilla no acabe
	ld a,008h		;43e9   ; el estado 8 con 0x50 fotogramas de espera
	ld (0e000h),a		;43eb
...
SIGUIENTE_ESTADO:		; Sube 0xE000 y pone el paso a cero
	ld hl,0e000h		;43f3   ; el estado sube uno y el paso vuelve a cero
	inc (hl)			;43f6
	xor a			;43f7
	ld (0e001h),a		;43f8
	ret			;43fb
ESPERA_80_Y_PASO:		; Espera 80 fotogramas y pasa al paso siguiente
	ld a,050h		;43fc   ; 0x50 fotogramas de espera y un paso mas
...
MUERTA_JUGADOR:		; Codigo muerto: escribe el numero de jugador. No la llama nadie
	...
	and 001h		;440d
	add a,031h		;440f   ; el 0x31 es el tile del "1": el numero de jugador sale de sumarle el bit 0 de 0xE002
	ld de,03933h		;4411   ; 0x3933 es la casilla donde va ese digito
...
MIRA_TECLAS_1_2:		; Empieza la partida si se pulsa 1 (joystick) o 2 (teclado)
	...
	ret nz			;4422
	ld a,050h		;4423   ; el 0x50 en el puerto 0xAA selecciona la fila del teclado, y se escribe dos veces para que se asiente
	out (0aah),a		;4425
	out (0aah),a		;4427
	in a,(0a9h)		;4429
	cpl			;442b
	and 006h		;442c   ; dos bits: las teclas 1 y 2
	ld b,040h		;442e   ; 0x40 con la tecla 1 -joystick- y 0x50 con la 2, que ademas enciende el bit del teclado
	cp 002h		;4430
...
TECLA_GUARDA:		; Apunta que tecla se ha elegido y monta el menu
	xor a			;4439   ; La tecla elegida (0 a 3) queda en los bits 4-5 de 0xE002
	ld (0e133h),a		;443a
	ld a,b			;443d
	ld (0e002h),a		;443e
	pop hl			;4441   ; el `pop hl` tira la direccion de retorno: de aqui se sale por el estado, no volviendo
	ld a,007h		;4442   ; estado 7, que es donde empieza la partida
	ld (0e000h),a		;4444
...
REINICIA_PARTIDA:		; Deja a cero el marcador y todas las variables de partida
	...
	ld de,0e044h		;444e   ; 0x100 bytes desde 0xE044 a cero: las fichas de los objetos
	ld bc,00100h		;4451
	ld (hl),000h		;4454
	ldir		;4456
	ld hl,04479h		;4458   ; Los nueve valores iniciales de 0xE0E0 (fase, tiempo, distancia...)
	ld de,0e0e0h		;445b   ; y nueve bytes mas desde 0xE0E0: fase, mapa y tiempo
	ld bc,00009h		;445e
	ldir		;4461
	ld de,00900h		;4463   ; Colores del banco 0 de la VRAM
	ld bc,00100h		;4466
	ld a,0f0h		;4469   ; el tile 0xF0 rellena la pantalla: el fondo de la fase
	call RELLENA_VRAM		;446b
	ld b,00ah		;446e   ; Los diez huecos de tiempo sobrante, uno por fase, a 5
	ld hl,0e0d5h		;4470   ; los diez huecos de tiempo sobrante vuelven a cinco segundos cada uno
REINICIA_HUECOS:
	ld (hl),005h		;4473
	inc hl			;4475
...
ARRANCA_MAQUINA:		; Registros del VDP, mezclador del PSG y VRAM a cero. Hace lo mismo que en la segunda japonesa, pero hablandole a los chips directamente
	call PONE_REGISTROS_VDP		;4482   ; Los registros del VDP, los tres volumenes del PSG y la VRAM entera: el encendido completo
	ld a,007h		;4485   ; registro 7 del PSG con 0xB8: los tres canales de tono abiertos y el ruido cerrado
	out (0a0h),a		;4487
	ld a,0b8h		;4489
	out (0a1h),a		;448b
	...
	ld de,00000h		;4493
	ld bc,04000h		;4496   ; los 16 KB de VRAM a cero de una vez
...
BORRA_NOMBRES:		; Las 768 casillas de la tabla de nombres
	ld de,03800h		;449e   ; 0x3800 son las 24 filas de la tabla de nombres, 0x300 bytes
	ld bc,00300h		;44a1
	jr VRAM_A_CERO		;44a4
APAGA_LOS_TRES_CANALES:		; Pone a cero los registros 8, 9 y 10 del PSG, que son los tres volumenes, escribiendo el puerto a pelo
	xor a			;44a6
	ld bc,003a0h		;44a7   ; 0xA0 en C es el puerto de seleccion de registro del PSG, y D arranca en el 8: los tres volumenes
	ld d,008h		;44aa   ; los registros del PSG del 8 al 10 son los tres volumenes
APAGA_CANAL:		; Registro tras registro, los tres volumenes a cero
	out (c),d		;44ac   ; El numero de registro por 0xA0 y el cero por 0xA1: escritura del PSG a pelo, sin WRTPSG
	inc d			;44ae
	out (0a1h),a		;44af
	djnz APAGA_CANAL		;44b1   ; Y detras el sonido 0x95, que es lo que suena al encender
	ld a,095h		;44b3   ; el sonido 0x95 acompana al arranque
...
PONE_REGISTROS_VDP:		; Copia los ocho registros a 0xE038 y los manda al VDP por el puerto 0x99, con el bit 0x80 del numero de registro que va sumando en D
	ld hl,044d6h		;44b9   ; 0xE038 guarda la copia en RAM de los ocho registros, que el VDP no deja leer
	ld de,0e038h		;44bc   ; los ocho registros del VDP se copian a 0xE038 y de ahi se mandan
	ld bc,00008h		;44bf
	ldir		;44c2
	ld hl,0e038h		;44c4
	ld b,008h		;44c7
	ld d,080h		;44c9   ; D arranca en 0x80: el bit alto es lo que convierte la escritura en 'a un registro'
MANDA_UN_REGISTRO:		; Manda un registro del VDP: el valor en E y el numero con el bit 0x80 en D
	ld e,(hl)			;44cb   ; El valor en E y el numero en D, que es exactamente el par que APUNTA_VRAM manda por el 0x99
	di			;44cc   ; el volcado va con las interrupciones cerradas: entre poner la direccion y escribir no puede meterse nadie
	call APUNTA_VRAM		;44cd
	ei			;44d0
	inc hl			;44d1   ; El `inc d` recorre 0x80-0x87: los ocho registros seguidos
	inc d			;44d2   ; el numero de registro sube con D, que es lo que pide el VDP
...
COPIA_A_VRAM:		; Copia BC bytes de (HL) a la VRAM DE. Apunta con el bit 6 puesto -escritura- y suelta los bytes por el puerto 0x98
	di			;44de
	set 6,d		;44df   ; Bit 6 de la direccion: es para escribir, y se quita despues para que el llamante vea DE como lo dejo
	call APUNTA_VRAM		;44e1
	res 6,d		;44e4   ; el bit 6 fuera: la direccion se pone para LEER, no para escribir
COPIA_BYTE:		; Suelta un byte por el puerto de datos del VDP
	ld a,(hl)			;44e6
	out (098h),a		;44e7   ; Al puerto de datos del VDP, sin pasar por la BIOS: la direccion ya quedo apuntada y el VDP la sube solo
	inc hl			;44e9
	dec bc			;44ea   ; El `dec bc` no toca banderas: el `ld a,b / or c` es el 'BC == 0' de un contador de 16 bits
	ld a,b			;44eb   ; el `or c` mira los dos bytes de la cuenta de una vez
	or c			;44ec
...
RELLENA_VRAM:		; Escribe el byte A en BC posiciones de la VRAM desde DE
	...
	res 6,d		;44f8
RELLENA_VRAM_BUCLE:
	ld a,h			;44fa   ; El byte se relee de H en cada vuelta, porque el `or c` del final gasta A
	out (098h),a		;44fb   ; el puerto 0x98 es el de datos de la VRAM: se escribe byte a byte sin volver a poner la direccion
	dec bc			;44fd
	ld a,b			;44fe
	or c			;44ff
...
PINTA_FRANJAS:		; Rellena tiras de la tabla de nombres a partir de una lista (largo, posicion), con el byte que va delante
	ld a,(hl)			;4504   ; el primer byte de la lista es el que se va a escribir; detras vienen las parejas de largo y posicion
	inc hl			;4505
	ld (0e0dfh),a		;4506   ; El byte con el que se rellena, que va el primero de la lista
	ld d,039h		;4509   ; 0x3900 es la tabla de nombres mas 0x100, o sea la fila 8: por ahi empiezan las franjas
FRANJAS_BUCLE:
	ld c,(hl)			;450b
	inc hl			;450c
	xor a			;450d   ; Un largo 0 cierra la lista
	cp c			;450e
	ret z			;450f
	ld b,a			;4510   ; B a cero: el largo cabe en un byte y el otro no se usa
	ld e,(hl)			;4511
	inc hl			;4512
	ld a,e			;4513
	cp 020h		;4514   ; Posicion menor de 0x20: la fila de mas abajo
	jr nc,FRANJAS_PINTA		;4516
	inc d			;4518   ; el `inc d` suma 0x100, que en la tabla de nombres son ocho filas
FRANJAS_PINTA:
	ld a,(0e0dfh)		;4519
	push hl			;451c   ; La lista y el destino se salvan alrededor del relleno, que gasta los seis registros
	push de			;451d
...
DIBUJA_BLOQUE:		; Interprete de los bloques de decorado y de pista
	...
	or a			;4526
	ret z			;4527
	and 0f0h		;4528   ; El nibble alto del primer byte es la columna
	ld c,a			;452a
	ld a,(hl)			;452b
	inc hl			;452c
	and 003h		;452d   ; El nibble bajo (0-3) mas 0x78: el byte alto de la casilla en la tabla de nombres, con el bit de escritura ya puesto
	add a,078h		;452f
	ld d,a			;4531   ; y con eso D queda hecho: el byte alto de la casilla, con el bit de escritura ya dentro
	ld a,c			;4532
BLOQUE_FILA:
	ld b,(hl)			;4533
	inc hl			;4534
	ld a,020h		;4535   ; Una fila mas abajo son 32 casillas: se suman al byte bajo y el acarreo sube el alto
	add a,c			;4537
	ld c,a			;4538
...
BLOQUE_APUNTA:
	ld a,c			;453c
	add a,b			;453d
	sub 0e0h		;453e   ; El byte que cerro la fila hace de desplazamiento de la siguiente: el `sub 0xE0` le quita la marca
	ld e,a			;4540
	call APUNTA_VRAM		;4541
BLOQUE_CASILLA:
	ld a,(hl)			;4544   ; El 0x00 cierra el bloque
	or a			;4545
	ret z			;4546   ; Y un byte de 0xE0 para arriba cierra la fila y abre la siguiente
	cp 0e0h		;4547
	jr nc,BLOQUE_FILA		;4549
	inc hl			;454b   ; Las casillas de la fila salen seguidas por el puerto de datos
	out (098h),a		;454c
...
DESCOMPRIME_SIGUE:		; El bucle
	ld a,(hl)			;455d
	inc hl			;455e
	or a			;455f   ; Un 0x80 cierra este bloque y arriba se lee otro destino
	jr z,DESCOMPRIME_FIN		;4560
	bit 7,a		;4562   ; Un 0x00 acaba del todo
	jr nz,DESCOMPRIME_TIRADA		;4564   ; Bit 7: tirada de literales; si no, repeticion
	ld b,a			;4566   ; la repeticion: B veces el MISMO byte, leido una vez por vuelta
...
DESCOMPRIME_REPITE:		; Repite el mismo byte n veces: no lo vuelve a leer, y ese es el detalle que distingue esta rama de la de al lado
	out (098h),a		;456a
	push hl			;456c   ; el `push hl / pop hl` no mueve nada: son doce ciclos de espera para que el VDP no se atragante
	pop hl			;456d
...
DESCOMPRIME_TIRADA:		; Una tirada de bytes literales, con el bit 7 quitado de la cuenta
	res 7,a		;4572   ; el bit 7 fuera y lo que queda es cuantos literales vienen detras
	ld b,a			;4574
...
LEE_Y_ESPEJA:		; Lee (HL) y, si C tiene el bit 0, le INVIERTE LOS BITS: el espejo horizontal del descompresor
	ld a,(hl)			;4580   ; el espejo se decide al entrar y vale para todo el bloque
	inc hl			;4581
	bit 0,c		;4582   ; El bit 0 de C lo fijo la entrada del descompresor: sin el, el byte sale tal cual
	ret z			;4584
	push bc			;4585   ; Ocho vueltas rotando en un sentido y metiendo por el otro: el espejo horizontal
	ld b,008h		;4586
	ld c,a			;4588
ESPEJA_BIT:		; Los ocho bits, rotando en un sentido y metiendolos en el otro
	rr c		;4589   ; `rr c` saca por abajo y `rla` mete por arriba: eso es dar la vuelta al byte
	rla			;458b
...
ESCRIBE_CADENA:		; Escribe la cadena de (HL); el destino va en los dos primeros bytes
	ld e,(hl)			;4590   ; cada cadena trae delante su destino de VRAM, dos bytes
	inc hl			;4591
	ld d,(hl)			;4592
	inc hl			;4593
ESCRIBE_CADENA_EN_DE:		; Igual, pero el destino ya viene en DE
	ld a,(hl)			;4594
	inc hl			;4595
	ld b,a			;4596   ; el byte se mira con dos `inc b` seguidos: el primero caza el 0xFF y el segundo el 0xFE
	inc b			;4597
...
REPITE_4_BYTES:		; Copia C veces los mismos cuatro bytes de (HL) a (DE): un atributo de sprite repetido
	push hl			;45a2   ; cuatro bytes por vuelta, y HL vuelve atras: el mismo grupo repetido C veces
	ld b,004h		;45a3
REPITE_4_BUCLE:
	ld a,(hl)			;45a5
	ld (de),a			;45a6
	...
	pop hl			;45ae   ; HL vuelve al principio de los cuatro bytes; DE sigue avanzando y la copia se repite
...
CORTINILLA:		; Borra la pantalla por columnas; devuelve M al terminar
	...
	ld d,038h		;45b6   ; 0x3800 es la tabla de nombres; la cortinilla se pinta ahi
	ld hl,0e004h		;45b8
	ld b,018h		;45bb   ; 0x18 filas: la pantalla entera
	bit 6,(hl)		;45bd   ; Bit 6 de 0xE004: un lado o el otro
	jr nz,CORTINILLA_DERECHA		;45bf
	ld a,01fh		;45c1   ; La columna izquierda es 0x1F menos la cuenta: las dos avanzan a la vez hacia el centro
	sub (hl)			;45c3
	ld e,a			;45c4
	set 6,(hl)		;45c5   ; el bit 6 se enciende y se apaga en vueltas alternas: una columna por lado y por fotograma
	jr CORTINILLA_COLUMNA		;45c7
CORTINILLA_DERECHA:
	res 6,(hl)		;45c9
	dec (hl)			;45cb   ; y solo se descuenta al cerrar la pareja; con el signo negativo, se acabo
	ret m			;45cc
	ld e,(hl)			;45cd
CORTINILLA_COLUMNA:
	...
	add a,e			;45d7
	ld e,a			;45d8
	dec b			;45d9   ; los dos `dec b` se comen las dos filas del panel
	dec b			;45da
CORTINILLA_BUCLE:
	xor a			;45db
	call ESCRIBE_EN_VRAM		;45dc
	ld a,020h		;45df   ; 0x20 casillas para bajar una fila
...
BORRA_SPRITES:		; Deja a cero los 128 bytes de atributos de sprite (VRAM 0x3B00), pasando por 0xE050
	ld hl,0e050h		;45e8   ; los sprites se borran en la copia de RAM y luego se vuelca entera
	push hl			;45eb
	ld b,080h		;45ec   ; 0x80 bytes son los 32 atributos de sprite
BORRA_SPRITES_BUCLE:
	ld (hl),000h		;45ee
	inc hl			;45f0
	djnz BORRA_SPRITES_BUCLE		;45f1
	ld de,03b00h		;45f3   ; Y la copia limpia entera a la tabla de atributos: 0xE050 es su espejo en RAM
	pop hl			;45f6
	ld bc,00080h		;45f7
...
PREPARA_EL_MANDO:		; Registro 15 del PSG a 0x8F, que es como se deja el puerto por el que se lee el joystick
	ld a,00fh		;45fd
	out (0a0h),a		;45ff   ; el 0xA0 selecciona el registro y el 0xA1 escribe: dos puertos, uno para elegir y otro para el dato
	ld a,08fh		;4601
	out (0a1h),a		;4603
	ret			;4605
LEE_GATILLOS_NUEVOS:		; Deja en A los gatillos que se acaban de pulsar en este fotograma
	ld a,(0e009h)		;4606   ; 0xE009 es lo pulsado ahora y 0xE008 lo de la vuelta anterior
	ld b,a			;4609
	ld a,(0e008h)		;460a
	and 030h		;460d
	cpl			;460f   ; Lo de antes negado y en AND con lo de ahora: el flanco de los bits 4-5, los gatillos recien pulsados
	ld c,a			;4610
	ld a,b			;4611
	and 030h		;4612
	and c			;4614   ; y el `and` de los dos deja solo lo que acaba de pulsarse
	ret			;4615
SUMA_AL_MARCADOR:		; Suma DE al marcador, en BCD, y actualiza el record
	...
	add a,a			;4619
	ret p			;461a
	ld hl,0e043h		;461b   ; el marcador son tres parejas BCD en 0xE043, y se suma pareja a pareja
	ld a,(hl)			;461e
	add a,e			;461f
	daa			;4620
	ld (hl),a			;4621
	ld e,a			;4622   ; Las dos parejas bajas vuelven a DE: COMPARA_RECORD las compara de golpe con el `sbc hl,de`
	inc hl			;4623
	ld a,(hl)			;4624
	adc a,d			;4625   ; la pareja de en medio va con `adc`: el acarreo viene de la de abajo
	daa			;4626
	ld (hl),a			;4627
	ld d,a			;4628
	inc hl			;4629
	...
	ld a,(hl)			;462c
	adc a,000h		;462d   ; y la de arriba igual, con su `daa`
	daa			;462f
	ld (hl),a			;4630
	jr nc,COMPARA_RECORD		;4631
	ld bc,09999h		;4633   ; Al pasarse de 999999 el record se clava en ese tope
	ld (0e040h),bc		;4636   ; los seis nueves se escriben dos veces, en 0xE040 y 0xE041, que es como se cubre la pareja de en medio
	ld (0e041h),bc		;463a
	jr PINTA_RECORD		;463e
COMPARA_RECORD:
	ld a,(0e042h)		;4640   ; el record se compara de arriba abajo, y solo se mira lo de abajo si lo de arriba empata
	ld b,(hl)			;4643
	sub (hl)			;4644   ; Byte alto contra byte alto: con acarreo hay record nuevo; empatados, deciden las parejas bajas
...
NUEVO_RECORD:
	ld (0e040h),de		;4650   ; el record nuevo son las dos parejas bajas y la alta, en ese orden
	ld a,b			;4654
	ld (0e042h),a		;4655
	jr PINTA_RECORD		;4658
CUENTA_EL_TIEMPO:		; Descuenta un segundo cada 64 fotogramas mientras 0xE133 diga que el reloj corre
	ld a,(0e133h)		;465a   ; 0xE133 apagado es el reloj parado: en el menu y en las cortinillas no corre
	or a			;465d
	ret z			;465e
	...
	ret			;466a
TIEMPO_CADA_64:
	ld a,(0e003h)		;466b   ; uno de cada 64 fotogramas, y 0xE003 sube uno por interrupcion: el "segundo" del juego dura 1,28 s en una maquina PAL y 1,07 en una NTSC, o sea que en las dos es mas largo que el de verdad
	and 03fh		;466e
	ret nz			;4670
	ld c,001h		;4671
RESTA_UN_SEGUNDO:		; Baja el reloj en uno, en BCD, y avisa con un pitido cuando quedan menos de once
	ld hl,0e0e3h		;4673   ; el reloj tambien va en BCD, dos parejas
	ld a,(hl)			;4676
	sub 001h		;4677   ; `sub 1 / daa` en vez de `dec`: el `dec` no deja el acarreo que la pareja alta BCD necesita
	daa			;4679
	ld (hl),a			;467a
	inc hl			;467b
	ld a,(hl)			;467c   ; La pareja alta solo baja si la baja pidio prestamo
...
TIEMPO_MIRA_AVISO:
	...
	push af			;468f   ; el aviso suena y la vuelta sigue: el `push` guarda las banderas que hacen falta despues
	push hl			;4690
	ld a,009h		;4691
...
PINTA_TIEMPO:		; Las cuatro cifras del reloj
	ld b,002h		;4698   ; dos parejas BCD: los cuatro digitos del reloj
	ld de,03827h		;469a
	ld hl,0e0e4h		;469d
	jp PINTA_BCD		;46a0
PINTA_PANEL:		; Pinta el panel entero: rotulos, tiempo, distancia, fase, record y marcador
	ld hl,05745h		;46a3   ; el panel se pinta entero de una vez: rotulos, reloj, distancia, fase, record y marcador
...
PINTA_RECORD:		; Las seis cifras del record
	ld hl,0e042h		;46b2   ; el record va en 0x380F, tres bytes
	ld de,0380fh		;46b5
	call PINTA_TRES_BYTES		;46b8
PINTA_MARCADOR:		; Las seis cifras del marcador
	ld de,03805h		;46bb   ; y el marcador en 0x3805
	ld hl,0e045h		;46be
PINTA_TRES_BYTES:
	ld b,003h		;46c1
	jr PINTA_BCD		;46c3
AVANZA_DISTANCIA:		; Descuenta la distancia que queda al ritmo que marca la velocidad
	...
	ret nz			;46c9
	ld a,(0e100h)		;46ca
	srl a		;46cd   ; la mitad del periodo: la distancia baja al doble de ritmo que la animacion
	dec a			;46cf
	ld (hl),a			;46d0
	ld hl,0e0e6h		;46d1   ; Distancia a cero: 0xE00D avisa de que se ha llegado a la meta
	ld a,(hl)			;46d4   ; los dos bytes de la distancia mirados de una vez con el `or`
	dec hl			;46d5
	or (hl)			;46d6
	...
	ret			;46dd
DISTANCIA_RESTA:
	ld a,(hl)			;46de   ; un metro por paso, en BCD
	sub 001h		;46df   ; Un descuento por paso, en BCD, con el prestamo subiendo al byte alto
	daa			;46e1
	ld (hl),a			;46e2
	ld c,a			;46e3   ; C se queda el byte bajo, que DISTANCIA_MIRA consulta
	inc hl			;46e4
...
DISTANCIA_MIRA:
	...
	or (hl)			;46f0   ; con el byte bajo a cero y el alto distinto, se acaba de cruzar una centena
	jr z,DISTANCIA_PINTA		;46f1
	ld a,(hl)			;46f3
	and 003h		;46f4   ; y de esas, una de cada cuatro: cada 400 metros
...
PINTA_DISTANCIA:		; Las cuatro cifras de la distancia
	ld b,002h		;46ff   ; dos parejas: los cuatro digitos de la distancia
	ld de,0382fh		;4701
	ld hl,0e0e6h		;4704
	jr PINTA_BCD		;4707
PINTA_FASE:		; Las dos cifras del numero de fase
	ld de,0381ch		;4709   ; la fase es un solo byte, dos digitos
	ld hl,0e0e0h		;470c
	ld b,001h		;470f
PINTA_BCD:		; Escribe B bytes BCD de (HL) hacia abajo en la VRAM (DE) hacia arriba, dos cifras por byte
	...
	ld c,a			;4717   ; el nibble bajo se guarda en C mientras se saca el alto
	pop af			;4718   ; El mismo byte otra vez: ahora el nibble alto, bajado con las cuatro rotaciones
	and 0f0h		;4719
	rra			;471b
	rra			;471c
	rra			;471d
	...
	inc de			;4724   ; el alto se escribe primero y el bajo detras, que en pantalla van en ese orden
	ld a,c			;4725
	call ESCRIBE_EN_VRAM		;4726
	dec hl			;4729   ; Los bytes BCD se recorren hacia abajo y la pantalla hacia arriba: van al reves
	inc de			;472a
	djnz PINTA_BCD		;472b
	ret			;472d
...
ELIGE_DECORADO:		; Segun la fase y la distancia, deja en 0xE18A y 0xE18B lo que toca dibujar a los lados de la pista
	ld a,(0e0e0h)		;472e   ; la fase, en su nibble bajo, elige la pareja de decorados
	and 00fh		;4731
	ld hl,04770h		;4733
	add a,a			;4736   ; por dos: la tabla son parejas
	call SUMA_A_HL		;4737
	ld a,(0e0e6h)		;473a
...
DECORADO_SEGUNDA:
	ld a,(hl)			;4742
	ld (0e18ah),a		;4743   ; 0xE18A se queda el byte del decorado elegido por la fase y el bit 4 de la distancia
	ld a,(0e0e0h)		;4746
	and 00fh		;4749
	ld hl,047ach		;474b
	add a,a			;474e
	call SUMA_A_HL		;474f
	ld e,(hl)			;4752   ; El puntero de la fase abre una VENTANA dentro de la tabla de veinte punteros; la distancia elige dentro de ella
	inc hl			;4753
	ld d,(hl)			;4754
	ex de,hl			;4755
	ld a,(0e0e6h)		;4756   ; La distancia, por cuartos
	and 0fch		;4759   ; los dos bits de abajo fuera y dos rotaciones: la distancia partida en cuartos de centena
	rrca			;475b
	rrca			;475c
	res 3,a		;475d
	cp 004h		;475f   ; por debajo de cuatro se coge la entrada tal cual
...
DECORADO_INDICE:
	...
	ld d,(hl)			;476a
	ex de,hl			;476b
	ld (0e18bh),hl		;476c   ; 0xE18B queda apuntando a la lista de ocho bytes que toca
	ret			;476f
...
HAY_SORPRESA:		; Mientras 0xE18E este encendido devuelve C=3, y descuenta 0xE18F hasta apagarlo
	ld a,(0e18eh)		;47e8   ; la sorpresa es la que aparece de tarde en tarde y no en cada tramo
	rra			;47eb   ; El bit 0 de 0xE18E al acarreo: apagado, no hay sorpresa
	ret nc			;47ec
	ld hl,0e18fh		;47ed
	dec (hl)			;47f0   ; 0xE18F es la vida que le queda; al agotarse, la sorpresa se apaga sola
	jr nz,SORPRESA_SI		;47f1
	xor a			;47f3
	ld (0e18eh),a		;47f4
SORPRESA_SI:
	ld c,003h		;47f7   ; C = 3 al apagarse: es lo que el llamador lee para saber que ya no esta
	ret			;47f9
MIRA_SORPRESA:		; Enciende 0xE18E en ciertos multiplos de 100 metros, con la duracion que da la tabla de al lado
	ld a,(0e0e0h)		;47fa   ; la tabla de 0x4822 da la vida de la sorpresa, una por fase
	and 00fh		;47fd
	ld hl,04822h		;47ff
	call SUMA_A_HL		;4802
	ld de,(0e0e5h)		;4805
	...
	ret nz			;480f
	ld a,(0e0e0h)		;4810
	add a,d			;4813   ; y la fase corre el reparto, asi que no salen siempre en los mismos sitios
	and 003h		;4814   ; Una de cada cuatro centenas, corrida segun la fase
	cp 002h		;4816
	ret nz			;4818
	inc a			;4819
	ld (0e18eh),a		;481a   ; encendida, con su vida cargada
	ld a,(hl)			;481d
	ld (0e18fh),a		;481e
	ret			;4821
...
PREPARA_ROTULO:		; Limpia la pantalla y pone en blanco los colores del tercio de abajo para el rotulo
	...
	ld bc,00180h		;4832
	ld a,070h		;4835   ; el 0x70 en los colores del banco 2 antes de escribir nada
	call RELLENA_VRAM		;4837
	xor a			;483a
	ld (0e00ah),a		;483b
	ld de,03966h		;483e   ; 0x13 casillas en 0x3966: el hueco del rotulo, limpio
	ld bc,00013h		;4841
...
DIBUJA_ROTULO:		; Dibuja una columna del rotulo por llamada, 23 columnas de dos casillas; devuelve C mientras queda
	ld hl,0e00ah		;4847   ; 0xE00A cuenta por que columna va el rotulo
	ld a,(hl)			;484a
	inc (hl)			;484b
	cp 017h		;484c   ; 23 columnas
	jr nc,ROTULO_ESPERA		;484e
	ld de,03885h		;4850   ; 0x3885 es la casilla por la que empieza
	ld c,a			;4853
	add a,e			;4854
	ld e,a			;4855
	ld a,c			;4856
	...
	ld c,a			;485a
	ld b,003h		;485b   ; tres escrituras por columna, una fila cada una
	xor a			;485d
ROTULO_COLUMNA:
	call ESCRIBE_EN_VRAM		;485e   ; La primera pasada escribe un 0 -la casilla vacia de encima- y las otras dos, las dos casillas del rotulo, una fila cada una
	ld a,020h		;4861   ; 0x20 casillas para bajar de fila
	call SUMA_A_DE		;4863
	ld a,c			;4866
	inc c			;4867
	djnz ROTULO_COLUMNA		;4868
	scf			;486a   ; El acarreo dice que quedan columnas
	ret			;486b
ROTULO_ESPERA:
	push af			;486c   ; y con las 23 columnas puestas, el copyright debajo
...
SUBE_LOGO:		; Dibuja el logotipo -tres filas de 3, 11 y 12 casillas- una fila mas arriba cada vez, y borra el rastro que deja debajo
	ld hl,(0e00eh)		;4879   ; 0xE00E lleva la altura a la que va el logotipo
	ld de,00020h		;487c
	...
	sbc hl,de		;4888
	ex de,hl			;488a
	ld a,044h		;488b   ; Las 26 casillas van seguidas desde la 0x44; las filas las corta el 0x0E - C de abajo: 3, 11 y 12 de ancho
	ld bc,00303h		;488d   ; tres filas y tres columnas para empezar
LOGO_FILA:
	push de			;4890
LOGO_CASILLA:
	call ESCRIBE_EN_VRAM		;4891
	inc de			;4894   ; A no se reinicia entre filas: las casillas van seguidas por todo el logotipo
	inc a			;4895
	...
	ex de,hl			;489d
	ld h,a			;489e
	ld a,00eh		;489f   ; B se recarga con 0x0E menos C: la fila de arriba lleva 3 casillas y las dos de abajo 11 y 12
	sub c			;48a1
	ld b,a			;48a2
	ld a,h			;48a3
	dec c			;48a4
...
ESCRIBE_EN_VRAM:		; Escribe el byte A en la VRAM DE: apunta con el bit de escritura y lo suelta por el puerto 0x98
	push af			;48b3   ; El byte se aparta en la pila porque A hace falta para la direccion
	set 6,d		;48b4   ; Bit 6: para escribir; y se quita para devolver DE intacto
	call APUNTA_VRAM		;48b6
	res 6,d		;48b9
	pop af			;48bb
	out (098h),a		;48bc
	ei			;48be   ; El dato por el puerto 0x98
	ret			;48bf
LEE_DE_VRAM_MUERTA:		; Lee un byte de la VRAM. Nadie la llama: el juego solo escribe en pantalla
	call APUNTA_VRAM		;48c0   ; leer es igual, pero sin encender el bit 6
	nop			;48c3   ; Los dos `nop` son la espera que el VDP pide entre la direccion y el dato; la gemela de escribir no los necesita
	nop			;48c4
	in a,(098h)		;48c5
	ei			;48c7
	ret			;48c8
APUNTA_VRAM:		; Manda DE al puerto 0x99 en dos escrituras, que es como se le dice al VDP donde va a escribir
	di			;48c9   ; Sale con las interrupciones PARADAS: las abre el que llama, cuando ha terminado con el VDP
	ld a,e			;48ca   ; Primero el byte bajo y luego el alto, los dos por el 0x99: asi se le dice al VDP donde va a escribir
	out (099h),a		;48cb
	ld a,d			;48cd
	out (099h),a		;48ce
	ret			;48d0
SUMA_A_HL:		; HL = HL + A
	add a,l			;48d1   ; suma A a HL de 16 bits, que el Z80 no tiene `add hl,a`
	ld l,a			;48d2
	ret nc			;48d3
	inc h			;48d4
	ret			;48d5
SUMA_A_DE:		; DE = DE + A
	add a,e			;48d6   ; y la misma para DE
	ld e,a			;48d7
	ret nc			;48d8
	inc d			;48d9
	ret			;48da
ESTADO_15_MAPA:		; Estado 15: el mapa, en siete pasos
	ld a,(0e001h)		;48db   ; el mapa tambien va por pasos, con su tabla
	call DESPACHA		;48de
...
MAPA_0_PREPARA:		; Paso 0: prepara los punteros y pinta de blanco los colores del mapa
	...
	ld (0e0f0h),hl		;48f8
	ld de,01080h		;48fb   ; los colores del banco 2 a 0xF4 antes de dibujar el mapa
	ld bc,00180h		;48fe
	ld a,0f4h		;4901
	call RELLENA_VRAM		;4903
	jp SIGUIENTE_PASO		;4906
MAPA_1_BORDE:		; Paso 1: la linea de arriba del marco
	ld de,03883h		;4909   ; y el borde de arriba con el tile 0x92
	ld a,092h		;490c
	jr MAPA_BORDE		;490e
MAPA_2_FILA:		; Paso 2: una fila del mapa cada dos fotogramas, hasta el 0x00 que cierra los datos
	ld a,(0e003h)		;4910   ; una fila cada dos fotogramas: el mapa se dibuja a la vista
	rra			;4913
	ret c			;4914
	ld hl,(0e0f0h)		;4915
	...
	pop de			;492a
	inc de			;492b   ; y por dentro el 4, 0x16 casillas: el marco deja una a cada lado
	ld a,004h		;492c
	ld c,016h		;492e
...
MAPA_3_BORDE:		; Paso 3: la linea de abajo del marco
	ld de,03aa3h		;4947   ; el borde de abajo: esquina, tramo de 24 y la otra esquina
	ld a,091h		;494a
MAPA_BORDE:
	call ESCRIBE_EN_VRAM		;494c
	inc de			;494f
	ld bc,00018h		;4950
	add a,004h		;4953   ; Tres casillas: la esquina (A), el tramo (A + 4, 24 veces) y la otra esquina (A + 2)
	push af			;4955
...
MAPA_5_TRAZA:		; Paso 5: un paso del recorrido cada dos fotogramas
	ld a,(0e003h)		;496e   ; tambien uno de cada dos fotogramas
	rra			;4971
	ret c			;4972
	ld hl,0e0f6h		;4973
	ld a,(hl)			;4976
...
TRAZO_PARTE:
	...
	scf			;49af   ; el acarreo dice que quedan pasos por trazar
	ret			;49b0
...
PASO_ARRIBA:		; Una fila menos: -0x20
	ld a,0e0h		;49b2   ; arriba es restar 0x20, o sea sumar 0xE0
	jr PASO_ARRIBA_SUMA		;49b4
PASO_DERECHA:		; Una casilla mas
	ld a,001h		;49b6   ; y a la derecha, una casilla
	jr PASO_SUMA		;49b8
...
MAPA_6_ESPERA:		; Paso 6: espera y pone 0xE000 a 9, pero ESPERA_80_Y_ESTADO lo sube: cae en el estado 10, el que monta la fase (el 9 ya corrio antes del mapa)
	...
	dec (hl)			;49c8
	ret nz			;49c9
	ld a,009h		;49ca   ; El 9 que ESPERA_80_Y_ESTADO subira a 10
	ld (0e000h),a		;49cc
...
MONTA_LA_FASE:		; Prepara la fase entera: borra las variables de pista, carga los graficos y monta el decorado
	...
	ld de,0e0f1h		;4ad5
	ld bc,00130h		;4ad8
	ld (hl),000h		;4adb   ; el `ld (hl),0` mas el `ldir` es el borrado clasico: se siembra el primero y se arrastra
	ldir		;4add
	ld a,010h		;4adf
	ld h,a			;4ae1
	ld l,a			;4ae2
	ld (0e100h),hl		;4ae3   ; Periodo inicial 0x10, cerca del tope lento: cada fase arranca despacio
	ld (0e110h),a		;4ae6   ; 0xE110 es el otro periodo, el del decorado
	ld a,008h		;4ae9   ; 0xE149 arranca en 8
	ld (0e149h),a		;4aeb
	ld a,005h		;4aee
	ld (0e0e9h),a		;4af0   ; y 0xE0E9 en 5, que es lo que tarda el primer descuento de distancia
	ld hl,03030h		;4af3   ; Segun sea la fase par o impar, una pareja de casillas u otra
	ld a,(0e0e0h)		;4af6   ; el bit 0 de la fase: par o impar
	rra			;4af9
...
FASE_CASILLAS:
	...
	xor a			;4b16
	ld (0e13bh),a		;4b17   ; montada la fase, se levanta el cerrojo
	ret			;4b1a
...
PASO_DE_PARTIDA:		; Todo lo que pasa en un fotograma de juego
	call PINTA_VELOCIMETRO		;4b1b   ; el paso de partida empieza por lo que se ve: el velocimetro y el pez
	...
	call AJUSTA_DIFICULTAD		;4b2f   ; la dificultad se ajusta antes de leer el mando
	call MUEVE_PINGUINO		;4b32   ; Los mandos
	call ARRASTRA_PINGUINO		;4b35
	call MIRA_EL_PEZ		;4b38   ; El pez
	call MIRA_EL_BORDE		;4b3b   ; y el borde se mira despues de moverse, no antes
	ld a,(0e140h)		;4b3e   ; si se ha caido al agua, la pista ya no avanza en este paso
	or a			;4b41
	ret nz			;4b42
	...
	call CREA_OBSTACULO		;4b4f   ; y por ultimo los obstaculos nuevos y las nubes
	jp LAS_NUBES		;4b52
...
MUEVE_PINGUINO:		; Lee los mandos y mueve al pinguino, o sigue el salto si ya estaba saltando
	...
	ld a,b			;4b8b   ; B trae el mando: en el salto solo importan los bits de los lados
...
PINGUINO_A_VRAM:		; Vuelca los cuatro atributos del pinguino a la VRAM
	...
	ld bc,00010h		;4b9d   ; 0x10 bytes: cuatro atributos de cuatro
...
PONE_POSE:		; Copia los cuatro patrones de la pose A a los atributos, saltando de cuatro en cuatro
	exx			;4ba6   ; la pose se escribe en el juego alterno para no gastar los registros del llamador
	ld hl,04b55h		;4ba7   ; la tabla de 0x4B55 son los cuatro patrones de cada pose
	call SUMA_A_HL		;4baa
	ld de,0e07ah		;4bad
	ld b,004h		;4bb0
POSE_BUCLE:
	ld a,(hl)			;4bb2
	ld (de),a			;4bb3
	ld a,004h		;4bb4   ; De cuatro en cuatro: el campo de patron de cada uno de los cuatro atributos
	add a,e			;4bb6
	ld e,a			;4bb7
	inc hl			;4bb8
...
COLOCA_SPRITES:		; Reparte HL (Y en L, X en H) por los cuatro sprites, sumando 16 a la derecha y abajo
	ld d,h			;4bbd   ; El pinguino son cuatro sprites de 16x16 en cuadro: 0xE078, 0xE07C, 0xE080 y 0xE084
	ld (0e078h),hl		;4bbe
	ld a,h			;4bc1
	add a,010h		;4bc2   ; la pareja de la derecha, 16 pixeles mas alla
	ld h,a			;4bc4
	ld (0e07ch),hl		;4bc5
	ld a,l			;4bc8
	add a,010h		;4bc9   ; Con la Y bajada 16, la pareja de abajo
	ld l,a			;4bcb
	ld e,a			;4bcc
	ld (0e080h),de		;4bcd
	ld (0e084h),hl		;4bd1
...
EMPIEZA_SALTO:		; Gatillo: suena el salto y se apunta hacia donde va
	...
	ld a,b			;4bda   ; el mando se mira una sola vez, al arrancar el salto: lo que se pulse despues ya no cambia el rumbo
...
SIGUE_SALTO:		; Un paso de salto cada cuatro fotogramas
	ld a,(0e003h)		;4be9   ; el salto avanza uno de cada cuatro fotogramas
	and 003h		;4bec
	ret nz			;4bee
SALTO_PASO:
	ld a,(hl)			;4bef
	inc (hl)			;4bf0   ; el paso sube y el valor leido es el de antes
	cp 00bh		;4bf1   ; Once pasos y vuelta a cero
	jr nz,SALTO_POSE		;4bf3
	ld (hl),000h		;4bf5
SALTO_POSE:
	push af			;4bf7
	ld c,000h		;4bf8   ; Tres poses: al rematar (paso 11) la de parado (0); durante el salto alternan la 0x10 y la 0x0C con el bit 0 del paso
	cp 00bh		;4bfa
...
SALTO_COLOCA:
	...
	call SUMA_A_HL		;4c0d
	ld a,(hl)			;4c10
	ld de,(0e078h)		;4c11   ; la altura se le suma a la Y del pinguino
	add a,e			;4c15
	ld e,a			;4c16
	ld hl,0e0fbh		;4c17
	ld a,(hl)			;4c1a
...
SALTO_TERMINA:
	...
	ld a,(0e0f9h)		;4c25   ; si el salto se acabo en este paso, no se miran choques
	or a			;4c28
	ret nz			;4c29
	call MIRA_CHOQUES		;4c2a   ; La sombra
	ld a,(0e140h)		;4c2d   ; ni en el agua ni cayendose se puntua el salto
	ld hl,0e142h		;4c30
	add a,(hl)			;4c33
	ret nz			;4c34
	ld hl,0e132h		;4c35   ; 0xE132 evita cobrar el mismo salto dos veces
	cp (hl)			;4c38
	ret z			;4c39
	ld (hl),a			;4c3a
...
SALTO_A_LA_IZQUIERDA:
	call MUEVE_IZQUIERDA		;4c41   ; el salto lateral mueve DOS casillas, no una: por eso las dos llamadas seguidas
...
MUEVE_IZQUIERDA:		; Una columna a la izquierda; el borde esta en X=0x14
	ld a,d			;4c6d   ; con la columna por debajo de 0x14 no se puede ir mas a la izquierda
	cp 014h		;4c6e
	ret c			;4c70
	dec d			;4c71
	set 0,(hl)		;4c72   ; Los bits 0 y 1 de 0xE0FA apuntan el ultimo sentido: izquierda enciende el 0 y apaga el 1
	res 1,(hl)		;4c74
	ret			;4c76
MANTIENE_SENTIDO:		; Con las dos direcciones metidas, la primera vez INVIERTE el sentido que llevaba y lo deja apuntado (bit 7 de 0xE0FA); las siguientes repite el nuevo
	ld a,(hl)			;4c77   ; 0xE0FA a cero: no habia sentido anterior que mantener
	or a			;4c78
	ret z			;4c79
	bit 7,a		;4c7a   ; El bit 7 marca el empate ya resuelto: la primera vez se invierte el sentido (bit 1: venia de la derecha) y las siguientes se repite el nuevo (bit 0)
...
MUEVE_DERECHA:		; Una columna a la derecha; el borde esta en X=0xCC
	ld a,d			;4c8a
	cp 0cch		;4c8b   ; y por la derecha el tope es 0xCC
	ret nc			;4c8d
	set 1,(hl)		;4c8e   ; El espejo de la izquierda: enciende el bit 1 y apaga el 0
	res 0,(hl)		;4c90
	inc d			;4c92
	ret			;4c93
ANIMA_ANDAR:		; Las tres poses de andar, una cada ocho fotogramas. La llama la interrupcion, no el paso de partida
	...
	or (hl)			;4c9a
	ret nz			;4c9b
	ld a,(0e003h)		;4c9c   ; uno de cada ocho fotogramas: una zancada
	and 007h		;4c9f
	ret nz			;4ca1
ANDAR_PASO:
	ld hl,0e0f8h		;4ca2
	inc (hl)			;4ca5   ; 0xE0F8 cuenta las zancadas
	ld a,(hl)			;4ca6
	ld c,000h		;4ca7   ; Los bits 0-1 de la cuenta reparten el ciclo de poses 0, 4, 0, 8: el paso central entre cada zancada
	rra			;4ca9
...
COLOCA_SOMBRA:		; Los dos sprites de debajo del pinguino; en el salto y en la caida las dos mitades se MONTAN una sobre otra y la sombra se estrecha cuanto mas alto va
	ld hl,(0e078h)		;4cba
	ld a,l			;4cbd   ; la sombra va 0x1E pixeles mas abajo y 0x10 a la derecha del pinguino
	add a,01eh		;4cbe
	ld l,a			;4cc0
	ld c,a			;4cc1
	ld a,h			;4cc2
...
SOMBRA_SEPARA:
	ex de,hl			;4cd8   ; la tabla se recorre con el paso de la caida
	call SUMA_A_HL		;4cd9
	ld l,(hl)			;4cdc
	ld a,d			;4cdd   ; La separacion se SUMA a la mitad izquierda y se RESTA a la derecha: las medias sombras se montan y la sombra se estrecha
	add a,l			;4cde
	ld d,a			;4cdf
	ld a,b			;4ce0
	sub l			;4ce1
	ld b,a			;4ce2
	ld e,0aeh		;4ce3   ; La Y de la sombra se clava en 0xAE, el suelo: no acompana al pinguino
	ld c,0aeh		;4ce5
	ex de,hl			;4ce7
SOMBRA_GUARDA:
	ld (0e0a0h),hl		;4ce8   ; las dos mitades de la sombra van a 0xE0A0 y 0xE0A4
	ld (0e0a4h),bc		;4ceb
SOMBRA_A_VRAM:
	ld hl,0e0a0h		;4cef
	ld de,03b50h		;4cf2   ; Sprites 20 y 21
	ld bc,00008h		;4cf5   ; ocho bytes: los dos atributos de la sombra
...
MIRA_CHOQUES:		; Con el pinguino en el suelo: recorre las fichas y mira si alguna esta en el paso 13 a su altura
	ld a,(0e0f9h)		;4d1a   ; saltando no se choca con nada: se pasa por encima
	or a			;4d1d
	ret nz			;4d1e
	ld b,004h		;4d1f   ; cuatro fichas de obstaculo, cinco a partir de la fase 5
...
CHOQUES_FICHA:
	...
	ld d,(hl)			;4d3a
	ex de,hl			;4d3b
	dec a			;4d3c   ; el tipo se compara con 5: los de mas arriba son objetos que se cogen
	cp c			;4d3d
...
CHOQUES_AGUJERO:
	ld c,(hl)			;4d4b   ; el hueco tiene dos zonas: el agua por dentro y el borde por fuera
	dec c			;4d4c
...
CHOQUES_NADA:
	...
	xor a			;4d5f
CHOQUES_SIGUIENTE:
	inc a			;4d60   ; seis bytes de una ficha a la siguiente
	call SUMA_A_HL		;4d61
	djnz CHOQUES_FICHA		;4d64
	ret			;4d66
MIRA_CHOQUES_SALTANDO:		; Lo mismo, pero en el aire: solo mira una lista propia, la de 0x4D99
	ld a,(0e0f9h)		;4d67   ; esta es la gemela de arriba para cuando SI se esta saltando
	or a			;4d6a
	ret z			;4d6b
	ld b,005h		;4d6c   ; aqui siempre son cinco fichas
	ld hl,0e112h		;4d6e
SALTANDO_FICHA:
	ld a,(hl)			;4d71
	inc hl			;4d72
	cp 00dh		;4d73   ; Solo las fichas que van por el paso 0x0D
	ld a,005h		;4d75   ; El 5 mas el `inc hl` de arriba: seis bytes por ficha
	jr nz,SALTANDO_SIGUIENTE		;4d77
	ex de,hl			;4d79
	ld a,(de)			;4d7a
	cp 005h		;4d7b   ; el `cp 5` y el `add a,a` juntos: el tipo por dos, que la tabla son parejas
	add a,a			;4d7d
	ld hl,04d99h		;4d7e
	call SUMA_A_HL		;4d81
	ld a,(0e079h)		;4d84   ; La X del pinguino contra el par (posicion, ancho): dentro del ancho, se ha saltado por encima
	sub (hl)			;4d87
	inc hl			;4d88
	cp (hl)			;4d89
	...
	ex de,hl			;4d8c
SALTANDO_SIGUIENTE:
	call SUMA_A_HL		;4d8d   ; y a la ficha siguiente
...
MIRA_EL_PEZ:		; Si el pinguino pisa el pez, suena, se lo lleva y suma 300 puntos
	ld a,(0e142h)		;4da3   ; ni en el agua ni cayendose se puede coger el pez
	ld hl,0e140h		;4da6
	add a,(hl)			;4da9
	ret nz			;4daa
	ld de,(0e188h)		;4dab   ; 0xE188 es donde esta el pez
	ld a,e			;4daf
	...
	ret nc			;4dba
	ld a,013h		;4dbb   ; la caja del pez no es un rectangulo: el ancho depende de la diferencia de altura, asi que es un rombo
	add a,e			;4dbd
	ld l,a			;4dbe
	ld a,e			;4dbf
	add a,a			;4dc0
	...
	ld hl,0e08ch		;4dce   ; cogido el pez, se le quita de la pantalla y se avanza su ciclo
	ld de,0e183h		;4dd1
...
MIRA_EL_BORDE:		; Choque con lo que haya en 0xE090 cuando esta abajo del todo
	ld hl,(0e090h)		;4de0   ; la foca solo pilla cuando esta asomada del todo
	ld a,l			;4de3
	cp 08fh		;4de4   ; Solo con la Y de 0xE090 en 0x8F: la foca asomada del todo
	ret nz			;4de6
	ld a,(0e079h)		;4de7
	ld l,a			;4dea
	ld a,h			;4deb
	sub l			;4dec
	push af			;4ded
	sub 018h		;4dee   ; La pareja `sub`/`add` deja el acarreo con la diferencia de X entre -11 y +23: el ancho del choque
	add a,023h		;4df0
	jp c,CHOCA		;4df2
	pop af			;4df5
	ret			;4df6
TROPIEZA:		; El pinguino tropieza y rueda hacia el lado por el que iba
	...
	or a			;4dfa
	ret nz			;4dfb
	ld a,003h		;4dfc   ; el sonido 3 es el tropiezo
	call PIDE_SONIDO		;4dfe   ; Sonido 3
	ld hl,00101h		;4e01   ; el bit 0 de 0xE0FA -el ultimo sentido- decide hacia donde se cae
	ld a,(0e0fah)		;4e04
	cpl			;4e07
	rra			;4e08
	jr PONE_CAIDA		;4e09
CHOCA:		; Choque de frente: se cae hacia atras
	ld hl,00101h		;4e0b   ; y esta es la caida al agua, con su sonido 8
	ld (0e136h),hl		;4e0e
	ld a,008h		;4e11
	call PIDE_SONIDO		;4e13   ; Sonido 8
	ld hl,00102h		;4e16
	ld a,(0e0f9h)		;4e19   ; cayendo desde un salto la cuenta empieza un paso mas alla
	or a			;4e1c
...
SIGUE_CAIDA:		; Un paso de caida cada cuatro fotogramas: rueda de lado y baja
	ld a,(0e003h)		;4e3a   ; la caida avanza uno de cada cuatro fotogramas, como el salto
	and 003h		;4e3d
	ret nz			;4e3f
	ld hl,0e142h		;4e40
	ld a,(hl)			;4e43
	...
	inc hl			;4e49
	ld a,(hl)			;4e4a   ; y su propio contador de paso
	inc (hl)			;4e4b
	ld hl,RET_COMPARTIDO		;4e4c   ; El desplazamiento de este paso, apuntado un byte antes
	call SUMA_A_HL		;4e4f
	ld c,(hl)			;4e52   ; C se queda el desplazamiento de este paso
	ld de,(0e078h)		;4e53
CAIDA_LADO:
	ld hl,0e0d0h		;4e57
	ld a,(0e144h)		;4e5a
	...
	ld hl,0e142h		;4e65   ; Mientras 0xE142 pase de 1, otra tanda: el tropiezo entra con 1, el choque con 2 y el choque saltando con 3 tandas de tres columnas
	ld a,(hl)			;4e68
	dec a			;4e69
...
CAIDA_COLOCA:
	ex de,hl			;4e6f
	ld a,l			;4e70   ; el desplazamiento se le suma a la Y y se recolocan los cuatro sprites
	add a,c			;4e71
	ld l,a			;4e72
...
CAIDA_ABAJO:
	...
	call PISTA_GRUPO_B		;4e82   ; al tocar el suelo se cambia de grupo de pista y se pasa a la siguiente
	call PISTA_SIGUIENTE		;4e85
	xor a			;4e88
	ld b,a			;4e89
	ld hl,0e136h		;4e8a   ; Si 0xE136 (el choque de frente) estaba puesto, se apaga y el repintado corre con 0xE135 levantado
	cp (hl)			;4e8d
	...
	ld (0e135h),a		;4e92
CAIDA_REPINTA:
	call MUEVE_OBSTACULOS		;4e95   ; los obstaculos se mueven con el repintado especial levantado
	xor a			;4e98
	ld (0e135h),a		;4e99
CAIDA_CUENTA:
	ld hl,0e143h		;4e9c
	...
	ret nz			;4ea2
	ld (hl),a			;4ea3
	dec hl			;4ea4   ; los tres contadores de la caida a cero
	ld (hl),a			;4ea5
	ld (0e137h),a		;4ea6
	ret			;4ea9
TRES_A_LA_DERECHA:		; Tres columnas de golpe
	call MUEVE_DERECHA		;4eaa   ; tres a la derecha de una vez: la caida arrastra tres columnas por tanda
	call MUEVE_DERECHA		;4ead
	jp MUEVE_DERECHA		;4eb0
TRES_A_LA_IZQUIERDA:
	call MUEVE_IZQUIERDA		;4eb3   ; y su espejo, tres a la izquierda
...
CAIDA_TERCER_PASO:		; El tercer paso de la caida, que ya es levantarse
	ld hl,0e0f9h		;4ed2   ; levantarse son once pasos, contados con el mismo byte que el salto
	ld a,(hl)			;4ed5   ; 0xE0F9 vuelve a contar como en el salto: once pasos de levantarse
	inc (hl)			;4ed6
	cp 00bh		;4ed7
	jr nz,CAIDA_LEVANTA		;4ed9
	ld (hl),000h		;4edb   ; y a los once vuelve a cero
CAIDA_LEVANTA:
	push af			;4edd
	ld a,(0e144h)		;4ede   ; la pose de levantarse sale de 0xE144, el lado por el que se cayo
	ld c,a			;4ee1
	call PONE_POSE		;4ee2
	pop af			;4ee5
	...
	add a,e			;4ef1
	ld e,a			;4ef2
	bit 2,c		;4ef3   ; El bit 2 de la pose (en C) dice el lado: se sigue rodando tres columnas por paso mientras se levanta
	ld hl,0e0d0h		;4ef5
	...
	or a			;4f05
	ret nz			;4f06
	ld a,001h		;4f07   ; el remate levanta 0xE135 para que el repintado sepa que es especial
	ld (0e135h),a		;4f09
	call CAIDA_ABAJO		;4f0c   ; El remate reutiliza CAIDA_ABAJO para repintar la fila y los obstaculos donde ha quedado
	xor a			;4f0f
	ld (0e135h),a		;4f10
	dec hl			;4f13   ; y deja la cuenta de la caida como se la encontro
	inc a			;4f14
	ld (hl),a			;4f15
	ld a,004h		;4f16
...
CAE_AL_AGUA:		; Se cae por el agujero: ocho sprites y a esperar
	ld hl,00001h		;4f1c   ; caer al agua enciende 0xE140 y apaga la caida
	ld (0e140h),hl		;4f1f   ; 0xE140: esta en el agua
	xor a			;4f22
	ld (0e142h),a		;4f23
	ld a,0ffh		;4f26   ; 0xE0F8 a 0xFF: el ciclo de zancadas se reinicia al salir
	ld (0e0f8h),a		;4f28
	ld a,005h		;4f2b
	call PIDE_SONIDO		;4f2d   ; Sonido 5
	ld hl,0e068h		;4f30
	ld bc,004b6h		;4f33   ; los cuatro sprites del chapoteo, de cuatro en cuatro bytes
AGUA_SPRITES:
	ld (hl),c			;4f36   ; el 0xB6 es el primer patron del chapoteo
	ld a,004h		;4f37
	call SUMA_A_HL		;4f39
	djnz AGUA_SPRITES		;4f3c   ; cuatro sprites
...
DIBUJA_EN_EL_AGUA:		; Coloca al pinguino asomando por el agujero, en Y=0x9F, y le PONE LAS PATAS AMARILLAS
	ld hl,(0e078h)		;4f3e
	ld l,09fh		;4f41   ; la Y se clava en 0x9F: la altura del agua
	call COLOCA_SPRITES		;4f43
	ld a,010h		;4f46
	call PONE_POSE		;4f48
	ld a,0e0h		;4f4b   ; la Y 0xE0 deja el sprite fuera de la pantalla
...
VUELCA_OCHO_SPRITES:		; Los ocho atributos de 0xE068 a la VRAM, sprites 6 a 13
	ld hl,0e068h		;4f56
	ld de,03b18h		;4f59   ; 0x3B18 son los sprites 6 a 13
	ld bc,00020h		;4f5c   ; 0x20 bytes: los ocho atributos
	call COPIA_A_VRAM		;4f5f
	jp SOMBRA_A_VRAM		;4f62
SIGUE_EN_EL_AGUA:		; Manotea hasta que se pulsa el gatillo
	ld hl,0e141h		;4f65   ; 0xE141 cuenta lo que se lleva en el agua
	inc (hl)			;4f68
	res 7,(hl)		;4f69   ; el bit 7 se apaga cada vuelta: la cuenta no pasa de 0x7F
	ld a,(hl)			;4f6b
...
CHAPUZON_CADA_OCHO:		; Uno de cada ocho fotogramas mientras se manotea en el agua
	...
	bit 3,c		;4f83   ; los bits 3 y 4 del contador reparten los tres dibujos
...
AGUA_ANIMA:		; Mueve las patas: el patron va cambiando entre 0x70, 0x74 y 0x78, y la posicion los sigue
	ld hl,(0e078h)		;4f97   ; A trae el ajuste de X, B la Y del chapoteo y DE la pareja pose (D) y patron de las patas (E)
	ld l,b			;4f9a
	add a,h			;4f9b
	ld c,a			;4f9c
	ld a,b			;4f9d
	ld b,e			;4f9e
	ld (0e0a1h),bc		;4f9f
	add a,010h		;4fa3   ; La Y del sprite de las patas queda 16 por debajo del cuerpo, que sube y baja con el manoteo
	ld (0e0a0h),a		;4fa5
	push de			;4fa8
	call COLOCA_SPRITES		;4fa9   ; los cuatro sprites del pinguino se recolocan con la Y del manoteo
	pop af			;4fac
	call PONE_POSE		;4fad
...
SALE_DEL_AGUA:		; Con el gatillo se sale, y el periodo vuelve a 0x13: se sale del agua a la minima velocidad
	xor a			;4fb3   ; salir del agua apaga las dos banderas y reinicia las zancadas
	ld (0e140h),a		;4fb4
	ld (0e0f8h),a		;4fb7
	ld hl,00313h		;4fba   ; 0xE100 vuelve al periodo 0x13: del agua se sale a la minima velocidad
	ld (0e100h),hl		;4fbd
	ld a,(0e079h)		;4fc0
	push af			;4fc3
	ld hl,06699h		;4fc4   ; El atributo de 0x6699, cuatro veces sobre los sprites 6 a 9: los que encendio el agua se recogen
	ld de,0e068h		;4fc7
	ld c,004h		;4fca   ; cuatro sprites con el mismo atributo
	call REPITE_4_BYTES		;4fcc
	ld b,004h		;4fcf   ; cuatro juegos mas
SALIDA_SPRITES:
	ld c,(hl)			;4fd1   ; La lista sigue con cuatro juegos mas: un byte de cuantas veces y los cuatro del atributo
	inc hl			;4fd2
	push bc			;4fd3
	call REPITE_4_BYTES		;4fd4
	pop bc			;4fd7
	djnz SALIDA_SPRITES		;4fd8   ; tantos juegos como diga B
	pop hl			;4fda
	ld l,090h		;4fdb   ; y el pinguino vuelve a la Y de andar, 0x90
	call COLOCA_SPRITES		;4fdd
	ld hl,004a0h		;4fe0   ; El patron 0xA0 y el color 4: la sombra vuelve a ser sombra
	ld (0e0a2h),hl		;4fe3
...
COGE_OBJETO:		; LAS BANDERAS DE LA PISTA. Los tipos 5 y 6 son las dos banderas que hay plantadas en el hielo -una inclinada a cada lado- y NO se esquivan: se recogen. Al tocarlas suena, se borra la ficha, se repinta el hueco y suman 500 puntos. Lo dice tambien el reparto de 0x4D3C: los tipos 0 a 4 se van al choque y solo el 5 y el 6 caen aqui. OJO, no confundirlas con la bandera que sube por el mastil de la base, que es otra cosa y va por sprites (0x55AD)
	ex de,hl			;4fed   ; el dibujo se lee hacia ATRAS: los bytes que van delante del puntero
	dec hl			;4fee
	dec hl			;4fef
	ld d,(hl)			;4ff0
	dec hl			;4ff1
	...
	ld hl,0e1a0h		;5008
	call DIBUJA_BLOQUE		;500b   ; y de ahi lo pinta el interprete de bloques normal
...
MONTA_LA_PISTA:		; Monta la pista de la fase: colores, cielo, hielo, decorados y la lista de lo que va saliendo
	...
	call SUMA_A_HL		;501b
	ld a,007h		;501e
	bit 0,(hl)		;5020   ; el bit 0 elige entre la casilla 7 y la 9 para el cielo
...
PISTA_COLORES:
	...
	ld de,0620ch		;5036
PISTA_DESCOMPRIME:
	push de			;5039   ; Las dos tiradas del descompresor -a 0x4588 y a 0x4F78- van con el juego de flujos del tipo de fase
	ld de,04588h		;503a   ; y se descomprimen en dos sitios: la tabla de colores y los patrones
	...
	ld bc,000e0h		;504a   ; 0xE0 casillas: siete filas de cielo
	ld a,(0e10ch)		;504d
...
SIGUIENTE_DECORADO:		; Coge el siguiente decorado de la lista de la fase; 0xFF quiere decir que ya no hay mas
	...
	ld hl,(0e10ah)		;5097
	call SUMA_A_HL		;509a
	ld a,(hl)			;509d   ; El 0xFF de la lista: ya no hay mas decorados en esta fase
	cp 0ffh		;509e
	ret z			;50a0
	ld (0e109h),a		;50a1
	ld bc,0e103h		;50a4   ; El bit 0 del decorado elige la pareja de punteros: 0xE103 (grupo A) o 0xE105 (grupo B)
	bit 0,a		;50a7
...
DECORADO_GRUPO:
	...
	ld a,(hl)			;50b4
	ld e,a			;50b5
	ld (bc),a			;50b6   ; El puntero elegido se apunta en la pareja y queda tambien en DE, que se dibuja ya
	inc hl			;50b7
	inc bc			;50b8
	ld a,(hl)			;50b9
	ld d,a			;50ba
	ld (bc),a			;50bb
	ex de,hl			;50bc
	ld a,008h		;50bd   ; Los ocho primeros bytes del grupo son sus cuatro punteros de trozos; el dibujo empieza en el byte 8
	call SUMA_A_HL		;50bf
...
PINTA_FILA_DE_PISTA:		; Escribe la fila compuesta en 0xE14E, que va siempre a la fila 9
	ld de,03920h		;50db   ; La cabecera 0x3920 (la fila 9) y el 0xFF de cierre arropan la fila compuesta, y ESCRIBE_CADENA la lleva a pantalla
	ld (0e14eh),de		;50de
	ld a,0ffh		;50e2
	ld (0e170h),a		;50e4
	ld hl,0e14eh		;50e7
...
AVANZA_DECORADO:		; Cada 400 metros toca decorado nuevo
	...
	ld hl,0e107h		;50f2   ; 0xE107 lo enciende el marcador de la distancia (0x46F9)
	ld a,(hl)			;50f5
	dec a			;50f6
	ret nz			;50f7
	ld a,(0e102h)		;50f8   ; Y solo con el contador de trozos en 1, para no pisar un trozo a medias
	dec a			;50fb
	ret nz			;50fc
	ld (hl),a			;50fd   ; el aviso se consume al atenderlo
	call SIGUIENTE_DECORADO		;50fe
	or a			;5101
	ret nz			;5102
	ld hl,(0e103h)		;5103
	ld a,(0e109h)		;5106
	bit 0,a		;5109   ; el bit 0 de 0xE109 elige entre los dos decorados apuntados
...
AVANZA_LA_PISTA:		; Al ritmo de la velocidad, va sacando los trozos de decorado y moviendo los obstaculos
	ld hl,0e100h		;514b   ; 0xE100 es el periodo y 0xE101 su cuenta atras: los trabajos del dibujo se reparten dentro del periodo (3 el decorado, 1 el grupo B, 0 la recarga y el grupo A)
	ld c,(hl)			;514e
	inc hl			;514f
	dec (hl)			;5150
...
PISTA_DIBUJA:		; Dibuja el trozo A de la tabla que apunta HL
	add a,a			;516f   ; A por dos: la tabla del grupo es de punteros y el trozo se dibuja con el interprete de bloques
	call SUMA_A_HL		;5170
	ld e,(hl)			;5173
	inc hl			;5174
	ld d,(hl)			;5175
...
PISTA_MIRA:
	...
	inc b			;5180
	srl c		;5181   ; A media cuenta (periodo/2 contra la cuenta) se entra con B = 1
	ld a,(hl)			;5183
	cp c			;5184
	ret nz			;5185
...
OBSTACULO_FICHA:
	...
	ld a,(hl)			;5198
	cp 00bh		;5199   ; A media cuenta solo avanzan las fichas del paso 0x0B en adelante: lo cercano corre el doble, que es la perspectiva
	ld a,006h		;519b
...
OBSTACULO_DIBUJA:
	...
	push de			;51b3
	push bc			;51b4
	call DIBUJA_BLOQUE		;51b5   ; el bloque se pinta antes de avanzar el puntero
	pop bc			;51b8
	pop de			;51b9
	inc hl			;51ba   ; El puntero de dibujo se guarda avanzado: cada paso dibuja el trozo siguiente de la secuencia
	ex de,hl			;51bb
	ld (hl),d			;51bc
	dec hl			;51bd
	ld (hl),e			;51be
	ld a,004h		;51bf   ; cuatro bytes por ficha de obstaculo
OBSTACULO_SIGUIENTE:
	call SUMA_A_HL		;51c1
	djnz OBSTACULO_FICHA		;51c4
	call SUELTA_EL_PEZ		;51c6   ; Detras del barrido van el pez, la foca y los dos choques: una vez por paso de pista
...
CREA_CUENTA:
	...
	dec (hl)			;51e6
	ret nz			;51e7
	ld (hl),a			;51e8   ; Al dispararse, la cuenta se recarga con el periodo de 0xE10E y se busca ficha libre
	ld hl,0e112h		;51e9
	ld b,003h		;51ec   ; Tres fichas donde elegir (cuatro desde la fase 5); la de 0xE12A no entra: es la reservada de la pareja
	ld a,(0e0e0h)		;51ee
	cp 005h		;51f1
...
CREA_BUSCA_FICHA:
	...
	ld a,006h		;51fa   ; El 6 salta a la ficha siguiente
	call SUMA_A_HL		;51fc
...
CREA_EN_ESTA:
	...
	inc hl			;5203
	ex de,hl			;5204
	ld hl,0e111h		;5205   ; 0xE111 cuenta 0-7 (el `res 3` lo pliega): la posicion en la lista de ocho del decorado
	inc (hl)			;5208
	res 3,(hl)		;5209
	ld a,(hl)			;520b
...
CREA_RELLENA:
	...
	ld a,(0e190h)		;523c   ; Si 0xE190 quedo levantado, la pareja: el complemento del lado en 0xE191 y la ficha reservada de 0xE12A
	rra			;523f
	ret nc			;5240
	ld a,(0e191h)		;5241
	cpl			;5244
	and 003h		;5245   ; los dos bits bajos son el lado
	ld c,a			;5247
	ld hl,0e12ah		;5248
	ld a,(hl)			;524b   ; Solo si la reservada esta libre; si no, la pareja se pierde
	or a			;524c
	jr nz,CREA_PAREJA_FIN		;524d
	inc (hl)			;524f   ; la ficha reservada queda ocupada
	inc hl			;5250
...
RELLENA_FICHA:		; Copia a la ficha el tipo, el puntero de dibujo y el de choque, sacados de la tabla de al lado
	...
	ld de,05277h		;5260
	ld a,c			;5263
	add a,a			;5264   ; Tipo por seis -el doble, el cuadruple y la suma-: lo que ocupa cada entrada de la tabla
	ld c,a			;5265
	add a,a			;5266
	add a,c			;5267
	call SUMA_A_DE		;5268
	ld a,(de)			;526b
	ld (hl),a			;526c   ; Los dos primeros bytes de la entrada, el puntero al primer trozo de dibujo
	inc de			;526d
	inc hl			;526e
	ld a,(de)			;526f
	ld (hl),a			;5270
	inc de			;5271
	inc hl			;5272
	ld (hl),e			;5273   ; Y como puntero de choque se guarda DE mismo: los cuatro bytes de pares estan ahi, en la propia tabla
	inc hl			;5274
	ld (hl),d			;5275
	ret			;5276
...
MIRA_LA_CURVA:		; Cada 200 del marcador (la distancia va en BCD: dispara con las centenas impares y el resto en 82) mira en la tabla de nibbles que curva toca y monta el cambio
	ld hl,(0e0e5h)		;52a1
	ld a,h			;52a4   ; La distancia va en BCD: dispara con las centenas impares y el resto en 82, cada 200 del marcador
	and 001h		;52a5
	ret z			;52a7
	ld a,l			;52a8
	cp 082h		;52a9
	ret nz			;52ab
	ld hl,0e0e2h		;52ac   ; 0xE0E2 es el indice de curva, que solo avanza aqui
	ld a,(hl)			;52af
	inc (hl)			;52b0
...
CURVA_MONTA:
	ld c,a			;52c2
	and 003h		;52c3   ; El nibble: los bits 0-1 son el rumbo de 0xE194 (11: recto, nada que hacer) y el bit 3 enciende el "va torcida" (bit 1)
	cp 003h		;52c5
	ret z			;52c7
	bit 3,c		;52c8
...
CURVA_GUARDA:
	...
	ld (hl),a			;52d1
	inc hl			;52d2
	bit 2,c		;52d3   ; El bit 2 del nibble alarga la curva: 0xE195 a 2
...
PINTA_HORIZONTE:		; Dibuja el horizonte que corresponde a la curva
	ld hl,053aeh		;52eb
PINTA_HORIZONTE_HL:
	ld a,(0e194h)		;52ee   ; 0xE194 por dos indexa los punteros: cada rumbo tiene su dibujo de horizonte
	add a,a			;52f1
	call SUMA_A_HL		;52f2
	ld e,(hl)			;52f5
	inc hl			;52f6
...
DESPLAZA_PINTA:
	...
	ld hl,0e197h		;532c
	inc (hl)			;532f   ; 0xE197 cuenta los desplazamientos: cada dieciseis se consume una tanda de 0xE195
	ld a,(hl)			;5330
	and 00fh		;5331   ; los cuatro bits bajos: una tanda cada dieciseis pasos
	jr nz,CURVA_ENDEREZA		;5333
	dec hl			;5335
	dec hl			;5336
	cp (hl)			;5337
	jr z,CURVA_ENDEREZA		;5338
	dec (hl)			;533a   ; y una tanda menos
	jr nz,CURVA_ENDEREZA		;533b
	dec hl			;533d   ; Agotadas las tandas, el rumbo (bit 0 de 0xE194) se INVIERTE y se repinta el horizonte: la curva se deshace girando al otro lado
	ld a,(hl)			;533e
	xor 001h		;533f
	ld (hl),a			;5341
...
CURVA_ENDEREZA:
	...
	ld hl,0e197h		;5350
	ld a,(hl)			;5353
	and 00fh		;5354   ; Solo con la cuenta de 0xE197 redonda, para no cortar una tanda a medias
	ret nz			;5356
	dec hl			;5357   ; 0xE196 a cero -la pista deja de girar- y el horizonte pasa a los dibujos de la llegada (0x53B6)
	ld (hl),a			;5358
	ld hl,053b6h		;5359
...
ARRASTRA_PINGUINO:		; En las curvas el pinguino se va de lado solo, al ritmo de la velocidad
	ld hl,0e196h		;5362
	ld a,(hl)			;5365   ; Sin curva en marcha (0xE196 a cero) no hay arrastre
	or a			;5366
	ret z			;5367
	inc hl			;5368
	inc hl			;5369
	dec (hl)			;536a
	ret nz			;536b
	ld a,(0e100h)		;536c   ; El mismo ritmo que el desplazamiento de la pista: periodo/4, contado en 0xE198
	srl a		;536f
	srl a		;5371
	ld (hl),a			;5373
	ld hl,0e0d1h		;5374   ; HL apunta a 0xE0D1, un byte de paso: el arrastre usa MUEVE_* sin tocar los bits de sentido de 0xE0FA
	ld de,(0e078h)		;5377
	ld a,(0e194h)		;537b
	rra			;537e
...
RETOCA_LADO:
	ld a,(0e194h)		;5475
	bit 1,a		;5478   ; El bit 1 de 0xE194 dice que la pista va torcida; el bit 0 (al acarreo por el `rra`) elige el juego de casillas, 0x10 mas abajo en un lado
	ret z			;547a
	rra			;547b
	ld a,(hl)			;547c
...
BASE_MULTIPLICA:
	...
	ld a,h			;54a2   ; Las ocho rotaciones en dos tandas montan HL/16: el nibble bajo de H y el alto de L
	rlca			;54a3
	rlca			;54a4
	rlca			;54a5
	rlca			;54a6
	...
	and 00fh		;54af
	or e			;54b1
	add a,c			;54b2   ; Sumado a la X de partida: la interpolacion lineal, paso/16 del camino hasta la bandera
	ld h,a			;54b3
BASE_ANDA:
	ld a,(0e078h)		;54b4
...
DIBUJA_LA_BASE_PASO:		; Alterna los dos bloques de la escena de la base
	...
	ld a,(hl)			;54ce
	inc (hl)			;54cf
	ld hl,054f8h		;54d0   ; El bit 0 de la cuenta 0xE13A elige bloque: pares el A (0x54F8), impares el B (0x550C)
	rra			;54d3
...
DIBUJA_EL_POLO:		; El remate del POLO SUR: descomprime cuatro sprites mas (0x6B2A -> VRAM 0x1F80, o sea los patrones 0xF0, 0xF4, 0xF8 y 0xFC), copia sus cuatro atributos de 0x66EF encima de los numeros 7 a 10 -dos en amarillo y dos en negro- y dibuja un tercer bloque de casillas. Los cuatro completan al pinguino inclinado que dibujan las casillas: el pico y la mancha de la barriga en amarillo, el ala y el lomo en negro
	ld hl,06b2ah		;54dd   ; 0x6B2A: los patrones comprimidos de los cuatro sprites del remate (0xF0-0xFC)
	call DESCOMPRIME		;54e0
	ld hl,066efh		;54e3
	ld de,0e06ch		;54e6   ; Los dieciseis bytes de 0x66EF, los atributos, encima de los sprites 7 a 10
	ld bc,00010h		;54e9
	ldir		;54ec
...
MONTA_LA_BASE:		; Escribe el nombre de la base y descomprime su bandera en los patrones de sprite
	...
	ld a,(0e0e1h)		;5537
	ld c,a			;553a   ; El nombre que toca sale de la tabla de las diez fases
	add a,a			;553b
	call SUMA_A_HL		;553c
	ld e,(hl)			;553f
	inc hl			;5540
	...
	ld hl,055f1h		;5549   ; La bandera se indexa aparte, con 0xE0E0, que es el numero del panel
	ld a,(0e0e0h)		;554c
	and 00fh		;554f
	add a,a			;5551
	call SUMA_A_HL		;5552
	...
	inc hl			;5556
	ld d,(hl)			;5557
	ex de,hl			;5558   ; Y detras de la bandera comprimida van sus dos colores
	ld de,05f40h		;5559
...
SUBE_LA_BANDERA:		; Sube la bandera dos pixeles por llamada hasta el tope del mastil. OJO CON DONDE SE PARA: el `cp 036h / ret z` se vuelve SIN GUARDAR el valor nuevo, asi que la bandera nunca llega a Y=0x36; se queda en 0x38. Medido en la llegada a Francia de la partida grabada: 0x50, 0x48, 0x3E y 0x38, y ahi se queda clavada
	...
	cp 036h		;556f
	ret z			;5571
	ld (0e060h),a		;5572   ; La misma Y en los tres atributos: los tres sprites de la bandera van montados en el mismo sitio
	ld (0e064h),a		;5575
	ld (0e068h),a		;5578
...
MONTA_LA_FUENTE:		; Monta la fuente y los colores en los tres bancos de la pantalla. OJO: que la fuente se escriba tres veces NO quiere decir que los tres bancos acaben iguales. Cada escena descomprime luego sus dibujos ENCIMA, banco por banco, y comparando los tres en una VRAM de verdad solo quedan iguales DIECINUEVE casillas: la 0x00-0x0F, que son los cuadrados de color liso, y la 0xFD-0xFF, que estan vacias. Cada tercio de la pantalla tiene su propio juego de 256 casillas, y por eso render_tiles.py saca una hoja por banco y no una para todo
	ld de,00000h		;5823   ; Los tres bancos de colores de SCREEN 2 (0x0000, 0x0800 y 0x1000), uno por llamada, con el `jp` haciendo de tercera
...
CARGA_BANCO_2:		; Descomprime los dibujos del banco 2
	ld hl,0626eh		;6238
	call DESCOMPRIME		;623b   ; Dos llamadas seguidas sin recargar HL: la segunda sigue el flujo donde acabo la primera, que cada bloque trae su destino
	call DESCOMPRIME		;623e
	ld hl,05bcfh		;6241
	call DESCOMPRIME_SIGUE		;6244
	ld hl,06270h		;6247
	ld de,072b0h		;624a   ; La entrada con espejo: el mismo dibujo con los bits del reves
...
SPRITES_BUCLE:
	...
	ld c,a			;6686   ; Un byte de cuantos y los cuatro del atributo; el cero cierra y se vuelca todo de una vez
...
DIBUJA_LA_META:		; En los ultimos 100 metros, cada 32 dibuja un trozo mas de la llegada
	...
	ret nz			;74cb
	ld a,l			;74cc
	rlca			;74cd   ; Las tres rotaciones y el doble convierten 0x00/0x20/0x40/0x60/0x80 en 0, 2, 4, 6 y 8: la palabra de la tabla que toca
	rlca			;74ce
	rlca			;74cf
	add a,a			;74d0   ; el `add a,a` remata el por 16
	ld hl,07506h		;74d1
	call SUMA_A_HL		;74d4
	ld e,(hl)			;74d7
	inc hl			;74d8
	...
	ex de,hl			;74da
	ld a,(hl)			;74db
	and 0f0h		;74dc   ; el nibble alto es el caracter
	ld c,a			;74de
	ld a,(hl)			;74df
	inc hl			;74e0
	and 003h		;74e1   ; y los dos bits bajos, el tercio
	add a,078h		;74e3
	ld d,a			;74e5
	ld a,c			;74e6
META_FILA:
	ld b,(hl)			;74e7
	inc hl			;74e8
	ld a,020h		;74e9   ; La misma cuenta que el interprete de bloques: 32 casillas por fila y el cierre 0xE0 de desplazamiento
	add a,c			;74eb
	ld c,a			;74ec
...
META_CASILLA:
	ld a,(hl)			;74f8
	or a			;74f9   ; El 0x00 cierra el bloque, igual que en DIBUJA_BLOQUE
	ret z			;74fa
	cp 0e0h		;74fb
...
PEZ_SIGUIENTE:
	...
	ret			;75b5
PEZ_SALE:
	ld (0e181h),de		;75b6   ; 0xE181 se queda apuntando al agujero; DE, a su tipo
	inc de			;75ba
	ld a,(0e18ah)		;75bb
	ld c,a			;75be
	...
	ld a,(0e009h)		;75c5
	and 00ch		;75c8   ; Con izquierda o derecha pulsadas el pez sale mirando alli; sin mando, alterna con 0xE185
	jr z,PEZ_LADO		;75ca
	bit 2,a		;75cc
	jr PEZ_MONTA		;75ce
PEZ_LADO:
...
PEZ_SALTO_MEDIO:
	ld b,064h		;75f6
PEZ_GUARDA:
	ld (hl),c			;75f8   ; C traia el 0x7A del `ld bc`: la Y de salida; B la X del salto y D el patron que quedo antes
	inc hl			;75f9
	ld (hl),b			;75fa
	inc hl			;75fb
	ld (hl),d			;75fc
...
PEZ_TARDE:		; Ya no da tiempo: se marca el agujero segun su tipo
	xor a			;75fe
	ld (0e192h),a		;75ff   ; El agujero queda marcado en los bits 5-7 de 0xE183 segun su tipo: de ahi saldra la foca
	ld a,(de)			;7602
	cp 001h		;7603
...
PEZ_PASO:		; Lo coloca, lo copia a la VRAM y le lleva el arco del salto
	...
	ld bc,00004h		;7623   ; cuatro bytes: el atributo entero del sprite
	call COPIA_A_VRAM		;7626
	ld de,0e183h		;7629
	ld a,(de)			;762c
	and 003h		;762d   ; los dos bits bajos: la fase del vuelo del pez
	ret z			;762f
	ld hl,0e08eh		;7630
	call PEZ_GIRA		;7633
	ld a,(de)			;7636
	...
	dec (hl)			;763b   ; dos pixeles por paso al subir
	dec (hl)			;763c
	jr PEZ_ARCO		;763d
PEZ_ARCO_SUBE:
	inc (hl)			;763f
...
PEZ_ARCO:
	push hl			;7641
	ld hl,0e184h		;7642   ; 0xE184 cuenta el vuelo: sube hasta el 8, plano hasta el 0x10, cae desde ahi y en el 0x22 se recoge
	inc (hl)			;7645
	ld a,(hl)			;7646
	pop hl			;7647
	dec hl			;7648
	cp 008h		;7649   ; hasta el 8 sube
	jr c,PEZ_SUBE		;764b
	cp 010h		;764d   ; del 8 al 0x10 vuela plano
	ret c			;764f
	jr z,PEZ_CAE		;7650
	cp 022h		;7652
	jr nc,QUITA_EL_PEZ		;7654
	ld c,005h		;7656   ; Cinco por paso al caer, y siete desde el 0x1A
	cp 01ah		;7658
...
PEZ_CAE:		; Al llegar al paso 0x10 del arco le SUMA 8 al byte del patron (0xE08E): el pez cambia al dibujo grande. Con eso y el bit 2 que voltea 0x7674 salen OCHO dibujos, cuatro por lado: 0x80-0x8C mirando a un lado y 0x90-0x9C al otro. Los ocho estan MEDIDOS en la partida grabada (work/sprites_medidos.txt), siempre en el color del atributo 15
	...
	inc hl			;7666
	ld a,(hl)			;7667
	add a,008h		;7668   ; Ocho arriba en el patron: el dibujo grande
	ld (hl),a			;766a
	ret			;766b
QUITA_EL_PEZ:		; Lo saca de la pantalla poniendole Y=0xE0
	ld (hl),0e0h		;766c
	xor a			;766e   ; Y borra estado y paso (0xE183/0xE184): el agujero queda libre
	ld (de),a			;766f
	inc de			;7670
	ld (de),a			;7671
	jr PEZ_PASO		;7672
PEZ_GIRA:		; Cada 16 fotogramas le da la vuelta al BIT 2 del patron y le deja los dos de abajo a cero: eso es lo que anima al pez. Los tres `srl` mas el `ccf` mas los tres `rla` dejan (patron & 0xF8) | (bit2 invertido) << 2
	ld a,(0e003h)		;7674
	and 00fh		;7677   ; Uno de cada dieciseis fotogramas
	ret nz			;7679
	ld a,(hl)			;767a
	srl a		;767b   ; Tres a la derecha, el acarreo invertido y tres a la izquierda: el bit 2 da la vuelta y los bits 0-1 quedan a cero
	srl a		;767d
	srl a		;767f
	ccf			;7681
	rla			;7682
...
ACELERA:		; Un escalon MENOS de periodo cada doce fotogramas, hasta 8: el tope de velocidad
	ld hl,0e0fdh		;76e2
	xor a			;76e5   ; El contador del freno se pone a cero: cambiar de idea empieza la cuenta de nuevo
	ld (hl),a			;76e6
	inc hl			;76e7
	inc hl			;76e8
	ld (hl),a			;76e9
	...
	inc (hl)			;76eb
	ld a,(hl)			;76ec
	sub 00ch		;76ed   ; 0x0C fotogramas para subir un escalon de velocidad
	ret nz			;76ef
	ld (hl),a			;76f0
	ld hl,0e100h		;76f1
	ld a,(hl)			;76f4
	cp 009h		;76f5   ; Nunca por debajo de 8, que es todo lo rapido que va el juego
	ret c			;76f7
	dec (hl)			;76f8
	ret			;76f9
FRENA:		; Un escalon MAS cada cuatro fotogramas, hasta 0x13
	ld hl,0e0fdh		;76fa
	xor a			;76fd   ; Y este apaga el del acelerador: cuatro fotogramas contra doce, se frena tres veces mas deprisa
	ld (hl),a			;76fe
	inc hl			;76ff
	ld (hl),a			;7700
	inc hl			;7701
	inc (hl)			;7702
	ld a,(hl)			;7703
	sub 004h		;7704   ; cuatro fotogramas para bajar un escalon de velocidad
	ret nz			;7706
	ld (hl),a			;7707
	ld hl,0e100h		;7708
	ld a,(hl)			;770b
	cp 013h		;770c   ; El tope lento es 0x13
	ret nc			;770e
	inc (hl)			;770f   ; y se baja un escalon
NI_UNA_COSA_NI_OTRA:
	ret			;7710
PINTA_VELOCIMETRO:		; Las seis casillas del velocimetro, en la fila 0
	...
	ld a,(0e100h)		;771d
	ld b,a			;7720
	and 001h		;7721   ; el bit 0 elige entre las dos medias casillas
	add a,042h		;7723
	ld c,a			;7725
	ld a,b			;7726
	rra			;7727   ; La media vuelta y el `cpl` invierten el periodo -barra larga, periodo corto- y el bit 0 elige la media casilla (0x42/0x43)
	cpl			;7728
	and 00fh		;7729   ; los cuatro bits bajos son la longitud de la barra
	sub 006h		;772b
...
NUBE_NUEVA:
	ld a,(hl)			;775a
	or a			;775b
	ld a,004h		;775c   ; El 4 es el salto de ficha para el `SUMA_A_DE` de abajo: cuatro bytes de atributo por nube
	jr nz,NUBE_SIGUIENTE		;775e
	push hl			;7760
	inc (hl)			;7761
	ld hl,077dfh		;7762   ; Donde empieza cada uno
	ld a,b			;7765
	add a,a			;7766   ; dos bytes por entrada
	call SUMA_A_HL		;7767
	ld a,(hl)			;776a
	ld (de),a			;776b
	inc hl			;776c
	...
	ld (de),a			;776f
	inc de			;7770
	ld a,0e0h		;7771   ; El patron de arranque es el 0xE0 y el color el blanco 0x0F
	ld (de),a			;7773
	inc de			;7774
	ld a,00fh		;7775
	ld (de),a			;7777
	ld a,001h		;7778   ; y la nube queda encendida
	pop hl			;777a
NUBE_SIGUIENTE:
	call SUMA_A_DE		;777b
	inc hl			;777e
	...
	ld (hl),a			;7789
	ld b,000h		;778a   ; B a cero: ahora hace de indice de nube para los desplazamientos
	ld hl,0e14ah		;778c
	ld de,0e0b8h		;778f
MUEVE_LAS_NUBES:
	ld a,(hl)			;7792
...
NUBE_PASO:
	...
	inc (hl)			;77a3
	ex de,hl			;77a4
	dec (hl)			;77a5   ; El paso: la Y sube uno y la X se corre lo que diga la tabla de 0x77DD para esa nube
	push de			;77a6
	ld de,077ddh		;77a7   ; La tabla de 0x77DD lleva un byte con signo por nube: -1, +1, -2 y +2
	ld a,b			;77aa
	call SUMA_A_DE		;77ab
	ld a,(de)			;77ae
	inc hl			;77af
	...
	ld (hl),a			;77b1
	ex de,hl			;77b2
	pop hl			;77b3   ; La Y de la nube, que el `dec (hl)` acaba de subir un pixel
	ld a,(hl)			;77b4
	cp 00ch		;77b5   ; En los pasos 0x0C y 0x18 el patron pasa a 0xDC y 0xD8: la nube crece al acercarse
	ld a,0dch		;77b7
...
NUBE_CRECE:
	inc de			;77c2   ; El patron nuevo, al tercer byte del atributo
	ld (de),a			;77c3
NUBE_RECUPERA:
	pop de			;77c4
NUBE_AVANZA:
	ld a,004h		;77c5   ; El 4 salta al atributo siguiente
	call SUMA_A_DE		;77c7
	inc hl			;77ca
	ld a,004h		;77cb   ; Cuatro nubes, y el volcado de los cuatro atributos de una vez (sprites 26 a 29)
	inc b			;77cd
	cp b			;77ce
...
ANIMA_LA_FOCA:		; Saca la foca del agujero: ocho pasos, del 7 al 14, con su fotograma sacado de la tabla de 0x7868. Dibujada, se la reconoce: primero asoma un filo, luego la cabeza, y del paso 10 en adelante el cuerpo entero con las dos aletas
	ld a,(0e183h)		;77e9
	and 0e0h		;77ec   ; Los bits 5-7 de 0xE183 los dejo PEZ_TARDE: sin ellos no hay foca
	ret z			;77ee
...
FOCA_FOTOGRAMA:		; Coge el fotograma del paso en que va
	...
	add a,008h		;7805
	ld b,a			;7807
	add a,a			;7808   ; dos bytes por entrada
	call SUMA_A_HL		;7809
	ld e,(hl)			;780c
	inc hl			;780d
	ld d,(hl)			;780e
	ld a,b			;780f
	ld b,004h		;7810   ; cuatro sprites para la foca
	cp 006h		;7812   ; En los pasos 13 y 14, con 0xE137 apagado, se enciende 0xE192: la foca pasa a los sprites 0-3, delante de todo
	jr c,FOCA_PASO		;7814
	ld hl,0e137h		;7816
	bit 0,(hl)		;7819
...
FOCA_PASO:
	cp 003h		;7822
	ex de,hl			;7824
	ld d,00ch		;7825   ; D es el tamano de cada variante: 12 bytes (cuatro sprites) o 6 (dos, en los tres primeros pasos)
	jr nc,FOCA_AVANZA		;7827
	ld d,006h		;7829
	ld b,002h		;782b
FOCA_AVANZA:
	ld a,(0e183h)		;782d
	cp 040h		;7830   ; 0x40 (tipo 0): la primera variante; 0x20: una mas alla; 0x80: dos, saltando D bytes por variante
...
FOCA_BYTE:		; Copia Y, X y patron, y se SALTA el cuarto byte del atributo: el color. Por eso el color de la foca no esta en el fotograma sino en la lista de atributos de 0x6698, que le deja el primer sprite en NEGRO y los otros tres en ROJO OSCURO. Dibujada asi es una foca con la cara oscura, porque el negro es el atributo 16 y en un MSX el numero mas bajo va delante
	...
	inc de			;784b   ; El `inc de` extra salta el byte de color del atributo, que no se toca
...
PIDE_SONIDO:		; Pone en marcha el sonido A, si su numero manda mas que el que ya suena
	di			;7970   ; Con las interrupciones paradas: ATIENDE_SONIDO corre en la interrupcion y toca estos mismos bloques
	push hl			;7971
	push de			;7972
	push bc			;7973
	push af			;7974
	call PIDE_SONIDO_SIN_GUARDAR		;7975   ; La entrada sin salvar registros es la que usan el rearranque y el encadenado
	pop af			;7978
	pop bc			;7979
	pop de			;797a
	pop hl			;797b
...
SONIDO_PRIORIDAD:
	...
	call SUMA_A_DE		;799c
SONIDO_MONTA_CANAL:
	dec hl			;799f   ; Los dos `dec` bajan del +2 del canal al +0: un fotograma de arranque, duracion 1 y el numero, que es lo que marca el canal ocupado
	dec hl			;79a0
	ld (hl),001h		;79a1   ; la cuenta de arranque, a uno
	inc hl			;79a3
	ld (hl),001h		;79a4
	inc hl			;79a6
	ld a,c			;79a7
	...
	ld a,(de)			;79ae
	ld (hl),a			;79af
	ld a,008h		;79b0   ; Ocho mas alla del +4 es el +2 del canal siguiente; los flujos de un sonido de varios canales van seguidos en la tabla
	call SUMA_A_HL		;79b2
	inc de			;79b5
	djnz SONIDO_MONTA_CANAL		;79b6   ; tantos canales como diga B
SONIDO_NO:
	ret			;79b8
SONIDO_REPITE:		; El 0xFE: repite el trozo las veces que diga el byte de detras
	inc hl			;79b9
	ld a,(hl)			;79ba
	inc a			;79bb   ; El 0xFF (el `inc a` lo caza) repite siempre; si no, el +9 cuenta las vueltas
	jr z,SONIDO_ENCADENA		;79bc
	inc (ix+009h)		;79be
	dec a			;79c1
	cp (ix+009h)		;79c2
	jr nz,SONIDO_ENCADENA		;79c5
	xor a			;79c7   ; Cumplidas las vueltas, el +9 a cero y el canal calla
	ld (ix+009h),a		;79c8
	jp CALLA_CANAL		;79cb
SONIDO_ENCADENA:		; Al acabarse un flujo puede arrancar otro
	ld a,(ix+002h)		;79ce   ; Rearrancar es volver a pedir el numero del +2: como empata consigo mismo, la prioridad lo deja pasar
	push bc			;79d1
...
SUENA_UN_PASO:		; Un paso del reproductor de sonido: recorre los TRES canales, con diez bytes de estado cada uno desde 0xE010
	ld c,001h		;79d7   ; C = 1: el registro alto del par de periodo del primer canal
	ld ix,0e010h		;79d9   ; IX recorre los tres bloques de estado, de diez en diez bytes
	exx			;79dd
	ld b,003h		;79de
	ld de,0000ah		;79e0
CANAL_SIGUIENTE:		; Pasa al canal siguiente, diez bytes de estado mas alla
	exx			;79e3
	ld a,(ix+002h)		;79e4   ; El +2 es el numero del sonido: a cero, el canal esta libre y no se toca
	or a			;79e7
	call nz,CANAL_SIGUE		;79e8
	inc c			;79eb   ; C sube de dos en dos (1, 3, 5): el registro alto de cada canal del PSG
	inc c			;79ec
	exx			;79ed
	add ix,de		;79ee
...
CANAL_SIGUE:		; Descuenta lo que le queda a la nota del canal y, si se acaba, coge el byte siguiente de su flujo
	jp m,NOTA_DECAE		;79f4   ; El signo del +2 -o sea su bit 7- separa la musica del efecto
	dec (ix+000h)		;79f7   ; El +0 es lo que le queda a la nota; hasta que no llega a cero no se lee nada mas
	ret nz			;79fa
CANAL_LEE_FLUJO:		; Coge el byte siguiente del flujo de este canal
	ld l,(ix+003h)		;79fb   ; El puntero del flujo vive en el +3 y el +4
	ld h,(ix+004h)		;79fe
	ld a,(hl)			;7a01
	cp 0feh		;7a02   ; 0xFE repite y 0xFF calla: los dos eventos comunes a musica y efecto
	jr z,SONIDO_REPITE		;7a04
	jr nc,CALLA_CANAL		;7a06
	bit 7,(ix+002h)		;7a08   ; El bit 7 del NUMERO de sonido -no del evento- es lo que manda
	jp nz,VOLUMEN_MIRA_FD		;7a0c
	and 0f0h		;7a0f   ; Un evento 0x2n de efecto trae duracion nueva en el nibble bajo
	cp 020h		;7a11
...
CANAL_NOTA:		; El nibble alto es la nota y el bajo lo que dura
	ld a,(hl)			;7a1c   ; Nibble alto el volumen; el bajo y el byte siguiente, el periodo de 12 bits
	and 0f0h		;7a1d
	ld b,a			;7a1f
	xor (hl)			;7a20   ; El `xor (hl)` deja el nibble bajo, que es la parte alta del periodo
	ld d,a			;7a21
	inc hl			;7a22
	ld e,(hl)			;7a23
	inc hl			;7a24
	ld (ix+003h),l		;7a25   ; Se apunta por donde va el flujo antes de tocar el PSG
	ld (ix+004h),h		;7a28
	ex de,hl			;7a2b
	call ESCRIBE_PERIODO		;7a2c
	ld a,b			;7a2f   ; Cuatro rotaciones bajan el volumen a su nibble
	rrca			;7a30
	rrca			;7a31
	rrca			;7a32
	rrca			;7a33
...
CANAL_ATACA:		; Arranca la nota: recarga su duracion y prepara el decaimiento del volumen
	ld h,a			;7a36
	ld a,(ix+001h)		;7a37   ; La duracion del +1 pasa al +0, que es lo que se descuenta
	ld (ix+000h),a		;7a3a
	add a,003h		;7a3d   ; Y el +8 arranca 3 por encima: ese hueco es el decaimiento
	ld (ix+008h),a		;7a3f
...
NOTA_DECAE:		; Mientras dura la nota le va bajando el volumen
	...
	dec (ix+008h)		;7a50   ; El +8 arranco 3 por encima del +0; mientras no se igualen baja DOS por vuelta (esta y DECAE_MAS) y el volumen cae un paso
	ld a,(ix+008h)		;7a53
	cp (ix+000h)		;7a56
...
DECAE_VOLUMEN:
	...
	ld h,a			;7a6b
ESCRIBE_VOLUMEN:
	ld a,c			;7a6c   ; La media vuelta convierte C (1, 3, 5) en 0x80, 0x81 y 0x82, y el `add 0x88` los deja en 8, 9 y 10: el bit alto se va por el acarreo
	rrca			;7a6d
	add a,088h		;7a6e
	out (0a0h),a		;7a70   ; Registro por 0xA0 y valor por 0xA1: el PSG a pelo
	ld a,h			;7a72
	out (0a1h),a		;7a73
	ret			;7a75
VOLUMEN_MIRA_FD:		; El 0xFD del flujo es una orden, no un volumen
	cp 0fdh		;7a76   ; El 0xFD es el unico evento de control de la musica: cambia octava y volumen de golpe
	jr nz,VOLUMEN_APLICA		;7a78
	inc hl			;7a7a
	ld a,(hl)			;7a7b
	and 007h		;7a7c   ; Los tres bits bajos son la octava, al +5
	ld (ix+005h),a		;7a7e
	xor (hl)			;7a81   ; El `xor (hl)` deja los cinco altos y tres rotaciones los bajan: el volumen de arranque, al +6
	rrca			;7a82
	rrca			;7a83
	rrca			;7a84
	ld (ix+006h),a		;7a85
	inc hl			;7a88   ; Y detras del 0xFD viene ya la nota, sin evento propio
	ld a,(hl)			;7a89
VOLUMEN_APLICA:		; Deja el volumen del canal como diga el flujo
	and 00fh		;7a8a   ; Nibble bajo: la nota, de 0 a 11, y el 12 es el silencio
	ld b,a			;7a8c
	xor (hl)			;7a8d   ; El `xor (hl)` deja el alto, que es el indice de duracion
	inc hl			;7a8e   ; La pista avanza UN byte: una nota de musica ocupa uno solo
	ld (ix+003h),l		;7a8f
	ld (ix+004h),h		;7a92
	rrca			;7a95   ; Cuatro rotaciones para bajarlo, que la tabla de duraciones es de bytes
	rrca			;7a96
	rrca			;7a97
	rrca			;7a98
	ld hl,07ae4h		;7a99
	call SUMA_A_HL		;7a9c
	ld a,(hl)			;7a9f
	ld (ix+001h),a		;7aa0   ; Restar 12 mira si la nota era el silencio y de paso deja el volumen a CERO, que es justo lo que toca si lo era; si no, se repone del +6
	ld a,b			;7aa3
	sub 00ch		;7aa4
	ld (ix+007h),a		;7aa6
...
VOLUMEN_ATACA:		; Arranca la nota y va a por su periodo
	call CANAL_ATACA		;7ab1   ; Primero el volumen y las cuentas; B ha guardado la nota para el periodo
	ld a,b			;7ab4
	ld hl,07ad8h		;7ab5
	call SUMA_A_HL		;7ab8
	ld l,(hl)			;7abb
	ld h,000h		;7abc   ; La tabla de doce periodos, la octava mas aguda
	ld a,(ix+005h)		;7abe
	or a			;7ac1   ; La octava del +5 a cero deja el periodo tal cual
	jr z,ESCRIBE_PERIODO		;7ac2
	ld b,a			;7ac4
PERIODO_DESPLAZA:		; Desplaza el periodo tantas octavas como diga B
	add hl,hl			;7ac5   ; Cada doblado del periodo BAJA una octava
	djnz PERIODO_DESPLAZA		;7ac6
ESCRIBE_PERIODO:		; Manda al PSG los dos bytes del periodo del canal, por los puertos 0xA0 y 0xA1
	ld a,c			;7ac8   ; Primero el registro alto del par (C) y luego el bajo (C-1), los dos por 0xA0/0xA1
	out (0a0h),a		;7ac9
	ld a,h			;7acb
	out (0a1h),a		;7acc
	dec c			;7ace   ; El `dec c` baja al registro bajo del par y el `inc c` de abajo lo deja como estaba
	ld a,c			;7acf
	out (0a0h),a		;7ad0
	ld a,l			;7ad2
	out (0a1h),a		;7ad3
